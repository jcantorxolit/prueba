...
use AdeN\Api\Modules\Onboarding\OnboardingRepository;
use Str;
use Auth;
use Mail;
use Event;
...
class User extends UserBase
{
     ...
     */
    public $rules = [
        'email'    => 'required|between:2,64|email|unique:users',
        'avatar'   => 'nullable|image|max:4000',
        'username' => 'required|between:2,64|unique:users',
        'password' => 'required:create|between:2,64|confirmed',
        'password_confirmation' => 'required_with:password|between:2,64'
    ];
    ...
    public function beforeValidate()
    {
        ...
        $this->rules['password'] = "required:create|between:$minPasswordLength,64|confirmed";
        $this->rules['password_confirmation'] = "required_with:password|between:$minPasswordLength,64";
    }
    ...
    {
        $fields->company->hidden = false;
        $fields->wg_provider->hidden = false;

        if ($this->wg_type == 'system') {
            $fields->company->hidden = true;
        } else if ($this->wg_type == 'provider') {
            $fields->wg_provider->hidden = true;
        }
    }

    public function getWgProviderOptions()
    {
        return [];
    }
}
