...
class Account extends ComponentBase
{
    ...
    public function onSignin()
    {
        try {
            ...
            */
             ...
             * Redirect
             */
            if ($this->makeRedirection(true)) {
                \Log::info('redirectIntendedToApp');
                return $this->redirectIntendedToApp($user);
            }
    ...
    public static function onCreate($data)
    {
         ...
         */
        if ($userActivation) {
            (new self)->sendActivationEmail($user);
        }
    ...
    protected function redirectIntendedToApp($user)
    {
        $redirectUrl = "/app/clientes/list";

        if ($user->wg_term_condition == null || $user->wg_term_condition == 0) {
            $redirectUrl = "/app/term-condition";
        } else if ($user->wg_type == "customerAdmin" || $user->wg_type == "customerUser") {
            $redirectUrl = "/app/clientes/edit/" . $user->company;
            ...
            $agent = Agent::whereUserId($user->id)->first();
            if ($agent != null) {
                //$redirectUrl = "/app/investigation/list" ;
                //if (Config::get('rootUri', '/sylogi/') == "/bolivar/") {
                    if ($agent->type == "02") {
                        //Externo, Solo investigaciones
                        $redirectUrl = "/app/investigation/list";
                    }
                //}
            } else {
        ...
        } else if ($user->wg_type == "provider") {
            $redirectUrl = "/app/professor-event-provider/list";
        }
