...
class Session extends ComponentBase
{
    ...
    public function onRun()
    {
        ...
        $user =  $this->user();
        if ($user) {
            $termsConditions = $user->wg_term_condition;
            if (($termsConditions == null || $termsConditions == 0) && Request::path() != "app/term-condition") {
                $redirectUrl = "/app/term-condition";
                return Redirect::intended($redirectUrl);
            } else if (
                $termsConditions != null && $termsConditions == 1 &&
                (starts_with(Request::path(), 'app/clientes/edit') || starts_with(Request::path(), 'app/clientes/view'))
            ) {
                $this->run();
    ...
    protected function checkUserSecurity()
    {
        ...
        if ($isAuthenticated) {
            ...
            if (!empty($allowedUserGroups)) {
                ...
                $allowedUserGroups = is_string($allowedUserGroups) ? [ $allowedUserGroups ] : $allowedUserGroups;
