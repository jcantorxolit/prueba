...
class ProjectDocumentJob
{
    ...
    public function fire($job, $data)
    {
        ...
        foreach ($data["criteria"]["mandatoryFilters"] as $filter) {            
            $mandatoryFilter = new \stdClass();
            $mandatoryFilter->operator = $filter["operator"];
            $mandatoryFilter->value = $filter["value"];
            $mandatoryFilter->field = $filter["field"];
            $newCriteria->mandatoryFilters[] = $mandatoryFilter;
        }
        $newCriteria->filter = $criteria->filter;       

        $data = $this->repository->getExportData($newCriteria);

        $excelFilename = 'GUIA_DOCUMENTOS_ACTIVIDADES_PLAN_TRABAJO_' . Carbon::now()->timestamp;
        ExportHelper::excelStorage($excelFilename, 'GUIA', $data['excel']);

        $zipFilename = 'DOCUMENTOS_ACTIVIDADES_PLAN_TRABAJO_' . Carbon::now()->timestamp . '.zip';
    ...
    {               
        $criteria->module = "DOCUMENTOS ACTIVIDADES PLAN DE TRABAJO";
        $criteria->url = $url;        
