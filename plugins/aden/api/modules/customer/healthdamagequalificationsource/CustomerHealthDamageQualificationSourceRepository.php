...
namespace AdeN\Api\Modules\Customer\HealthDamageQualificationSource;
...
class CustomerHealthDamageQualificationSourceRepository extends BaseRepository
{
    public function __construct()
    {
        parent::__construct(new CustomerHealthDamageQualificationSourceModel());
    ...
    // AllHealthDamageQualificationSource

    public function AllHealthDamageQualificationSource($criteria)
    {
        $urlImages = "uploads/public";

        $this->setColumns([
            "id" => "wg_customer_health_damage_qs.id",
            "createdAt" => "wg_customer_health_damage_qs.created_at",
        ...
        $query->join("wg_customer_employee", function ($join) {
            $join->on('wg_customer_health_damage_qs.customer_employee_id', '=', 'wg_customer_employee.id');
        ...
        //LEFT JOIN (SELECT * FROM system_parameters WHERE system_parameters.namespace = 'wgroup' AND system_parameters.GROUP = 'arl') wg_arl
            //ON `wg_customer_health_damage_qs`.`arl` = `wg_arl`.`value`
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('arl','wg_arl')), function ($join) {
            $join->on('wg_customer_health_damage_qs.arl', '=', 'wg_arl.value');
