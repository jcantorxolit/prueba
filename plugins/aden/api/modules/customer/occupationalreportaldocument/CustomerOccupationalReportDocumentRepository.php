...
class CustomerOccupationalReportDocumentRepository extends BaseRepository
{
    protected $service;

    public function __construct()
    {
        parent::__construct(new CustomerOccupationalReportDocumentModel());
    ...
    {
        $this->setColumns([
            "id" => "wg_customer_occupational_report_al_document.id",
            "documentType" => "document_type.item AS documentType",
            "classification" => "customer_document_classification.item AS classification",
            "description" => "wg_customer_occupational_report_al_document.description",
            "version" => "wg_customer_occupational_report_al_document.version",
            "createdAt" => "wg_customer_occupational_report_al_document.created_at",
            ...
            "statusCode" => "wg_customer_occupational_report_al_document.status AS statusCode",
            ...
            "CustomerOccupationalReportId" => "wg_customer_occupational_report_al_document.customer_occupational_report_id",            
        ]);
        ...
        $query->leftjoin(DB::raw(CustomerModel::getDocumentTypeRelation('document_type')), function ($join) {
            ...
            $join->on('wg_customer_occupational_report_al_document.label', '=', 'minimum_standard_item_documenta_0312_label.value');
        })->join("wg_customer_occupational_report_al", function ($join) {
            $join->on('wg_customer_occupational_report_al.id', '=', 'wg_customer_occupational_report_al_document.customer_occupational_report_id');
        })->leftjoin("wg_customer_document_security", function ($join) {
            $join->on('wg_customer_occupational_report_al.customer_id', '=', 'wg_customer_document_security.customer_id');
            $join->on('wg_customer_occupational_report_al_document.type', '=', 'wg_customer_document_security.documentType');
            $join->on('wg_customer_occupational_report_al_document.origin', '=', 'wg_customer_document_security.origin');
        })->leftjoin(DB::raw(CustomerDocumentModel::getSecurityUserRelationTable('security')), function ($join) use ($authUser) {
            $join->on('wg_customer_occupational_report_al_document.id', '=', 'security.id');
        ...
        $result["data"] = $this->parseModelWithDocument($data, CustomerOccupationalReportDocumentModel::CLASS_NAME);
    ...
    public function insertOrUpdate($entity)
    {
        ...
        $entityModel->CustomerOccupationalReportId = $entity->CustomerOccupationalReportId;
    ...
    public function parseModelWithRelations($model)
    {
        ...
        if ($model instanceof $modelClass) {
            ...
            $entity->id = $model->id;
            $entity->CustomerOccupationalReportId = $model->CustomerOccupationalReportId;
    ...
    private function getYears($customerId)
    {
        ...
        $query
            ...
            ->select(DB::raw("YEAR(wg_customer_occupational_report_al_document.created_at) as year"))
            ->where("wg_customer_occupational_report_al.customer_id", $customerId)
            ->groupBy(DB::raw("YEAR(wg_customer_occupational_report_al_document.created_at)"))
            ->orderBy(DB::raw("YEAR(wg_customer_occupational_report_al_document.created_at)", "DESC"));
    ...
    private function getMonths($customerId, $year)
    {
        ...
        $query
            ...
            ->select(DB::raw("MONTH(wg_customer_occupational_report_al_document.created_at) as month"))
            ->where("wg_customer_occupational_report_al.customer_id", $customerId)
            ->when($year != null, function($query) use($year) {
                $query->whereYear("wg_customer_occupational_report_al_document.created_at", $year);
            })               
            ->groupBy(DB::raw("MONTH(wg_customer_occupational_report_al_document.created_at)"));
    ...
    public function export($criteria, $zipFilename = null)
    {
        ...
        Queue::push(CustomerOccupationalReportDocumentJob::class, ['criteria' => $criteria], 'zip');
