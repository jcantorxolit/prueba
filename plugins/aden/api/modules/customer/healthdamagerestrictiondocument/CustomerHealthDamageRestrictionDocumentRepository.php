...
namespace AdeN\Api\Modules\Customer\HealthDamageRestrictionDocument;
...
class CustomerHealthDamageRestrictionDocumentRepository extends BaseRepository
{
    public function __construct()
    {
        parent::__construct(new CustomerHealthDamageRestrictionDocumentModel());

        $this->service = new CustomerHealthDamageRestrictionDocumentService();
    }
    ...
    // AllHealthDamageRestrictionDocument

    public function AllHealthDamageRestrictionDocument($criteria)
    {
        $urlImages = "uploads/public";

        $this->setColumns([
            "id" => "wg_customer_health_damage_restriction_document.id",
            "type" => "work_health_damage_restriction_document_type.item AS type",
            "name" => "wg_customer_health_damage_restriction_document.name",
            "description" => "wg_customer_health_damage_restriction_document.description",
            "version" => "wg_customer_health_damage_restriction_document.version",
            "status" => "wg_customer_health_damage_restriction_document.status",
            "customerHealthDamageRestrictionId" => "wg_customer_health_damage_restriction_document.customer_health_damage_restriction_id",
        ]);
        ...
        $query->join("wg_customer_health_damage_restriction", function ($join) {
            $join->on('wg_customer_health_damage_restriction_document.customer_health_damage_restriction_id', '=', 'wg_customer_health_damage_restriction.id');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('work_health_damage_restriction_document_type','work_health_damage_restriction_document_type')), function ($join) {
            $join->on('wg_customer_health_damage_restriction_document.type', '=', 'work_health_damage_restriction_document_type.value');
        });
