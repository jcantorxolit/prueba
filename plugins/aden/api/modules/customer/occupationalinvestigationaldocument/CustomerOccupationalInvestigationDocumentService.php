...
class CustomerOccupationalInvestigationDocumentService extends BaseService
{
    ...
    public function getExportData($criteria)
    {
        ...
        if ($uids != null) {
            $documents = CustomerOccupationalInvestigationDocumentModel::whereIn('id', $uids->value)->get();
        }
    ...
    private function prepareQueryExportData($criteria)
    {
        ...
        $query = DB::table('wg_customer_occupational_investigation_al_document')
            ->join('wg_customer_occupational_investigation_al', function ($join) use($customerId) {
                $join->on('wg_customer_occupational_investigation_al_document.customer_occupational_investigation_id', '=', 'wg_customer_occupational_investigation_al.id');
                if ($customerId) {
                    $join->where('wg_customer_occupational_investigation_al.customer_id', '=', $customerId->value);
                }
            })
            ->leftjoin(DB::raw(SystemParameter::getRelationTable('tracking_tiposeg')), function ($join) {
                $join->on('wg_customer_occupational_investigation_al.type', '=', 'tracking_tiposeg.value');
            ...
            ->join('system_files', function ($join) use ($customerId) {
                $join->on('wg_customer_occupational_investigation_al_document.id', '=', 'system_files.attachment_id');
                $join->where('system_files.attachment_type', '=', CustomerOccupationalInvestigationDocumentModel::CLASS_NAME);
                $join->where('system_files.field', '=', 'document');                
            })
            ->when(!empty($criteria->filter->type), function($query) use ($criteria) {
                $query->where('wg_customer_occupational_investigation_al_document.type', $criteria->filter->type->value);
                $query->where('wg_customer_occupational_investigation_al_document.origin', $criteria->filter->type->origin);
            })
            ->when(!empty($criteria->filter->year), function($query) use ($criteria) {
                $query->whereYear('wg_customer_occupational_investigation_al_document.created_at', $criteria->filter->year->value);                
            })
            ->when(!empty($criteria->filter->month), function($query) use ($criteria) {
                $query->whereMonth('wg_customer_occupational_investigation_al_document.created_at', $criteria->filter->month->value);                
            })     
            ->select(
                "tracking_tiposeg.item AS type",                
                "wg_customer_occupational_investigation_al.eventDateTime",
                "document_type.item AS documentType",
                "customer_document_classification.item AS classification",
                "wg_customer_occupational_investigation_al_document.description",
                "wg_customer_occupational_investigation_al_document.id",
                "wg_customer_occupational_investigation_al_document.version",
                "customer_document_status.item AS status",
                "wg_customer_occupational_investigation_al_document.created_at",
                ...
                'wg_customer_occupational_investigation_al.customer_id AS customerId'
            )
            ->whereRaw("(`wg_customer_occupational_investigation_al`.`customer_id` = `document_type`.`customer_id` OR `document_type`.`customer_id` IS NULL)");

        if ($uids != null) {
            $query->whereIn('wg_customer_occupational_investigation_al_document.id', $uids->value);
        }
