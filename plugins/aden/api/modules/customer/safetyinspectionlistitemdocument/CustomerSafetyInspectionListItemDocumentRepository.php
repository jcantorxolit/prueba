...
class CustomerSafetyInspectionListItemDocumentRepository extends BaseRepository
{
    public function __construct()
    {
        parent::__construct(new CustomerSafetyInspectionListItemDocumentModel());

        $this->service = new CustomerSafetyInspectionListItemDocumentService();
    }
    ...
    public function all($criteria)
    {
        ...
        $this->setColumns([
            "id" => "wg_customer_safety_inspection_list_item_document.id",
            "type" => "contract_detail_document_type.item AS documentType",
            "description" => "wg_customer_safety_inspection_list_item_document.description",
            "version" => "wg_customer_safety_inspection_list_item_document.version",
            "createdAt" => "wg_customer_safety_inspection_list_item_document.created_at",
            ...
            "customerSafetyInspectionListItemId" => "wg_customer_safety_inspection_list_item_document.customer_safety_inspection_list_item_id",
        ]);
        ...
        $query->join('wg_customer_safety_inspection_list_item', function ($join) {
            $join->on('wg_customer_safety_inspection_list_item.id', '=', 'wg_customer_safety_inspection_list_item_document.customer_safety_inspection_list_item_id');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable("contract_detail_document_type")), function ($join) {
            $join->on('wg_customer_safety_inspection_list_item_document.type', '=', 'contract_detail_document_type.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable("customer_document_status")), function ($join) {
            $join->on('wg_customer_safety_inspection_list_item_document.status', '=', 'customer_document_status.value');

        })/*->leftjoin(DB::raw(CustomerSafetyInspectionListItemDocumentModel::getSystemFile()), function ($join) {
            $join->on('wg_customer_safety_inspection_list_item_document.id', '=', 'system_files.attachment_id');

        })*/->leftjoin('users', function ($join) {
            $join->on('users.id', '=', 'wg_customer_safety_inspection_list_item_document.created_by');
        ...
        $result["data"] = $this->parseModelWithDocument($data, CustomerSafetyInspectionListItemDocumentModel::CLASS_NAME);
