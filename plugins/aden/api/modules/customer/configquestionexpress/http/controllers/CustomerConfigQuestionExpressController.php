...
namespace AdeN\Api\Modules\Customer\ConfigQuestionExpress\Http\Controllers;
...
class CustomerConfigQuestionExpressController extends BaseController
{
    use UserSecurity;
    ...
    public function __construct()
    {
        ...
        $this->repository = new CustomerConfigQuestionExpressRepository();
    ...
    public function index()
    {
        ...
        try {
            ...
            $defaultFilters = [
                ...
                array("field" => 'workplaceId', "operator" => 'like', "value" => $criteria->search),
                array("field" => 'questionExpressId', "operator" => 'like', "value" => $criteria->search),
                array("field" => 'rate', "operator" => 'like', "value" => $criteria->search),
    ...
    public function showHazard()
    {
        ...
        try {
            $criteria = HttpHelper::parse($content, false);
            $result = $this->repository->findHazard($criteria);
            $this->response->setResult($result);
        } catch (\Exception $ex) {
            Log::error($ex);
            $this->response->setStatuscode(500);
    ...
    public function showHazardIntervention()
    {
        $content = $this->request->get("data", "");;

        try {
            $criteria = HttpHelper::parse($content, false);
            $result = $this->repository->findHazardIntervention($criteria);
            $this->response->setResult($result);
        } catch (\Exception $ex) {
            Log::error($ex);
            $this->response->setStatuscode(500);
