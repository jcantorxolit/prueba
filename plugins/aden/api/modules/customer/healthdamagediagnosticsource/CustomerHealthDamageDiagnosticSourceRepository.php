...
namespace AdeN\Api\Modules\Customer\HealthDamageDiagnosticSource;
...
class CustomerHealthDamageDiagnosticSourceRepository extends BaseRepository
{
    public function __construct()
    {
        parent::__construct(new CustomerHealthDamageDiagnosticSourceModel());
    ...
    // AllHealthDamageDiagnosticSource

    public function AllHealthDamageDiagnosticSource($criteria)
    {
        $urlImages = "uploads/public";

        $this->setColumns([
            "customerId" => "wg_customer_employee.customer_id",
            "createdAt" => "wg_customer_health_damage_diagnostic_source.created_at",
            ...
            "id" => "wg_customer_health_damage_diagnostic_source.id",
        ]);
        ...
        $query->join("wg_customer_employee", function ($join) {
            $join->on('wg_customer_health_damage_diagnostic_source.customer_employee_id', '=', 'wg_customer_employee.id');
        ...
        //LEFT JOIN (SELECT * FROM system_parameters WHERE system_parameters.namespace = 'wgroup' AND system_parameters.GROUP = 'arl') wg_arl
            // ON `wg_customer_health_damage_diagnostic_source`.`arl` = `wg_arl`.`value`
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('arl','wg_arl')), function ($join) {
            $join->on('wg_customer_health_damage_diagnostic_source.arl', '=', 'wg_arl.value');
