...
class CustomerCovidService extends BaseService
{
    ...
    public function getPeriodList($criteria)
    {
        $query = DB::table('wg_customer_covid')
            ...
            ->where('wg_customer_covid_head.customer_id', $criteria->customerId)
            ->groupBy(
                'wg_customer_covid_head.customer_id',
                DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m')")
            )
            ->orderBy("wg_customer_covid.registration_date", "desc");

        if(!empty($criteria->isEmployee)) {
            $query->whereNotNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",0);
        }
        if(!empty($criteria->isExternal)) {
            $query->whereNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",1);
        }
    ...
    public function getDateList($criteria)
    {
        $query = DB::table('wg_customer_covid')
            ...
            if(!empty($criteria->customerId)) {
                $query->where('wg_customer_covid_head.customer_id', $criteria->customerId);
                $query->addSelect(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%d/%m/%Y') AS item"));
                $query->addSelect(DB::raw("wg_customer_covid.registration_date AS value"));
                $query->groupBy("wg_customer_covid.registration_date");
        }
        if(!empty($criteria->covidHeadId)) {
            $query->where('wg_customer_covid.customer_covid_head_id', $criteria->covidHeadId);
            $query->addSelect(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') AS item"));
            $query->addSelect(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') AS value"));
            $query->groupBy(DB::raw("DATE_FORMAT(registration_date, '%Y%m')"));
        }
        if(!empty($criteria->isEmployee)) {
            $query->whereNotNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",0);
        }
    ...
    public function getExportData($criteria, $type = null)
    {
        $customerId = CriteriaHelper::getMandatoryFilter($criteria, 'customerId');
        $customerCovidId = CriteriaHelper::getMandatoryFilter($criteria, 'customerCovidId');

        $qQuestion = $this->prepareQuestionQuery($customerCovidId);
        $qAnswer =   $this->prepareAnswerQuery($customerCovidId);
        $qTemperature = $this->prepareTemperature($customerCovidId);
        ...
        $query->join("wg_customer_covid_head", function ($join) {
            ...
            ->select(
                ...
                "question.questions",
                'wg_customer_covid.customer_covid_head_id AS customerCovidId',
        ...
        if(!empty($customerCovidId)){
            $query->where("wg_customer_covid_head.id","=",$customerCovidId->value);
        }
        ...
        return ExportHelper::headings($query->get()->toArray(), $this->getHeader());
    }
    ...
    private function prepareQuestionQuery($customerCovidId)
    {
        return DB::table('wg_customer_covid_question')
            ...
            ->where('wg_customer_covid.customer_covid_head_id', $customerCovidId->value)
            ->select(
    ...
    private function prepareAnswerQuery($customerCovidId)
    {
        return DB::table('wg_customer_covid_question')
            ->join(DB::raw("wg_customer_covid"), function ($join) {
                $join->on('wg_customer_covid_question.customer_covid_id', '=', 'wg_customer_covid.id');
    ...
    private function prepareTemperature($customerCovidId)
    {
    ...
    public function getGenreCharPie($criteria)
    {
        ...
        $detail = DB::table("wg_customer_covid")
                    ->join("wg_customer_covid_head", "wg_customer_covid.customer_covid_head_id", "=", "wg_customer_covid_head.id")
                    ->whereRaw("wg_customer_covid_head.customer_id = {$criteria->customerId}")
                    ->select("wg_customer_covid.*")
                    ->groupBy("customer_covid_head_id");
        
        if ($criteria->type == 1) {
            $detail->whereRaw("wg_customer_covid.registration_date = ?", [$criteria->day]);
            $detail->groupBy("wg_customer_covid.registration_date");
        } else if ($criteria->type == 0) {
            $detail->whereRaw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') = ?", [$criteria->period]);
            $detail->groupBy(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m')"));
        }

  
        $query = DB::table(DB::raw(" ({$detail->toSql()}) as wg_customer_covid"))
            ...
            ->leftJoin('wg_customer_employee', function ($join) {
                $join->on('wg_customer_employee.id', '=', 'wg_customer_covid_head.customer_employee_id');
                $join->on('wg_customer_employee.customer_id', '=', 'wg_customer_covid_head.customer_id');
            })
            ->leftJoin('wg_employee', function ($join) {
                $join->on('wg_employee.id', '=', 'wg_customer_employee.employee_id');
            })
        ...
        if(!empty($criteria->workplaceId)) {
            $query->where('wg_customer_covid_head.customer_workplace_id', $criteria->workplaceId);
        }
        if(!empty($criteria->contractorId)) {
            $query->where('wg_customer_covid_head.contractor_id', $criteria->contractorId);
        }
        if(!empty($criteria->isEmployee)) {
            $query->whereNotNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",0);
        }
    ...
    public function getPregnantCharPie($criteria)
    {
        $qBase = DB::table('wg_customer_covid')
            ->join("wg_customer_covid_head", function($join) use($criteria) {
                $join->on("wg_customer_covid.customer_covid_head_id", "=","wg_customer_covid_head.id");
                $join->where('wg_customer_covid_head.customer_id', $criteria->customerId);
            })
            ->leftJoin('wg_customer_employee', function ($join) use($criteria) {
                $join->on('wg_customer_employee.id', '=', 'wg_customer_covid_head.customer_employee_id');
                $join->on('wg_customer_employee.customer_id', '=', 'wg_customer_covid_head.customer_id');
                $join->where('wg_customer_employee.customer_id', $criteria->customerId);
            })
            ->leftJoin('wg_employee', function ($join) {
                $join->on('wg_employee.id', '=', 'wg_customer_employee.employee_id');
            })
        ...
        if(!empty($criteria->workplaceId)) {
            $qBase->where('wg_customer_covid_head.customer_workplace_id', $criteria->workplaceId);
        }
        if(!empty($criteria->contractorId)) {
            $qBase->where('wg_customer_covid_head.contractor_id', $criteria->contractorId);
        }
        if(!empty($criteria->isEmployee)) {
            $qBase->whereNotNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",0);
        }
        if(!empty($criteria->isExternal)) {
            $qBase->whereNull('wg_customer_covid_head.customer_employee_id')->where("wg_customer_covid_head.is_external",1);
        }
        ...
        if ($criteria->type == 1) {
            ...
            if(!empty($criteria->isEmployee)) {
                ...
                $qBase->addSelect('wg_customer_covid_head.customer_id');
                ...
                $qBase->addSelect(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') AS period"));
                $qBase->addSelect('wg_customer_covid_head.document_number');
                $qBase->whereRaw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') = ?", [$criteria->period]);
                $qBase->groupBy(
                    DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m')"),
                    'wg_customer_covid_head.document_number'
    ...
    {
        $qTemperature1 = DB::table('wg_customer_covid_temperature')
            ->join("wg_customer_covid", "wg_customer_covid_temperature.customer_covid_id", "=", "wg_customer_covid.id")
            ->join("wg_customer_covid_head", "wg_customer_covid.customer_covid_head_id", "=", "wg_customer_covid_head.id")
            ->whereRaw("wg_customer_covid_head.customer_id = {$criteria->customerId}")
            ->select(
                'customer_covid_id', DB::raw(" MAX(temperature) as temperature")
            )
            ->whereRaw("temperature >= 37.3")
            ->groupBy('customer_covid_id');

        $qTemperature2 = DB::table('wg_customer_covid_temperature')
            ->join("wg_customer_covid", "wg_customer_covid_temperature.customer_covid_id", "=", "wg_customer_covid.id")
            ->join("wg_customer_covid_head", "wg_customer_covid.customer_covid_head_id", "=", "wg_customer_covid_head.id")
            ->join(DB::raw("({$qTemperature1->toSql()}) as temperature"), function($join) {
                $join->on("wg_customer_covid_temperature.customer_covid_id","=","temperature.customer_covid_id")
                        ->on("wg_customer_covid_temperature.temperature","=","temperature.temperature");
            })
            ->where('wg_customer_covid_head.customer_id', $criteria->customerId)
            ->select("wg_customer_covid_temperature.temperature","wg_customer_covid_temperature.customer_covid_id")
            ->groupBy('wg_customer_covid_temperature.customer_covid_id','wg_customer_covid_temperature.temperature')
            ->orderBy("wg_customer_covid_temperature.temperature");

        if ($criteria->type == 1) {
            $qTemperature2->where("wg_customer_covid.registration_date", $criteria->day);
            $qTemperature2->groupBy("wg_customer_covid.registration_date");
        } else if ($criteria->type == 0) {
            $qTemperature2->whereRaw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m') = {$criteria->period}");
            $qTemperature2->groupBy(DB::raw("DATE_FORMAT(wg_customer_covid.registration_date, '%Y%m')"));
        }
        ...
        $query = DB::table('wg_customer_covid')
            ...
            ->join(DB::raw("({$qTemperature2->toSql()}) as wg_customer_covid_temperature"), function ($join) {
                $join->on('wg_customer_covid_temperature.customer_covid_id', '=', 'wg_customer_covid.id');
            })
            ->mergeBindings($qTemperature2)
            ->select(
    ...
    public function getEmployeeCharBar($criteria)
    {
        $query = DB::table(DB::raw('(
                    select wg_customer_covid.* from wg_customer_covid
                    join wg_customer_covid_head on wg_customer_covid.customer_covid_head_id = wg_customer_covid_head.id
                    AND customer_id = '.$criteria->customerId.'
                    group by customer_covid_head_id, registration_date
                ) as wg_customer_covid
            '))
            ->join("wg_customer_covid_head", function($join) use($criteria) {
                $join->on("wg_customer_covid.customer_covid_head_id", "=","wg_customer_covid_head.id");
                $join->where('wg_customer_covid_head.customer_id', $criteria->customerId);
    ...
    public function getEmployeeWorkplaceCharBar($criteria)
    {
        $query = DB::table(DB::raw('(
                    ...
                    join wg_customer_covid_head on wg_customer_covid.customer_covid_head_id = wg_customer_covid_head.id
                    AND customer_id = '.$criteria->customerId.'
                    group by customer_covid_head_id
                ) as wg_customer_covid
            '))
            ->join("wg_customer_covid_head", function($join) use($criteria) {
                $join->on("wg_customer_covid.customer_covid_head_id", "=","wg_customer_covid_head.id");
    ...
    public function getRiskLevelCharBar($criteria)
    {
        $query = DB::table(DB::raw('(
                    ...
                    join wg_customer_covid_head on wg_customer_covid.customer_covid_head_id = wg_customer_covid_head.id
                    AND customer_id = '.$criteria->customerId.'
                    group by customer_covid_head_id, risk_level, registration_date
    ...
    public function getOximetriaCharBar($criteria)
    {
        ...
        $qTemperature1 = DB::table('wg_customer_covid_temperature')
            ...
            ->whereRaw("wg_customer_covid_head.customer_id = {$criteria->customerId}")
            ->select(
                'customer_covid_id', DB::raw(" MIN(oximetria) as oximetria")
            )
            ->whereRaw("oximetria > 0 AND oximetria < 95")
            ->groupBy('customer_covid_id');

        $qTemperature2 = DB::table('wg_customer_covid_temperature')
            ->join(DB::raw("({$qTemperature1->toSql()}) as temperature"), function($join) {
                $join->on("wg_customer_covid_temperature.customer_covid_id","=","temperature.customer_covid_id")
                        ->on("wg_customer_covid_temperature.oximetria","=","temperature.oximetria");
            })
            ->select("wg_customer_covid_temperature.oximetria","wg_customer_covid_temperature.customer_covid_id")
            ->groupBy('wg_customer_covid_temperature.customer_covid_id','wg_customer_covid_temperature.oximetria')
            ->orderBy("wg_customer_covid_temperature.oximetria");
        ...
        $query = DB::table('wg_customer_covid')
            ...
            ->mergeBindings($qTemperature2)
            ->select(
                "wg_customer_covid_temperature.oximetria AS label",
                DB::raw("COUNT(*) AS value")
            )
            ->where('wg_customer_covid_head.customer_id', $criteria->customerId)
            ->groupBy('wg_customer_covid_temperature.oximetria');
    ...
    public function getPulsometriaCharBar($criteria)
    {
        ...
        $qTemperature1 = DB::table('wg_customer_covid_temperature')
            ...
            ->whereRaw("wg_customer_covid_head.customer_id = {$criteria->customerId}")
            ->select(
                'customer_covid_id', DB::raw(" MIN(CONVERT(pulse, UNSIGNED)) as pulse")
            )
            ->whereRaw("pulse NOT BETWEEN 60 AND 100")
            ->whereRaw("pulse > 0")
            ->groupBy('customer_covid_id');
        ...
        $qTemperature2 = DB::table('wg_customer_covid_temperature')
            ->join(DB::raw("({$qTemperature1->toSql()}) as temperature"), function($join) {
                $join->on("wg_customer_covid_temperature.customer_covid_id","=","temperature.customer_covid_id")
                        ->on("wg_customer_covid_temperature.pulse","=","temperature.pulse");
            })
            ->select(DB::raw(" CONVERT(wg_customer_covid_temperature.pulse, UNSIGNED) as pulse"),"wg_customer_covid_temperature.customer_covid_id")
            ->groupBy('wg_customer_covid_temperature.customer_covid_id','wg_customer_covid_temperature.pulse')
            ->orderBy("wg_customer_covid_temperature.pulse");
        ...
        $query = DB::table('wg_customer_covid')
            ...
            ->mergeBindings($qTemperature2)
            ->select(
                "wg_customer_covid_temperature.pulse AS label",
                DB::raw("COUNT(*) AS value")
            )
            ->where('wg_customer_covid_head.customer_id', $criteria->customerId)
            ->groupBy('wg_customer_covid_temperature.pulse')
            ->orderBy('wg_customer_covid_temperature.pulse');
    ...
    public function getCovidWorkplaceList($criteria)
    {
        ...
        $query = DB::table('wg_customer_covid')
            ...
            ->join('wg_customer_config_workplace', function ($join) {
                $join->on('wg_customer_covid_head.customer_workplace_id', '=', 'wg_customer_config_workplace.id');
            })
