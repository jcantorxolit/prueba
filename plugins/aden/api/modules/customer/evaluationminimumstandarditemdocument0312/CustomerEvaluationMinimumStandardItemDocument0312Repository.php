...
namespace AdeN\Api\Modules\Customer\EvaluationMinimumStandardItemDocument0312;
...
class CustomerEvaluationMinimumStandardItemDocument0312Repository extends BaseRepository
{
    protected $service;

    public function __construct()
    {
        parent::__construct(new CustomerEvaluationMinimumStandardItemDocument0312Model());

        $this->service = new CustomerEvaluationMinimumStandardItemDocument0312Service();
    }

    public function all($criteria)
    {
        $this->setColumns([
            "id" => "wg_customer_evaluation_minimum_standard_item_document_0312.id",
            "documentType" => "document_type.item AS documentType",
            "classification" => "customer_document_classification.item AS classification",
            "description" => "wg_customer_evaluation_minimum_standard_item_document_0312.description",
            "version" => "wg_customer_evaluation_minimum_standard_item_document_0312.version",
            "createdAt" => "wg_customer_evaluation_minimum_standard_item_document_0312.created_at",
            ...
            "statusCode" => "wg_customer_evaluation_minimum_standard_item_document_0312.status AS statusCode",
            ...
            "program" => "wg_customer_evaluation_minimum_standard_item_document_0312.program",
            "customerEvaluationMinimumStandardItemId" => "wg_customer_evaluation_minimum_standard_item_document_0312.customer_evaluation_minimum_standard_item_id",
        ...
        $query->leftjoin(DB::raw(CustomerModel::getDocumentTypeRelation('document_type')), function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.type', '=', 'document_type.value');
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.origin', '=', 'document_type.origin');
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('customer_document_classification')), function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.classification', '=', 'customer_document_classification.value');
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('customer_document_status')), function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.status', '=', 'customer_document_status.value');
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('minimum_standard_item_documenta_0312_label')), function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.label', '=', 'minimum_standard_item_documenta_0312_label.value');
        })->join("wg_customer_evaluation_minimum_standard_item_0312", function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_0312.id', '=', 'wg_customer_evaluation_minimum_standard_item_document_0312.customer_evaluation_minimum_standard_item_id');
            ...
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.type', '=', 'wg_customer_document_security.documentType');
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.origin', '=', 'wg_customer_document_security.origin');
        })->leftjoin(DB::raw(CustomerDocumentModel::getSecurityUserRelationTable('security')), function ($join) use ($authUser) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.id', '=', 'security.id');
            $join->on('security.user_id', '=', DB::raw($authUser ? $authUser->id : 0));
        })->leftjoin("users", function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.created_by', '=', 'users.id');
        });
        ...
        $result["data"] = $this->parseModelWithDocument($data, CustomerEvaluationMinimumStandardItemDocument0312Model::CLASS_NAME);
    ...
    public function allAvailable($criteria)
    {
        ...
        $q1 = DB::table('wg_customer_evaluation_minimum_standard_item_document_0312')
            ->select(
                'wg_customer_evaluation_minimum_standard_item_document_0312.customer_evaluation_minimum_standard_item_id',
                'wg_customer_evaluation_minimum_standard_item_document_0312.customer_document_id'
            );

        if ($criteria != null) {
            if ($criteria->mandatoryFilters != null) {
                foreach ($criteria->mandatoryFilters as $item) {
                    if ($item->field == 'customerEvaluationMinimumStandardItemId') {
                        $q1->where(SqlHelper::getPreparedField('wg_customer_evaluation_minimum_standard_item_document_0312.customer_evaluation_minimum_standard_item_id'), SqlHelper::getOperator($item->operator), SqlHelper::getPreparedData($item), 'and');
                    }
                }
            }
        }
        ...
        })->leftjoin(DB::raw("({$q1->toSql()}) AS wg_customer_evaluation_minimum_standard_item_document_0312"), function ($join) {
            $join->on('wg_customer_evaluation_minimum_standard_item_document_0312.customer_document_id', '=', 'wg_customer_document.id');
        })->mergeBindings($q1);

        $query->whereIn("wg_customer_document.program", ['P', 'H', 'V', 'A']);
        $query->whereNull("wg_customer_evaluation_minimum_standard_item_document_0312.customer_document_id");

        $this->applyCriteria($query, $criteria, ['customerEvaluationMinimumStandardItemId']);
    ...
    public function allAvailablePreviousPeriod($criteria)
    {
        ...
        $query->whereNotIn('wg_customer_evaluation_minimum_standard_item_document_0312.id', function ($query) {
            $query->select('old_id')
                ->from('wg_customer_evaluation_minimum_standard_item_document_0312')
                ->whereNotNull('old_id');
        });
    ...
    public function import($entity)
    {
        foreach ($entity->documentList as $document) {
            ...
            $this->duplicate($model, $entity->customerEvaluationMinimumStardardItem->customerEvaluationMinimumStandardItemId);
        }
    ...
    public function importHistorical($entity)
    {
        ...
        $this->duplicate($model, $entity->customerEvaluationMinimumStandardItemId, 'H');
    }

    private function duplicate($model, $customerEvaluationMinimumStandardItemId, $label = 'M')
    {
        if ($model) {
            ...
            if ($documentFile) {
                ...
                $newEntity->id = 0;
                $newEntity->customerEvaluationMinimumStandardItemId = $customerEvaluationMinimumStandardItemId;
