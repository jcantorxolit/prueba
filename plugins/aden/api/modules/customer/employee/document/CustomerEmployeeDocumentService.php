...
class CustomerEmployeeDocumentService extends BaseService
{
    ...
    public function bulkInsertCustomerEmployeeDocumentTracking()
    {
        $query = DB::table('wg_customer_employee_document');
        ...
        $query->join('wg_customer_employee', function ($join) {
            ...
            DB::raw('NULL id'),
            'wg_customer_employee_document.id AS customer_employee_document_id',
        ...
        $query->whereRaw("(`wg_customer_employee`.`customer_id` = `document_type`.`customer_id` OR `document_type`.`customer_id` IS NULL)")
            ->where('wg_customer_employee_document.status', '<>', 2)
            ->whereRaw("(wg_customer_employee.isActive = 0 OR wg_customer_employee.isActive IS NULL)")
            ->where('wg_customer_employee_document.isApprove', 1)
            ->where('wg_customers.status', 1);

        $sql = 'INSERT INTO wg_customer_employee_document_tracking (`id`, `customer_employee_document_id`, `type`, `observation`, `createdBy`, `updatedBy`, `created_at`, `updated_at`)  ' . $query->toSql();
    ...
    public function bulkIDeniedCustomerEmployeeDocument()
    {
        $query = DB::table('wg_customer_employee_document');
        ...
        $query->update([
            'wg_customer_employee_document.isApprove' => 4,
            'wg_customer_employee_document.isDenied' => 1,
            'wg_customer_employee_document.updated_at' => DB::raw("NOW()")
        ]);
    }
    ...
    public function getExportData($criteria)
    {
        ...
        if ($customerEmployeeId != null) {
            $documents = \Wgroup\CustomerEmployeeDocument\CustomerEmployeeDocument::where('customer_employee_id', $customerEmployeeId->value)->get();
        } else if ($uids != null) {
            $documents = \Wgroup\CustomerEmployeeDocument\CustomerEmployeeDocument::whereIn('id', $uids->value)->get();
        }
    ...
    {
        $baseQuery = DB::table('wg_customer_employee_document');
        ...
        $baseQuery->join('wg_customer_employee', function ($join) {
            ...
            DB::raw("CASE WHEN wg_customer_employee_document.isApprove = 1 THEN 'Aprobado' WHEN wg_customer_employee_document.isDenied = 1 THEN 'Denegado' ELSE '' END AS isVerified"),
        ...
        $baseQuery->whereIn('wg_customer_employee_document.status', [1, 3]);
    ...
    private function prepareQueryExportData($criteria)
    {
        ...
        $query = DB::table('wg_customer_employee_document')
            ...
            ->leftjoin(DB::raw(CustomerEmployeeDocumentModel::getRelationTrackingMax('document_tracking')), function ($join) {
            ...
            ->join('system_files', function ($join) use ($customerEmployeeId) {
                $join->on('wg_customer_employee_document.id', '=', 'system_files.attachment_id');
                $join->whereRaw("system_files.attachment_type = 'Wgroup\\\\CustomerEmployeeDocument\\\\CustomerEmployeeDocument'");
                $join->whereRaw("system_files.field = 'document'");
                if ($customerEmployeeId) {
                    $join->whereRaw("wg_customer_employee_document.customer_employee_id = $customerEmployeeId->value");
                }
            })
            ->when(!empty($criteria->filter) && !empty($criteria->filter->type), function($query) use ($criteria) {
                $query->where('wg_customer_employee_document.requirement', $criteria->filter->type->value);
                $query->where('wg_customer_employee_document.origin', $criteria->filter->type->origin);
            })
            ->when(!empty($criteria->filter) && !empty($criteria->filter->year), function($query) use ($criteria) {
                $query->whereYear('wg_customer_employee_document.created_at', $criteria->filter->year->value);                
            })
            ->when(!empty($criteria->filter) && !empty($criteria->filter->month), function($query) use ($criteria) {
                $query->whereMonth('wg_customer_employee_document.created_at', $criteria->filter->month->value);                
            })     
            ->select(
                "wg_employee.documentNumber",
                ...
                DB::raw("CASE WHEN document_type.isRequired = '1' THEN 'Requerido' ELSE 'Opcional' END AS isRequired"),
                "customer_document_status.item AS status",
                DB::raw("CASE WHEN wg_customer_employee_document.isApprove IS NOT NULL THEN 'Aprobado' WHEN wg_customer_employee_document.isDenied IS NOT NULL THEN 'Denegado' ELSE '' END AS isVerified"),
                DB::raw("CASE WHEN wg_customer_employee_document.isApprove IS NOT NULL THEN '' WHEN wg_customer_employee_document.isDenied IS NOT NULL THEN document_tracking.observation ELSE '' END AS observation"),
                "wg_customer_employee_document.created_at",
                "users.name AS createdBy",
                "wg_customer_employee_document.status AS statusCode",
                "document_type.isRequired AS isRequiredCode",
                "wg_customer_employee_document.customer_employee_id",
                DB::raw("YEAR(`endDate`) AS `year`"),
                DB::raw("MONTH(`endDate`) AS `month`"),
                DB::raw("IF (
                ...
                'wg_customer_employee.customer_id AS customerId',
                "wg_customer_employee_document.customer_employee_id AS customerEmployeeId",
                "wg_customer_employee_document.id"
            )
            ->whereRaw("(`wg_customer_employee`.`customer_id` = `document_type`.`customer_id` OR `document_type`.`customer_id` IS NULL)");

        if ($uids != null) {
            $query->whereIn('wg_customer_employee_document.id', $uids->value);
        }
    ...
    public function findEntity($model, $criteria)
    {
        ...
        if ($model->customer_employee_id) {
            $qTrackingGroup->where('wg_customer_employee_document.customer_employee_id', $model->customer_employee_id);
        }
        ...
        ->leftjoin(DB::raw(SystemParameter::getRelationTable('customer_document_status')), function ($join) {
            ...
            "customer_document_status.item AS status",
            DB::raw("CASE WHEN wg_customer_employee_document.isApprove = 1 THEN 'Aprobado' WHEN wg_customer_employee_document.isDenied = 1 THEN 'Denegado' ELSE '' END AS isVerified"),
            DB::raw("CASE WHEN wg_customer_employee_document.isApprove = 1 THEN '' WHEN wg_customer_employee_document.isDenied = 1 THEN document_tracking.observation ELSE '' END AS observation"),
            "wg_customer_employee_document.created_at",
            "users.name AS createdBy",
            "wg_customer_employee_document.status AS statusCode",
        ...
        $query->whereRaw("(`wg_customer_employee`.`customer_id` = `document_type`.`customer_id` OR `document_type`.`customer_id` IS NULL)");
        $query->where('wg_customer_employee_document.id', $model->id);
