...
class CustomerOccupationalReportIncidentRepository extends BaseRepository
{
    protected $service;
    ...
    {
        parent::__construct(new CustomerOccupationalReportIncidentModel());

        $this->service = new CustomerOccupationalReportIncidentService();
    }
    ...
    {
        $this->setColumns([
            "id" => "wg_customer_occupational_report_incident.id",
            "documentType" => "employee_document_type.item AS documentType",
            "documentNumber" => "wg_customer_occupational_report_incident.document_number AS documentNumber",
            "firstName" => DB::raw("CONCAT_WS(' ', IFNULL(wg_customer_occupational_report_incident.first_name, ''), IFNULL(wg_customer_occupational_report_incident.second_name, '')) AS firstName"),
            "lastName" => DB::raw("CONCAT_WS(' ', IFNULL(wg_customer_occupational_report_incident.first_lastname, ''), IFNULL(wg_customer_occupational_report_incident.second_lastname, '')) AS lastName"),
            "job" => "wg_customer_config_job_data.name AS job",
            "eps" => "eps.item AS eps",
            "arl" => "arl.item AS arl",
            "afp" => "afp.item AS afp",
            "accidentDate" => "wg_customer_occupational_report_incident.accident_date AS accidentDate",
            "status" => "report_incident_status.item AS status",
            "statusCode" => "wg_customer_occupational_report_incident.status AS statusCode",
            "customerId" => "wg_customer_occupational_report_incident.customer_id"
        ]);
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('employee_document_type')), function ($join) {
            $join->on('wg_customer_occupational_report_incident.document_type', '=', 'employee_document_type.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('eps')), function ($join) {
            $join->on('wg_customer_occupational_report_incident.eps', '=', 'eps.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('arl')), function ($join) {
            $join->on('wg_customer_occupational_report_incident.arl', '=', 'arl.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('afp')), function ($join) {
            $join->on('wg_customer_occupational_report_incident.afp', '=', 'afp.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('report_incident_status')), function ($join) {
            ...
            $join->on('wg_customer_occupational_report_incident.job', '=', 'wg_customer_config_job_data.id');

        })->leftjoin("users", function ($join) {
            $join->on('wg_customer_occupational_report_incident.createdBy', '=', 'users.id');
    ...
    public function updateStatus($entity)
    {
        ...
        if ($entity->reason && !empty($entity->reason)) {
            $entityModel->status_reason .= $entity->reason . "\r\n";
        }
        $entityModel->updatedBy = $authUser ? $authUser->id : 1;
        $entityModel->save();
