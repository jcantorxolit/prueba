...
namespace AdeN\Api\Modules\Customer\User\Http\Controllers;
...
class CustomerUserController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->repository = new CustomerUserRepository();
    ...
    public function index()
    {
        ...
        try {
            ...
            $defaultFilters = [
                ...
                array("field" => 'relation', "operator" => 'like', "value" => $criteria->search),
                array("field" => 'profile', "operator" => 'like', "value" => $criteria->search),
                array("field" => 'module', "operator" => 'like', "value" => $criteria->search),
            ];

            $criteria = CriteriaHelper::addFilters($criteria, $defaultFilters);

            if (in_array('customer-user-contractor-economic-group', $segments)) {
    ...
    public function indexCustomerAvailable()
    {
        $request = Request::instance();
        ...
        try {
            ...
            $mandatoryFilters = [
                array("field" => 'customerId', "operator" => 'eq'),
                array("field" => 'userId', "operator" => 'notInRaw'),
                array("field" => 'relation', "operator" => 'eq'),
            ];
            ...
            $result = $this->repository->allCustomerAvailable($criteria);
    ...
    public function indexCustomerRelated()
    {
        $request = Request::instance();
        ...
        try {
            ...
            $mandatoryFilters = [
                array("field" => 'customerId', "operator" => 'eq'),
                array("field" => 'userId', "operator" => 'eq'),
                array("field" => 'relation', "operator" => 'eq'),
            ];
            ...
            $result = $this->repository->allCustomerRelated($criteria);
    ...
    public function store()
    {
        ...
        try {
            ...
            if (!$this->repository->canCreate($entity)) {
                throw new \Exception('No es posible guardar el registro. Ya existe un usuario con el E-mail ingresado.');
            }
    ...
    public function relateCustomer()
    {
        $content = $this->request->get("data", "");
        ...
        try {
            ...
            $result = $this->repository->relateCustomer($entity);
            $this->response->setResult($result);
        } catch (\Exception $ex) {
            Log::error($ex);
            $this->response->setStatuscode(500);
    ...
    public function destroy()
    {
        ...
        try {
            ...
            $validate = $this->repository->canDelete($id);
    ...
    public function show()
    {
        ...
        try {
            $result = $this->repository->parseModelWithRelations($this->repository->updateRelation($id, $relation));
            $this->response->setResult($result);
        } catch (Exception $ex) {
            $this->response->setResult(0);
            $this->response->setStatuscode(500);
    ...
    public function import()
    {
        ...
        try {
            ...
            foreach ($allFiles as $file) {
                $result = $this->repository->import($file, $customerId);
            }

            $end = Carbon::now();

            $result['elapseTime'] = $end->diffInSeconds($start);
