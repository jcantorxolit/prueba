...
namespace AdeN\Api\Modules\Customer\SafetyInspectionList;
...
class CustomerSafetyInspectionListRepository extends BaseRepository
{
    public function __construct()
    {
        parent::__construct(new CustomerSafetyInspectionListModel());

        $this->service = new CustomerSafetyInspectionListService();
    }
    ...
    {
        $this->setColumns([
            "id" => "wg_customer_safety_inspection_list.id",
            "user" => "users.name AS user",
            "createdAt" => "wg_customer_safety_inspection_list.created_at",
            "customerSafetyInspectionId" => "wg_customer_safety_inspection_list.customer_safety_inspection_id",
        ]);
        ...
        $query->join('wg_customer_safety_inspection', function ($join) {
            $join->on('wg_customer_safety_inspection.id', '=', 'wg_customer_safety_inspection_list.customer_safety_inspection_id');

        })->leftjoin('users', function ($join) {
            $join->on('users.id', '=', 'wg_customer_safety_inspection_list.created_by');
    ...
    public function insertOrUpdate($entity)
    {
        ...
        $entityModel->customer_safety_inspection_id = $entity->customerSafetyInspectionId;
