...
class CustomerTrackingDocumentService extends BaseService
{
    ...
    public function getExportData($criteria)
    {
        ...
        if ($uids != null) {
            $documents = CustomerTrackingDocumentModel::whereIn('id', $uids->value)->get();
        }
    ...
    private function prepareQueryExportData($criteria)
    {
        ...
        $query = DB::table('wg_customer_tracking_document')
            ->join('wg_customer_tracking', function ($join) use($customerId) {
                $join->on('wg_customer_tracking_document.customer_tracking_id', '=', 'wg_customer_tracking.id');
                if ($customerId) {
                    $join->where('wg_customer_tracking.customer_id', '=', $customerId->value);
                }
            })
            ...
            ->join('system_files', function ($join) use ($customerId) {
                $join->on('wg_customer_tracking_document.id', '=', 'system_files.attachment_id');
                $join->where('system_files.attachment_type', '=', CustomerTrackingDocumentModel::CLASS_NAME);
                $join->where('system_files.field', '=', 'document');                
            })
            ->when(!empty($criteria->filter->type), function($query) use ($criteria) {
                $query->where('wg_customer_tracking_document.type', $criteria->filter->type->value);
                $query->where('wg_customer_tracking_document.origin', $criteria->filter->type->origin);
            })
            ->when(!empty($criteria->filter->year), function($query) use ($criteria) {
                $query->whereYear('wg_customer_tracking_document.created_at', $criteria->filter->year->value);                
            })
            ->when(!empty($criteria->filter->month), function($query) use ($criteria) {
                $query->whereMonth('wg_customer_tracking_document.created_at', $criteria->filter->month->value);                
            })     
            ->select(
                ...
                "wg_customer_tracking_document.description",
                "wg_customer_tracking_document.id",
                "wg_customer_tracking_document.version",
                "customer_document_status.item AS status",
                "wg_customer_tracking_document.created_at",
                ...
                'wg_customer_tracking.customer_id AS customerId'
            )
            ->whereRaw("(`wg_customer_tracking`.`customer_id` = `document_type`.`customer_id` OR `document_type`.`customer_id` IS NULL)");

        if ($uids != null) {
            $query->whereIn('wg_customer_tracking_document.id', $uids->value);
        }
