...
class CustomerTrackingDocumentJob
{
    protected $service;
    ...
    public function fire($job, $data)
    {
        ...
        $excelFilename = 'GUIA_DOCUMENTOS_SEGUIMIENTOS_' . Carbon::now()->timestamp;
        ExportHelper::excelStorage($excelFilename, 'GUIA', $data['excel']);

        $zipFilename = 'DOCUMENTOS_DOCUMENTOS_SEGUIMIENTOS_' . Carbon::now()->timestamp . '.zip';
    ...
    {               
        $criteria->module = "DOCUMENTOS SEGUIMIENTOS";
        $criteria->url = $url;        
