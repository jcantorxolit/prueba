 ...
 * Message Model
 */
class TclMessage extends Model {
    ...
    public static function get($messageId) {
        ...
        $debuglang = Parameters::get('system::lenguages.debug', '-1');

        if ($debuglang == "1") {
            $msg = $item->forLocale(self::$locale, "(" . $item->id . ")" . $messageId);
        } else {
            $msg = $item->forLocale(self::$locale, $messageId);
        }
    ...
    public static function allByLang($locale) {
        ...
        foreach ($messages as $msg) {
            ...
            if (array_key_exists($locale, $msg->message_data)) {
                $data[$msg->message_data["x"]] = $msg->message_data[$locale];
            } else {
                if($locale == "en"){
                    $data[$msg->message_data["x"]] = $msg->message_data["x"];
                }else{
                    $data[$msg->message_data["x"]] = "(" . $msg->id . ")" . $msg->message_data["x"];
                }
