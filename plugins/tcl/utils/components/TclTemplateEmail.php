...
class TclTemplateEmail extends ComponentBase {
    ...
    public function componentDetails() {
        return [
            'name' => 'TCL Mail Template',
            'description' => 'Displays a mail template to contact form where ever it\'s been embedded. (Custom by TCL)'
        ];
    }
    ...
    public function onTclMailSent() {
        ...
        if (!$template['multiple_recipients']) {
            ...
            Mail::send($this->buildSlugTemplate($template), $post, function($message) use($data) {
        ...
        if ((isset($post[$this->aREmail]) and $post[$this->aREmail]) and ( isset($post[$this->aRName]) and $post[$this->aRName]) and ( isset($template['autoresponse']) and $template['autoresponse'])) {
            ...
            if ($slgtpl = $this->buildSlugTemplate($responseMailTemplate, true)) {
                Mail::send($slgtpl, $post, function($autoresponse) use ($response) {
    ...
    protected function buildSlugTemplate($template, $isnamepl = false) {
        
        $locale = Translator::instance()->getLocale();
        
        if (!$isnamepl) {
            $slugtemplate = 'octodevel.octomail::mail.' . $template['slug'];
        } else {
        ...
        try {
            ...
            if ($tpl = $model->where("code", $slugtemplate . "_" . strtolower(trim($locale)))->first()) {
                $slugtemplate = $tpl->code;
            }
        } catch (Exception $exc) {
