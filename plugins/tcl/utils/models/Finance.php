...
class Finance extends Model {
     ...
     */
    public $table = 'tcl_finance_services';
    ...
    public function beforeUpdate() {
        if ($this->isDirty('is_default')) {
            ...
            if (!$this->is_default)
                throw new ValidationException(['is_default' => Lang::get('tcl.utils::lang.finance.unset_default', ['currency' => $this->name])]);
        }
    }
    ...
    public function makeDefault() {
        if (!$this->is_enabled)
            throw new ValidationException(['is_enabled' => Lang::get('tcl.utils::lang.finance.disabled_default', ['currency' => $this->name])]);
    ...
    public static function getDefault($reload = false) {
        ...
        return self::$defaultService = self::where('is_default', true)
                ->remember(1440, 'tcl.currency.defaultService')
                ->first()
        ;
    }
    ...
    public static function listEnabled() {
        ...
        return self::$cacheListEnabled = self::isEnabled()
                ->remember(1440, 'tcl.utils.finances')
                ->lists('name', 'code')
        ;
    }
    ...
    public static function clearCache() {
        Cache::forget('tcl.utils.finances');
        Cache::forget('tcl.currency.defaultService');
    }
