...
namespace JBonnyDev\UserPermissions;
...
class Plugin extends \System\Classes\PluginBase
{
    ...
    protected function extendUserModel()
    {
        function getAllowedPermissions($model)
        {
            ...
            $groupPermissionsQuery = DB::table('users_groups')
                ->join('jbonnydev_userpermissions_group_permission', function ($join) {
                    $join->on('jbonnydev_userpermissions_group_permission.group_id', '=', 'users_groups.user_group_id');
                })
                ->join('jbonnydev_userpermissions_permissions', function ($join) {
                    $join->on('jbonnydev_userpermissions_permissions.id', '=', 'jbonnydev_userpermissions_group_permission.permission_id');
                })
                ->select(
                    ...
                    'jbonnydev_userpermissions_group_permission.permission_state',
                    'jbonnydev_userpermissions_group_permission.created_at',
                    'jbonnydev_userpermissions_group_permission.updated_at'
                )
                ->where('users_groups.user_id', $model->id)
                ->where('jbonnydev_userpermissions_group_permission.permission_state', '=', 1);

            $groupUserPermissionsQuery = $model->user_permissions()->where('jbonnydev_userpermissions_user_permission.permission_state', 2)
                ...
                ->join(
                    'jbonnydev_userpermissions_permissions as permissions',
                    'jbonnydev_userpermissions_group_permission.permission_id',
                    '=',
                    'permissions.id'
                )->select(
            ...
            $permissionsQueryResult = $model->user_permissions()
                ->select('id', 'code')
                ->where('permission_state', 1)
                ->union($groupUserPermissionsQuery)
                ->union($groupPermissionsQuery)
                ->get();

            if (!$permissionsQueryResult) {
                $permissionsQueryResult = [];
            } else {
                $permissionsQueryResult = $permissionsQueryResult->unique()->toArray();
            }
        ...
        UserModel::extend(function ($model) {
            ...
            $model->addDynamicMethod('allowedPermissions', function () use ($model) {
                return getAllowedPermissions($model);
            });
