 ...
 */
app.controller('ConfigurationComplementaryTestCtrl', ['$scope', '$stateParams', '$log','$compile', '$rootScope', '$timeout', '$filter', '$state',
    function ($scope, $stateParams, $log, $compile, $rootScope, $timeout, $filter, $state) {

        var log = $log;

        $scope.isCreate = $state.is("app.configuration.complementary-test");

        $scope.views =
            [
                { name: 'basic', url: $rootScope.app.views.urlRoot + 'modules/configuration/complementary-test/_form.htm'},
                { name: 'classification', url: $rootScope.app.views.urlRoot + 'modules/configuration/complementary-test/_result.htm'}
            ];
