 ...
 */
app.controller('customerInternalCertificateAdminGradeEditCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder', 'DTColumnDefBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        var save = function () {
            ...
            return $http({
                method: 'POST',
                url: 'api/customer-internal-certificate-grade/save',
        ...
        var afterInit = function()
        {
            ...
            $http({
                method: 'POST',
                url: 'api/customer-internal-certificate-program',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                data: $.param(req)
            })
                .catch(function (e, code) {
        ...
        $scope.onAddCalendarDate = function () {
            if ($scope.grade.startDateTime == null || $scope.grade.hourDuration == null || $scope.grade.hourDuration == 0) {
                ...
                if (($scope.totalHourDuration + parseInt($scope.grade.hourDuration)) > parseInt($scope.grade.program.hourDuration)) {
                    toaster.pop('error', 'Error', 'La duración total supera las horas asignadas al programa.');
                    return;
                }
                var toTest = $filter('date')($scope.grade.startDateTime, "dd/MM/yyyy");

                var isSet = false;

                var i;
                for (i = 0; i < $scope.grade.calendar.length; i++) {
                    if ($scope.grade.calendar[i].startDateFormat == toTest){
                        isSet = true;
                    }
                }
                if (!isSet){
                    ...
                    toaster.pop('error', 'Error', 'La fecha ya ha sido asignada al programa.');
        ...
        $scope.changeprograma = function (item, model) {
                    ...
                    if (isConfirm) {
                        $timeout(function () {
                            ...
                            if (agent.id != 0) {
                                ...
                                $http({
                                    method: 'POST',
                                    url: 'api/customer-internal-certificate-grade-agent/delete',
                                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                                    data: $.param(req)
                                }).then(function (response) {
                                    swal("Eliminado", "Registro eliminado satisfactoriamente", "info");
