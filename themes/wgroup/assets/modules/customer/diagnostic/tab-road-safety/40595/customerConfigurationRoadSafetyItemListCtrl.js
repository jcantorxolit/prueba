 ...
 */
app.controller('customerConfigurationMinimumStandardItem40595ListCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        $scope.dtOptionsCustomerConfigMinimumStandardItem40595 = DTOptionsBuilder.newOptions()
        ...
        $scope.dtColumnsCustomerConfigMinimumStandardItem40595 = [
            ...
            DTColumnBuilder.newColumn('roadSafety.cycle.name').withTitle("Ciclo").withOption('width', 200).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('roadSafety.parent.numeral').withTitle("Numeral (Padre)").withOption('width', 200).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('roadSafety.parent.description').withTitle("Estándar (Padre)").withOption('width', 200).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('roadSafety.numeral').withTitle("Numeral (Hijo)").withOption('width', 200).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('roadSafety.description').withTitle("Estandar (Hijo)").withOption('width', 200).withOption('defaultContent', ''),
        ...
        var loadRow = function () {
            ...
            $("#dtCustomerConfigMinimumStandardItem40595 a.editRow").on("click", function () {
                var id = $(this).data("id");
                onEdit(id);
            });

            $("#dtCustomerConfigMinimumStandardItem40595 a.delRow").on("click", function () {
                var id = $(this).data("id");
        ...
        $scope.dtInstanceCustomerConfigMinimumStandardItem40595Callback = function (instance) {
            $scope.dtInstanceCustomerConfigMinimumStandardItem40595 = instance;
        };

        $scope.reloadData = function () {
            $scope.dtInstanceCustomerConfigMinimumStandardItem40595.reloadData();
        };
        ...
        var onOpenModal = function (id) {
            ...
            var modalInstance = $aside.open({
                //templateUrl: 'app_modal_customer_configuration_road_safety_item.htm',
                templateUrl: $rootScope.app.views.urlRoot + "modules/customer/diagnostic/tab-road-safety/40595/customer_configuration_road_safety_item_modal.htm",
                placement: 'right',
                size: 'lg',
                backdrop: true,
                controller: 'ModalInstanceSideCustomerConfigurationMinimumStandardItemEditCtrl',
                scope: $scope,
                resolve: {
                    standard: function () {
                        return standard;
...
app.controller('ModalInstanceSideCustomerConfigurationMinimumStandardItem40595EditCtrl', function ($rootScope, $stateParams, $scope, $uibModalInstance, standard, $log, $timeout, SweetAlert, $http, toaster, $filter, $aside, $document, DTColumnBuilder, DTOptionsBuilder, $compile) {
    ...
    var loadListVerificationMode = function () {
        ...
        var req = {
            road_safety_item_id: standard.id,
    ...
    $scope.$watch("standard.roadSafetyParent", function (newValue, oldValue, scope) {

        $scope.standardList = [];

        if (oldValue != null && !angular.equals(newValue, oldValue)) {
            $scope.standard.roadSafety = null;
        }

        if ($scope.standard.roadSafetyParent != null) {
            $scope.standardList = $filter('filter')($scope.standardListAll, {parentId: $scope.standard.roadSafetyParent.id});
        }
