 ...
 */
app.controller('customerConfigWizardJobActivityStagingListCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        $scope.dtInstanceCustomerConfigWizardJobActivityStagingDT = {};
        $scope.dtOptionsCustomerConfigWizardJobActivityStagingDT = DTOptionsBuilder.newOptions()
            ...
            .withOption('ajax', {
                ...
                url: 'api/customer-config-job-activity-staging',
        ...
        $scope.dtColumnsCustomerConfigWizardJobActivityStagingDT = [
        ...
        var loadRow = function () {
            angular.element("#dataCustomerConfigWizardJobActivityStagingDT a.editRow").on("click", function () {
                var id = angular.element(this).data("id");
                onEdit(id);
            });
        }

        $scope.dtInstanceCustomerConfigWizardJobActivityStagingDTCallback = function(instance) {
            $scope.dtInstanceCustomerConfigWizardJobActivityStagingDT = instance;
        };

        $scope.reloadData = function () {
            $scope.dtInstanceCustomerConfigWizardJobActivityStagingDT.reloadData();
        };

        var onEdit = function (id) {
            var modalInstance = $aside.open({
                templateUrl: $rootScope.app.views.urlRoot + "modules/customer/sgsst/massive/jobactivity/customer_profile_config_sgsst_wizard_tab_job_activity_staging_modal.htm",
                placement: 'right',
                windowTopClass: 'top-modal',
                size: 'lg',
                backdrop: true,
                controller: 'ModalInstanceSideCustomerConfigWizardJobActivityStagingEditCtrl',
                scope: $scope,
                resolve: {
                    dataItem: function () {
                        return { id: id ? id : 0 };
        ...
        $scope.onProcess = function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    return $http({
                        method: 'POST',
                        url: $exportUrl + 'api/v1/customer-config-job-activity-confirm',
...
app.controller('ModalInstanceSideCustomerConfigWizardJobActivityStagingEditCtrl', function ($rootScope, $stateParams, $scope, dataItem, isView, $uibModalInstance, $log, $timeout, SweetAlert, $http, toaster, ngNotify, $document, $filter, $aside, ListService) {
    ...
    function getList() {
        var entities = [
            ...
            {name: 'customer_job', value: $stateParams.customerId},
            {name: 'customer_activity', value: $stateParams.customerId},
    ...
    function onDemandMacroprocess() {
        var entities = [
            {name: 'customer_macroprocess', criteria: {
    ...
    function onDemandProcess() {
        var entities = [
            {name: 'customer_process', criteria: {
                customerId: $stateParams.customerId,
                workplaceId: $scope.entity.workplace ? $scope.entity.workplace.id : null,
                macroprocessId: $scope.entity.macroprocess ? $scope.entity.macroprocess.id : null
            }},
        ];
    ...
    $scope.refreshMacroprocess = function() {
        onDemandMacroprocess();
    }
    ...
    $scope.onLoadRecord = function () {
        $http({
            method: 'GET',
            url: 'api/customer-config-job-activity-staging/get',
            params: {
                id: $scope.entity.id
            }
    ...
    var save = function () {
        ...
        return $http({
            method: 'POST',
            url: 'api/customer-config-job-activity-staging/save',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $.param(req)
        }).then(function (response) {
            $timeout(function () {
