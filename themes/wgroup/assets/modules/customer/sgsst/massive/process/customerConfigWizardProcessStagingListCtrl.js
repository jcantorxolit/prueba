 ...
 */
app.controller('customerConfigWizardProcessStagingListCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        $scope.dtInstanceCustomerConfigWizardProcessStagingDT = {};
        $scope.dtOptionsCustomerConfigWizardProcessStagingDT = DTOptionsBuilder.newOptions()
            ...
            .withOption('ajax', {
                ...
                url: 'api/customer-config-process-staging',
        ...
        $scope.dtColumnsCustomerConfigWizardProcessStagingDT = [
        ...
        var loadRow = function () {
            angular.element("#dataCustomerConfigWizardProcessStagingDT a.editRow").on("click", function () {
                var id = angular.element(this).data("id");
                onEdit(id);
            });
        }

        $scope.dtInstanceCustomerConfigWizardProcessStagingDTCallback = function(instance) {
            $scope.dtInstanceCustomerConfigWizardProcessStagingDT = instance;
        };

        $scope.reloadData = function () {
            $scope.dtInstanceCustomerConfigWizardProcessStagingDT.reloadData();
        };

        var onEdit = function (id) {
            var modalInstance = $aside.open({
                templateUrl: $rootScope.app.views.urlRoot + "modules/customer/sgsst/massive/process/customer_profile_config_sgsst_wizard_tab_process_staging_modal.htm",
                placement: 'right',
                windowTopClass: 'top-modal',
                size: 'lg',
                backdrop: true,
                controller: 'ModalInstanceSideCustomerConfigWizardProcessStagingEditCtrl',
                scope: $scope,
                resolve: {
                    dataItem: function () {
                        return { id: id ? id : 0 };
        ...
        $scope.onProcess = function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    return $http({
                        method: 'POST',
                        url: $exportUrl + 'api/v1/customer-config-process-confirm',
...
app.controller('ModalInstanceSideCustomerConfigWizardProcessStagingEditCtrl', function ($rootScope, $stateParams, $scope, dataItem, isView, $uibModalInstance, $log, $timeout, SweetAlert, $http, toaster, ngNotify, $document, $filter, $aside, ListService) {
    ...
    $scope.onLoadRecord = function () {
        $http({
            method: 'GET',
            url: 'api/customer-config-process-staging/get',
            params: {
                id: $scope.entity.id
            }
    ...
    var save = function () {
        ...
        return $http({
            method: 'POST',
            url: 'api/customer-config-process-staging/save',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            data: $.param(req)
        }).then(function (response) {
            $timeout(function () {
