 ...
 */
app.controller('customerConfigMacroProcessesCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        $scope.onLoadRecord = function () {
            if ($scope.macro.id != 0) {
                ...
                log.info("editando cliente con código: " + $scope.macro.id);
                var req = {
                    id: $scope.macro.id
                };
                $http({
                    method: 'GET',
                    url: 'api/customer/config-sgsst/macro/get',
                    params: req
                })
                    ...
                    .then(function (response) {
                        ...
                        $timeout(function () {
                            $scope.macro = response.data.result;
                        });
        ...
        var save = function () {
            var req = {};
            var data = JSON.stringify($scope.macro);
            req.data = Base64.encode(data);
            return $http({
                method: 'POST',
                url: 'api/customer/config-sgsst/macro/save',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                data: $.param(req)
            }).then(function (response) {
        ...
        $scope.dtInstanceConfigMacro = {};
		$scope.dtOptionsConfigMacro = DTOptionsBuilder.newOptions()
            // Add Bootstrap compatibility
            .withBootstrap().withOption('responsive', true)
            .withOption('ajax', {
                // Either you specify the AjaxDataProp here
                data: request,
                url: 'api/customer/config-sgsst/macro',
                type: 'POST',
                beforeSend: function () {
        ...
        $scope.dtColumnsConfigMacro = [
            ...
            DTColumnBuilder.newColumn('name').withTitle($translate.instant('grid.matrix.MACROPROCESS')),
        ...
        $scope.viewConfigMacro = function (id) {
            $scope.macro.id = id;
            $scope.isView = true;
            $scope.onLoadRecord();
        };
        ...
        var loadRow = function () {
            ...
            $("#dtConfigMacro a.editRow").on("click", function () {
                var id = $(this).data("id");
                $scope.editConfigMacro(id);
            });

            $("#dtConfigMacro a.viewRow").on("click", function () {
                var id = $(this).data("id");

                $scope.macro.id = id;
                $scope.viewConfigMacro(id);

            });

            $("#dtConfigMacro a.delRow").on("click", function () {
                var id = $(this).data("id");
                ...
                SweetAlert.swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $http({
                                method: 'POST',
                                url: 'api/customer/config-sgsst/macro/delete',
                                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                                data: $.param(req)
                            }).then(function (response) {
        ...
        $scope.reloadData = function () {
            $scope.dtInstanceConfigMacro.reloadData();
        };


        $scope.editConfigMacro = function (id) {
            $scope.macro.id = id;
            $scope.isView = false;
            $scope.onLoadRecord()
        };

        $scope.createConfigMacro = function () {
            var req = {};
            var request = {
                id: 0,
