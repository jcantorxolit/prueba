...
*/
app.controller('customerSafetyInspectionConfigHeaderListCtrl', ['$scope', '$stateParams', '$log','DTOptionsBuilder', 'DTColumnBuilder',
...
function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder,
    ...
    var log = $log;
    var request = {};
        log.info("loading..customerSafetyInspectionConfigHeaderListCtrl ");

    // Datatable configuration
    request.operation = "safety";
    request.customer_id = $stateParams.customerId;

    $scope.dtInstanceSafetyInspectionConfigHeader = {};
		$scope.dtOptionsSafetyInspectionConfigHeader = DTOptionsBuilder.newOptions()
    ...
    $scope.dtColumnsSafetyInspectionConfigHeader = [
    ...
    var loadRow = function () {
        ...
        $("#dtSafetyInspectionConfigHeader a.editRow").on("click", function () {
            var id = $(this).data("id");
            $scope.onEditRecord(id);
        });

        $("a.downloadRow").on("click", function () {
            var id = $(this).data("id");
            var url = $(this).data("url");
            //$scope.editTracking(id);
            if (url == "") {
                jQuery("#downloadDocument")[0].src = "api/customer/safety-inspection-config-header/download?id=" + id;
            }
        });

        $("#dtSafetyInspectionConfigHeader a.viewRow").on("click", function () {
            var id = $(this).data("id");
            $scope.onViewRecord(id);
        });

        $("#dtSafetyInspectionConfigHeader a.delRow").on("click", function () {
            var id = $(this).data("id");
            ...
            SweetAlert.swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $http({
                            method: 'POST',
                            url: 'api/customer/safety-inspection-config-header/delete',
    ...
    $scope.reloadData = function () {
        $scope.dtInstanceSafetyInspectionConfigHeader.reloadData();
    };
