 ...
 * @since  1.0
 */
app.controller('customerMatrixConfigWizardTabProjectCtrl', ['$scope', '$stateParams', '$log',
    ...
    function ($scope, $stateParams, $log, $compile, $state,
        ...
        $scope.onLoadRecord = function () {
            if ($scope.project.id != 0) {
                ...
                log.info("editando cliente con código: " + $scope.project.id);
                var req = {
                    id: $scope.project.id
                };
                $http({
                    method: 'GET',
                    url: 'api/customer/matrix-project',
                    params: req
                })
        ...
        var save = function () {
            ...
            return $http({
                method: 'POST',
                url: 'api/customer/matrix-project/save',
        ...
        $scope.dtInstanceMatrixConfigProject = {};
		$scope.dtOptionsMatrixConfigProject = DTOptionsBuilder.newOptions()
        ...
        $scope.dtColumnsMatrixConfigProject = [
            ...
            DTColumnBuilder.newColumn('name').withTitle("Proyecto").withOption('defaultContent', ''),
        ...
        var loadRow = function () {
            ...
            $("#dtMatrixConfigProject a.editRow").on("click", function () {
                var id = $(this).data("id");
                $scope.editMatrixConfigProject(id);
            });

            $("#dtMatrixConfigProject a.delRow").on("click", function () {
                var id = $(this).data("id");
                ...
                SweetAlert.swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $http({
                                method: 'POST',
                                url: 'api/customer/matrix-project/delete',
                                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                                data: $.param(req)
                            }).then(function (response) {
        ...
        $scope.reloadData = function () {
            $scope.dtInstanceMatrixConfigProject.reloadData();
        };

        $scope.editMatrixConfigProject = function (id) {
            if (id) {
                $scope.project.id = id;
                $scope.onLoadRecord();
            }
        ...
        $scope.clearMatrixConfigProject = function () {
            init();
        };
    }]);
