 ...
 * @since  1.0
 */
app.controller('customerOccupationalReportIncidentEditCtrl', ['$scope', '$stateParams', '$log',
    '$compile', '$state', 'SweetAlert', '$rootScope', '$http', '$timeout', '$uibModal', 'flowFactory', 'cfpLoadingBar', '$filter', '$aside', 'toaster',
    function ($scope, $stateParams, $log, $compile,  $state,
        ...
        log.info("loading..customerOccupationalReportIncidentEditCtrl.js con el id de tracking: ", idTraking);
        ...
        $scope.report = {
            ...
            isAnotherFactor: false,
            anotherFactorDescription: "",
            accidentDescription: "",
        ...
        var loadAllOccupations = function() {
            ...
            return $http({
                method: 'POST',
                url: 'api/customer/config-sgsst/job-activity/listBy',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                data: $.param(req)
            }).then(function (response) {
                $timeout(function () {
                    $scope.report_regular_task = response.data.data;
                });
            }).catch(function (e) {
        ...
        var onLoadRecord = function(id){
            ...
            if (id) {
                ...
                $http({
                    method: 'GET',
                    url: 'api/occupational-report-incident',
                    params: req
                })
...
*/
            ...
            return $http({
                method: 'POST',
                url: 'api/occupational-report-incident/save',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                data: $.param(req)
            }).then(function (response) {
                ...
                $timeout(function(){
                    ...
                    if (isNew) {
                        //$scope.$parent.navToSection("list", "list");
                        $scope.$parent.currentReport = response.data.result.id;
                        onLoadRecord($scope.$parent.currentReport);
                    } else {
                        $scope.report = response.data.result;
        ...
        $scope.$watch("report.employee", function () {
            ...
            $scope.report.job = $scope.report.employee.job.job ? $scope.report.employee.job.job : $scope.report.employee.job;
            //$scope.report.occupation = $scope.report.employee.occupation != '' ? $scope.report.employee.occupation : $scope.report.occupation;
            $scope.report.occupation = ($scope.report.occupation && $scope.report.occupation != '') ? $scope.report.occupation : $scope.report.employee.occupation;
        ...
        $scope.onAddEmployeeList = function() {
            var modalInstance = $aside.open({
                ...
                templateUrl: $rootScope.app.views.urlRoot + "modules/customer/occupational/report-incident/customer_absenteeism_disability_employee_list_modal.htm",
                placement: 'right',
                size: 'lg',
                backdrop: true,
