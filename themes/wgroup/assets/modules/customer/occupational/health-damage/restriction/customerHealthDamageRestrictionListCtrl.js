...
*/
app.controller('customerHealthDamageRestrictionListCtrl', ['$scope', '$stateParams', '$log','DTOptionsBuilder', 'DTColumnBuilder',
...
function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder,
    ...
    $scope.dtInstanceHealthDamageRestriction = {};
		$scope.dtOptionsHealthDamageRestriction = DTOptionsBuilder.newOptions()
    ...
    $scope.dtColumnsHealthDamageRestriction = [
    ...
    var loadRow = function () {
        ...
        $("#dtHealthDamageRestriction a.editRow").on("click", function () {
            var id = $(this).data("id");
            $scope.editWorkMedicine(id);
        });

        $("#dtHealthDamageRestriction a.viewRow").on("click", function () {
            var id = $(this).data("id");
            $scope.viewWorkMedicine(id);
        });

        $("#dtHealthDamageRestriction a.delRow").on("click", function () {
            var id = $(this).data("id");
            ...
            SweetAlert.swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $http({
                            method: 'POST',
                            url: 'api/customer/health-damage/restriction/delete',
    ...
    $scope.reloadData = function () {
        $scope.dtInstanceHealthDamageRestriction.reloadData();
    };
    ...
    $scope.dtInstanceHealthDamageDisability = {};
		$scope.dtOptionsHealthDamageDisability = DTOptionsBuilder.newOptions()
        // Add Bootstrap compatibility
        .withBootstrap().withOption('responsive', true)
        .withOption('ajax', {
            // Either you specify the AjaxDataProp here
    ...
    $scope.dtColumnsHealthDamageDisability = [
        DTColumnBuilder.newColumn(null).withTitle("Acciones").withOption('width', 150).notSortable()
            .renderWith(function (data, type, full, meta) {
                var actions = "";
                var editTemplate = '<a class="btn btn-primary btn-xs editRow lnk" href="#" uib-tooltip="Editar" data-id="' + data.id + '" >' +
        ...
        DTColumnBuilder.newColumn('examinationDateFormat').withTitle("Fecha Registro").withOption('width', 200).withOption('defaultContent', ''),
        DTColumnBuilder.newColumn('employee.entity.firstName').withTitle("Nombre").withOption('width', 200).withOption('defaultContent', ''),
        DTColumnBuilder.newColumn('employee.entity.lastName').withTitle("Apellidos").withOption('width', 200).withOption('defaultContent', ''),
        DTColumnBuilder.newColumn('employee.job.name').withTitle("Cargo").withOption('width', 200).withOption('defaultContent', '')
    ];
