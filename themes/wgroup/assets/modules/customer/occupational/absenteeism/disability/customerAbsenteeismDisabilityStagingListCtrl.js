 ...
 */
app.controller('customerAbsenteeismDisabilityStagingListCtrl', ['$scope', '$stateParams', '$log', 'DTOptionsBuilder', 'DTColumnBuilder',
    ...
    function ($scope, $stateParams, $log, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $compile, toaster, $state,
        ...
        $scope.dtInstanceDiagnosticDisabilityStagingDT = {};
        $scope.dtOptionsDiagnosticDisabilityStagingDT = DTOptionsBuilder.newOptions()
            ...
            .withOption('ajax', {
                ...
                url: 'api/customer-absenteeism-disability-staging',
        ...
        $scope.dtColumnsDiagnosticDisabilityStagingDT = [
            DTColumnBuilder.newColumn(null).withTitle("Acciones").withOption('width', 150).notSortable()
                .renderWith(function (data, type, full, meta) {
                    ...
                    var addDisabilityTemplate = '<a class="btn btn-info btn-xs addRow lnk" href="#" uib-tooltip="Relacionar ausentismo"  data-id="' + data.id + '"' + disabled + ' >' +
                    '   <i class="fa fa-plus"></i></a> ';

                    var actions = data.isValid == 0 || !data.isValid ? editTemplate : '';

                    actions += (data.disabilityParentIsRequired == 1 || data.disabilityParentIsRequired) ? addDisabilityTemplate : '';
            ...
            DTColumnBuilder.newColumn('category').withTitle("Tipo Ausentismo").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('type').withTitle("Tipo Incapacidad").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('cause').withTitle("Causa Incapacidad").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('accidentType').withTitle("Tipo Impacto").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('disabilityParent').withTitle("Relacionar Ausentismo").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('diagnostic').withTitle("Diagnóstico").withOption('width', 100).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('start').withTitle("F Inicial").withOption('width', 180).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('end').withTitle("F Final").withOption('width', 180).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('bodyPart').withTitle("Parte Cuerpo").withOption('width', 180).withOption('defaultContent', ''),
            DTColumnBuilder.newColumn('observation').withTitle("Observación").withOption('width', 280).withOption('defaultContent', '')
        ];

        var loadRow = function () {
            angular.element("#dataDiagnosticDisabilityStagingDT a.editRow").on("click", function () {
                var id = angular.element(this).data("id");
                onEdit(id);
            });

            angular.element("#dataDiagnosticDisabilityStagingDT a.addRow").on("click", function () {
                var id = angular.element(this).data("id");
                onAddDisabilityParent({
                    id: id
                });
            });
        }

        $scope.dtInstanceDiagnosticDisabilityStagingDTCallback = function(instance) {
            $scope.dtInstanceDiagnosticDisabilityStagingDT = instance;
        };

        $scope.reloadData = function () {
            $scope.dtInstanceDiagnosticDisabilityStagingDT.reloadData();
        };

        var onEdit = function (id) {
            var modalInstance = $aside.open({
                templateUrl: $rootScope.app.views.urlRoot + "modules/customer/occupational/absenteeism/disability/customer_absenteeism_disability_staging_modal.htm",
                placement: 'right',
                windowTopClass: 'top-modal',
                size: 'lg',
                backdrop: true,
                controller: 'ModalInstanceSideCustomerAbsenteeismDisabilityStagingEditCtrl',
                scope: $scope,
                resolve: {
                    dataItem: function () {
                        return { id: id ? id : 0 };
        ...
        var onAddDisabilityParent = function (dataItem) {
            var modalInstance = $aside.open({
                ...
                size: 'lg',
                backdrop: true,
                controller: 'ModalInstanceSideCustomerAbsenteeismDisabilityParentStagingCtrl',
                resolve: {
                    dataItem: function () {
                        return dataItem;
                    }
        ...
        $scope.onProcess = function () {
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    return $http({
                        method: 'POST',
                        url: $exportUrl + 'api/v1/customer-absenteeism-import/confirm',
