 ...
 * @author TeamCloud
 */
class CustomerConfigActivityRepository
{
    ...
    protected $columns = array('wg_customer_config_activity.*');
    ...
    public function __construct(CustomerConfigActivity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_config_activity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        $query = $this->query();
        $query->select(['wg_customer_config_activity.*']);
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function ($join) {
            $join->on('wg_customer_config_activity.status', '=', 'config_workplace_status.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_activity.customer_id") {
