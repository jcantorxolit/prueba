...
class SystemParameterRepository {
     ...
     * @var array
     */
    protected $currentSort = [array('id', 'asc')];
    protected $columns = array('system_parameters.*');
    ...
    public function __construct(SystemParameter $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['system_parameters.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "system_parameters.group") {
    ...
    public function getFilteredRelation($filter = array(), $isCount = false, $parent = "") {
        $query = $this->query();
        $query->select(
            ['system_parameters.*', "parent_relation.item as parent"]
        );

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable($parent, 'parent_relation')), function($join)
        {
            $join->on('system_parameters.code', '=', 'parent_relation.value');
        });
        ...
        if ($isCount) {
            ...
            return ($this->perPage > 0) ? $query->paginate($this->perPage, ['system_parameters.*', "parent_relation.item as parent"]) : $query->get(['system_parameters.*', "parent_relation.item as parent"]);
        }
    }
