 ...
 * @author TeamCloud
 */
class CustomerMatrixActivityRepository
{
    ...
    protected $columns = array('wg_customer_matrix_activity.*');
    ...
    public function __construct(CustomerMatrixActivity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_matrix_activity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_matrix", "wg_customer_matrix.id", "=", "wg_customer_matrix_activity.customer_matrix_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix_activity.customer_matrix_id") {
