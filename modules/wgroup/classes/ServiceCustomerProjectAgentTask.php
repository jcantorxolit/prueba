...
class ServiceCustomerProjectAgentTask {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId = 0, $isCustomerVisible = false) {
        ...
        $this->customerProjectAgentTaskRepository = new CustomerProjectAgentTaskRepository($model);
        ...
        $columns = [
            ...
            'wg_customer_project_agent_task.agent_id',
            'wg_customer_project_agent_task.observation',
            'wg_customer_project_agent_task.status',
            'wg_customer_project_agent_task.eventDateTime'
        ];
        ...
        if (empty($sorting)) {
            $this->customerProjectAgentTaskRepository->sortBy('wg_customer_project_agent_task.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_customer_project_agent_task.customer_id', $customerId);

        if ($isCustomerVisible)
        {
            $filters[] = array('wg_customer_project_agent_task.isVisible', 1);
        }

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_customer_project_agent_task.type', $search);
            $filters[] = array('wg_customer_project_agent_task.agent_id', $search);
            $filters[] = array('wg_customer_project_agent_task.observation', $search);
            $filters[] = array('wg_customer_project_agent_task.status', $search);
            $filters[] = array('wg_customer_project_agent_task.eventDateTime', $search);
            $filters[] = array('wg_agent.name', $search);
        }

        if ($typeFilter == "1") {
            $filters[] = array('wg_customer_project_agent_task.status', '1');
        } else if ($typeFilter == "0") {
            $filters[] = array('wg_customer_project_agent_task.status', '0');
        }


        $this->customerProjectAgentTaskRepository->setColumns(['wg_customer_project_agent_task.*']);
