 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionConfigListRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_config_list.*');
    ...
    public function __construct(CustomerSafetyInspectionConfigList $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_config_list.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_safety_inspection_config_list.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_config_list.customer_id") {
        ...
        if ($filterContractor) {
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_customer_safety_inspection_config_list.contractor_id") {
                        $query->where($field, "=", $valueField, 'and');
                    } else if ($field == "wg_customer_safety_inspection_config_list.customer_id") {
                        $query->where($field, "=", $valueField, 'and');
                    }
