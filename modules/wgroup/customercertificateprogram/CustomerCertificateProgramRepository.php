 ...
 * @author TeamCloud
 */
class CustomerCertificateProgramRepository {
    ...
    protected $columns = array('wg_customer_certificate_program.*');
    ...
    public function __construct(CustomerCertificateProgram $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_certificate_program.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_certificate_program", function($join)
        {
            $join->on('wg_customer_certificate_program.certificate_program_id', '=', 'wg_certificate_program.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_certificate_program.customer_id") {
