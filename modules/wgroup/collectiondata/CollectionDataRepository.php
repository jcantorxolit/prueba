...
/**
 * Description of CollectionDataRepository
 *
 * @author TeamCloud
 */
class CollectionDataRepository {
    ...
    protected $columns = array('wg_collection_data.*');
    ...
    public function __construct(CollectionData $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_collection_data.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_collection_data.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_collection_data.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_collection_data.type', '=', 'diags.value');
            });*/
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_collection_data.id") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_collection_data.module") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function($query) use ($filter){
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_collection_data.id" && $field != "wg_collection_data.module") {
