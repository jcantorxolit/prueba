...
namespace Wgroup\CustomerOccupationalInvestigationAlFactor;
 ...
 * @author jdblandon
 */
class CustomerOccupationalInvestigationAlFactorDTO {
    ...
    public static function  bulkInsert($object, $entityId)
    {
        ...
        try {
            foreach ($object as $record) {
                ...
                if ($record) {
                    if ($record->id) {
                        if (!($model = CustomerOccupationalInvestigationAlFactor::find($record->id))) {
                            $isEdit = false;
                            $model = new CustomerOccupationalInvestigationAlFactor();
                        } else {
                            if (!$record->isActive) {
                                $model->delete();
                            }
        ...
        return CustomerOccupationalInvestigationAlFactor::find($model->id);
    }
