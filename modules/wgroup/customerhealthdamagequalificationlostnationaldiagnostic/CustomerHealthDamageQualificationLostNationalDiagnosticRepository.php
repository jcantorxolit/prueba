 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostNationalDiagnosticRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_second_instance_detail.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostNationalDiagnostic $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_second_instance_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_ql_second_instance", function ($join) {
            $join->on('wg_customer_health_damage_ql_second_instance_detail.customer_health_damage_ql_second_instance_id', '=', 'wg_customer_health_damage_ql_second_instance.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_second_instance_detail.customer_health_damage_ql_second_instance_id") {
