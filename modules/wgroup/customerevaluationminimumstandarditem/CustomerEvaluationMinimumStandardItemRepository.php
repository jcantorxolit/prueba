 ...
 * @author TeamCloud
 */
class CustomerEvaluationMinimumStandardItemRepository
{
    ...
    protected $columns = array('wg_customer_evaluation_minimum_standard_item.*');
    ...
    public function __construct(CustomerEvaluationMinimumStandardItem $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_evaluation_minimum_standard_item.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_evaluation_minimum_standard_item.status") {
