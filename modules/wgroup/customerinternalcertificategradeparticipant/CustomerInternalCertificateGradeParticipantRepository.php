...
/**
 * Description of CustomerInternalCertificateGradeParticipantRepository
 *
 * @author TeamCloud
 */
class CustomerInternalCertificateGradeParticipantRepository {
    ...
    protected $columns = array('wg_customer_internal_certificate_grade_participant.*');
    ...
    public function __construct(CustomerInternalCertificateGradeParticipant $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_grade_participant.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_grade_participant.*']);
        $query->join("wg_customer_internal_certificate_grade", "wg_customer_internal_certificate_grade.id", "=", "wg_customer_internal_certificate_grade_participant.customer_internal_certificate_grade_id");
        $query->join("wg_customer_employee", "wg_customer_employee.id", "=", "wg_customer_internal_certificate_grade_participant.customer_employee_id");
        $query->join("wg_employee", "wg_employee.id", "=", "wg_customer_employee.employee_id");
        $query->join("wg_customers", function($join)
        {
            $join->on('wg_customer_internal_certificate_grade_participant.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_certificate_grade_participant.customer_internal_certificate_grade_id") {
