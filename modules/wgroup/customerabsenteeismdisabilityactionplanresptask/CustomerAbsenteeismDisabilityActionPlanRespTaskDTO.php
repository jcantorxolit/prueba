 ...
 * @author jdblandon
 */
class CustomerAbsenteeismDisabilityActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo CustomerAbsenteeismDisabilityActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerAbsenteeismDisabilityActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new CustomerAbsenteeismDisabilityActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new CustomerAbsenteeismDisabilityActionPlanRespTask();
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new CustomerAbsenteeismDisabilityActionPlanRespTaskTracking();
        ...
        return CustomerAbsenteeismDisabilityActionPlanRespTask::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerAbsenteeismDisabilityActionPlanRespTask) {
                    $parsed[] = (new CustomerAbsenteeismDisabilityActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerAbsenteeismDisabilityActionPlanRespTask) {
            return (new CustomerAbsenteeismDisabilityActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerAbsenteeismDisabilityActionPlanRespTaskDTO();
            }
        }
    }
