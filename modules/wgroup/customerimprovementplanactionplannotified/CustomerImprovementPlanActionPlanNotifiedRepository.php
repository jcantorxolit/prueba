 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanActionPlanNotifiedRepository
{
    ...
    public function __construct(CustomerImprovementPlanActionPlanNotified $model)
    {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join('wg_customer_improvement_plan', function ($join) {
            $join->on('wg_customer_improvement_plan_tracking.customer_improvement_plan_action_plan_id', '=', 'wg_customer_improvement_plan.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_tracking.customer_improvement_plan_action_plan_id") {
