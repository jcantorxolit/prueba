 ...
 * @author TeamCloud
 */
class CustomerConfigWorkPlaceRepository {
    ...
    protected $columns = array('wg_customer_config_workplace.*');
    ...
    public function __construct(CustomerConfigWorkPlace $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_workplace.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin("rainlab_user_countries", "rainlab_user_countries.id", "=", "wg_customer_config_workplace.country_id")
            ->leftjoin("rainlab_user_states", "rainlab_user_states.id", "=", "wg_customer_config_workplace.state_id")
            ->leftjoin("wg_towns", "wg_towns.id", "=", "wg_customer_config_workplace.city_id")
            ->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function($join)
            {
                $join->on('wg_customer_config_workplace.status', '=', 'config_workplace_status.value');
            })->leftjoin(DB::raw(SystemParameter::getRelationTable('wg_structure_type')), function($join)
            {
                $join->on('wg_customer_config_workplace.type', '=', 'wg_structure_type.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_workplace.customer_id") {
