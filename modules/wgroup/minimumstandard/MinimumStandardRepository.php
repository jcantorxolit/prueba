...
/**
 * Description of MinimumStandardRepository
 *
 * @author TeamCloud
 */
class MinimumStandardRepository
{
    ...
    protected $columns = array('wg_minimum_standard.*');
    ...
    public function __construct(MinimumStandard $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_minimum_standard.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('minimum_standard_type')), function ($join) {
            $join->on('wg_minimum_standard.type', '=', 'minimum_standard_type.value');
