...
/**
 * Description of CustomerInternalCertificateGradeRepository
 *
 * @author TeamCloud
 */
class CustomerInternalCertificateGradeRepository {
    ...
    protected $columns = array('wg_customer_internal_certificate_grade.*');
    ...
    public function __construct(CustomerInternalCertificateGrade $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_grade.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_grade.*']);
        //$query->join("wg_customers", "wg_customers.id", "=", "wg_customer_internal_certificate_grade.customer_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_certificate_grade.customer_id") {
