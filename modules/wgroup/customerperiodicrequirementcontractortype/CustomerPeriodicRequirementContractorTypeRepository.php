 ...
 * @author TeamCloud
 */
class CustomerPeriodicRequirementContractorTypeRepository {
    ...
    protected $columns = array('wg_customer_periodic_requirement.*');
    ...
    public function __construct(CustomerPeriodicRequirementContractorType $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_periodic_requirement.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_periodic_requirement.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_periodic_requirement.customer_id") {
