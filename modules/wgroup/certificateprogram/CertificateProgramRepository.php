...
/**
 * Description of CertificateProgramRepository
 *
 * @author TeamCloud
 */
class CertificateProgramRepository {
    ...
    protected $columns = array('wg_certificate_program.*');
    ...
    public function __construct(CertificateProgram $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_program.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_certificate_program.*']);
        //$query->join("wg_customers", "wg_customers.id", "=", "wg_certificate_program.customer_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_program.customer_id") {
