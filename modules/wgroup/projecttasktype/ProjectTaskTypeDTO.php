...
class ProjectTaskTypeDTO
{
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = ProjectTaskType::find($object->id))) {
                // No existe
        ...
        return ProjectTaskType::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1")
    {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof ProjectTaskType) {
                    $parsed[] = (new ProjectTaskTypeDTO())->parseModel($model, $fmt_response);
                } else {
                    $parsed[] = (new ProjectTaskTypeDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof ProjectTaskType) {
            return (new ProjectTaskTypeDTO())->parseModel($data, $fmt_response);
        } else {
