 ...
 * @author TeamCloud
 */
class ProjectTaskTypeRepository {
     ...
     * @var array
     */
    protected $currentSort = [array('description', 'asc')];
    protected $columns = array('wg_project_task_type.*');
    ...
    public function __construct(ProjectTaskType $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_project_task_type.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_project_task_type.isActive") {
