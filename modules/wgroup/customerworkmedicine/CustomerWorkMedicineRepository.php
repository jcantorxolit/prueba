 ...
 * @author TeamCloud
 */
class CustomerWorkMedicineRepository {
    ...
    protected $columns = array('wg_customer_work_medicine.*');
    ...
    public function __construct(CustomerWorkMedicine $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_work_medicine.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_work_medicine.customer_employee_id', '=', 'wg_customer_employee.id');
        });
        $query->join("wg_employee", function($join)
        {
            $join->on('wg_customer_employee.employee_id', '=', 'wg_employee.id');
        });
        $query->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_medicine_medical_concept') work_medicine_medical_concept "), function($join)
        {
            $join->on('wg_customer_work_medicine.medicalConcept', '=', 'work_medicine_medical_concept.value');
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_medicine_examination_type') work_medicine_examination_type "), function($join)
        {
            $join->on('wg_customer_work_medicine.examinationType', '=', 'work_medicine_examination_type.value');
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'employee_contract_type') employee_contract_type "), function($join)
        {
            $join->on(DB::raw('wg_customer_employee.contractType COLLATE utf8_general_ci'), '=', 'employee_contract_type.value');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                } else if ($field == "wg_customer_work_medicine.customer_employee_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
