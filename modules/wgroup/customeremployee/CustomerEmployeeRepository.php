...
class CustomerEmployeeRepository {
     ...
     * @var array
     */
    protected $currentSort = [array('firstName', 'asc')];
    protected $columns = array('wg_customer_employee.*');
    ...
    public function __construct(CustomerEmployee $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_employee.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join(DB::raw("(SELECT * FROM system_parameters sp where namespace = 'wgroup' and sp.group = 'gender') wg_gender "), function($join)
        {
            $join->on(DB::raw('wg_employee.gender COLLATE utf8_general_ci'), '=', 'wg_gender.value');
        })->join(DB::raw("(SELECT * FROM system_parameters sp where namespace = 'wgroup' and sp.group = 'employee_document_type') wg_document_type "), function($join)
        {
            $join->on(DB::raw('wg_employee.documentType COLLATE utf8_general_ci'), '=', 'wg_document_type.value');
        });
