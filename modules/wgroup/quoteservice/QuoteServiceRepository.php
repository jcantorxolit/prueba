...
/**
 * Description of QuoteServiceRepository
 *
 * @author TeamCloud
 */
class QuoteServiceRepository {
    ...
    protected $columns = array('wg_quote_service.*');
    ...
    public function __construct(QuoteService $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_quote_service.isActive") {
