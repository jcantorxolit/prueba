 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanAlertRepository
{
    ...
    protected $columns = array('wg_customer_improvement_plan_alert.*');
    ...
    public function __construct(CustomerImprovementPlanAlert $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_alert.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_alert.status") {
