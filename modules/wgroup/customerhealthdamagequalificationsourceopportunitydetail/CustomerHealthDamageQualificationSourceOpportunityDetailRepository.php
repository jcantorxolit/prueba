 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceOpportunityDetailRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_first_opportunity_detail.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceOpportunityDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_first_opportunity_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        {
            $join->on('wg_customer_health_damage_qs_first_opportunity_detail.diagnostic', '=', 'wg_disability_diagnostic.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_first_opportunity_detail.customer_health_damage_qs_first_opportunity_id") {
    ...
    public function getFilterContract($filter = array(), $isCount = false, $join = "")
    {
        ...
        {
            $join->on('wg_customer_health_damage_qs_first_opportunity_detail.restriction', '=', 'work_health_damage_restriction.value');
        ...
        {
            $join->on('wg_customer_health_damage_qs_first_opportunity_detail.whoPerceived', '=', 'work_health_damage_who_perceived.value');
        });
