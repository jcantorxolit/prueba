...
/**
 * Description of CustomerInternalCertificateProgramSpecialityRepository
 *
 * @author TeamCloud
 */
class CustomerInternalCertificateProgramSpecialityRepository {
    ...
    protected $columns = array('wg_customer_internal_certificate_program_speciality.*');
    ...
    public function __construct(CustomerInternalCertificateProgramSpeciality $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program_speciality.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program_speciality.*']);
        $query->join("wg_certificate_program", "wg_certificate_program.id", "=", "wg_customer_internal_certificate_program_speciality.certificate_program_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_certificate_program_speciality.certificate_program_id") {
