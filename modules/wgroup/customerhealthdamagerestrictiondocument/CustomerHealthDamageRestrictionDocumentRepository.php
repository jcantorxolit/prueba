 ...
 * @author TeamCloud
 */
class CustomerHealthDamageRestrictionDocumentRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_restriction_document.*');
    ...
    public function __construct(CustomerHealthDamageRestrictionDocument $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_restriction_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('work_health_damage_restriction_document_type')), function ($join) {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_restriction_document.customer_health_damage_restriction_id") {
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_health_damage_restriction_document.customer_health_damage_restriction_id") {
                        if ($field == 'wg_customer_health_damage_restriction_document.isPermanentManagement') {
