 ...
 * @author TeamCloud
 */
class CustomerOccupationalInvestigationAlRepository {
    ...
    protected $columns = array('wg_customer_occupational_investigation_al.*');
    ...
    public function __construct(CustomerOccupationalInvestigationAl $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_occupational_investigation_al.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_occupational_investigation_al.customer_id', '=', 'wg_customers.id');
        });

        $query->join("wg_customer_employee", function($join)
        {
            $join->on('wg_customer_occupational_investigation_al.customer_employee_id', '=', 'wg_customer_employee.id');
        });
...
;
        ...
        {
            $join->on('wg_customer_occupational_investigation_al.accidentType', '=', 'investigation_accident_type.value');
        });
