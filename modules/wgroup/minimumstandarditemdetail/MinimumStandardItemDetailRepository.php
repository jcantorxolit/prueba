 ...
 * @author TeamCloud
 */
class MinimumStandardItemDetailRepository
{
    ...
    public function __construct(MinimumStandardItemDetail $model)
    {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_minimum_standard_item", function ($join) {
            $join->on('wg_minimum_standard_item_detail.minimum_standard_item_id', '=', 'wg_minimum_standard_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_minimum_standard_item_detail.minimum_standard_item_id") {
