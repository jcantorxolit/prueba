...
/**
 * Description of CertificateGradeAgentRepository
 *
 * @author TeamCloud
 */
class CertificateGradeAgentRepository {
    ...
    protected $columns = array('wg_certificate_grade_agent.*');
    ...
    public function __construct(CertificateGradeAgent $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_grade_agent.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_certificate_grade_agent.*']);
        $query->join("wg_certificate_grade", "wg_certificate_grade.id", "=", "wg_certificate_grade_agent.certificate_grade_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_grade_agent.certificate_grade_id") {
