...
/**
 * Description of CertificateGradeAgentDTO
 *
 * @author jdblandon
 */
class CertificateGradeAgentDTO {
    ...
    /**
     * @param $model: Modelo CertificateGradeAgent
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CertificateGradeAgent::find($object->id))) {
                // No existe
        ...
        return CertificateGradeAgent::find($model->id);
    }
    ...
    public static function  bulkInsert($object)
    {
        ...
        try {
            ...
            foreach ($object->agents as $agent) {
                ...
                if ($agent) {
                    if ($agent->id) {
                        if (!($model = CertificateGradeAgent::find($agent->id))) {
                            $isEdit = false;
                            $model = new CertificateGradeAgent();
                        }
                    } else {
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CertificateGradeAgent) {
                    $parsed[] = (new CertificateGradeAgentDTO())->parseModel($model, $fmt_response);
                }else {
                    $parsed[] = (new CertificateGradeAgentDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CertificateGradeAgent) {
            return (new CertificateGradeAgentDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CertificateGradeAgentDTO();
            }
        }
    }
