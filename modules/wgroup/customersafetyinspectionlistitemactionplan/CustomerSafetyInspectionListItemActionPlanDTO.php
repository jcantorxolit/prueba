...
use Wgroup\CustomerSafetyInspectionListItemActionPlanAlert\CustomerSafetyInspectionListItemActionPlanAlertDTO;
use Wgroup\CustomerSafetyInspectionListItemActionPlanResp\CustomerSafetyInspectionListItemActionPlanResp;
use Wgroup\CustomerSafetyInspectionListItemActionPlanResp\CustomerSafetyInspectionListItemActionPlanRespDTO;
 ...
 * @author jdblandon
 */
class CustomerSafetyInspectionListItemActionPlanDTO {
    ...
    /**
     * @param $model: Modelo CustomerSafetyInspectionListItemActionPlan
     */
    private function getBasicInfo($model) {
        ...
        $this->alerts = CustomerSafetyInspectionListItemActionPlanAlertDTO::parse($model->getAlerts());
        $this->responsibles = CustomerSafetyInspectionListItemActionPlanRespDTO::parse($model->getResponsible());
    ...
    public static function  UpdateModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerSafetyInspectionListItemActionPlan::find($object->id))) {
                // No existe
                $model = new CustomerSafetyInspectionListItemActionPlan();
                $isEdit = false;
            }
        } else {
            $model = new CustomerSafetyInspectionListItemActionPlan();
    ...
    public static function  fillAndSaveModel($object)
    {
            ...
            foreach ($object->alerts as $actionPlanAlert) {
                if ($actionPlanAlert->type != null) {
                    ...
                    if ($actionPlanAlert->id) {
                        // Existe
                        if (!($customerManagementActionPlanAlert = CustomerSafetyInspectionListItemActionPlanAlert::find($actionPlanAlert->id))) {
                            // No existe
                            $customerManagementActionPlanAlert = new CustomerSafetyInspectionListItemActionPlanAlert();
                            $isAlertEdit = false;
                        }
                    } else {
                        $customerManagementActionPlanAlert = new CustomerSafetyInspectionListItemActionPlanAlert();
                ...
                if ($actionPlanResponsible && $actionPlanResponsible->contactId > 0) {
                    if ($actionPlanResponsible->isActive)
                    {
                        if ($actionPlanResponsible->id) {
                            // Existe
                            if (!($customerManagementActionPlanResp = CustomerSafetyInspectionListItemActionPlanResp::find($actionPlanResponsible->id))) {
                                // No existe
                                $customerManagementActionPlanResp = new CustomerSafetyInspectionListItemActionPlanResp();
                                $isAlertEdit = false;
                            }
                        } else {
                            $customerManagementActionPlanResp = new CustomerSafetyInspectionListItemActionPlanResp();
        ...
        return CustomerSafetyInspectionListItemActionPlan::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerSafetyInspectionListItemActionPlan) {
                    $parsed[] = (new CustomerSafetyInspectionListItemActionPlanDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerSafetyInspectionListItemActionPlan) {
            return (new CustomerSafetyInspectionListItemActionPlanDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerSafetyInspectionListItemActionPlanDTO();
            }
        }
    }
