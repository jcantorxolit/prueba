...
/**
 * Description of PollQuestionRepository
 *
 * @author TeamCloud
 */
class PollQuestionRepository {
    ...
    protected $columns = array('wg_poll_question.*');
    ...
    public function __construct(PollQuestion $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_poll_question.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_poll_question.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_poll_question.type', '=', 'diags.value');
            });*/
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_poll_question.poll_id") {
