 ...
 * @author TeamCloud
 */
class CustomerRoadSafetyRepository {
    ...
    protected $columns = array('wg_customer_road_safety.*');
    ...
    public function __construct(CustomerRoadSafety $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_road_safety.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_road_safety.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_road_safety.customer_id") {
