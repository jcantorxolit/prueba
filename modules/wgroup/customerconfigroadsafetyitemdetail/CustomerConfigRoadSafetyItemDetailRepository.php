 ...
 * @author TeamCloud
 */
class CustomerConfigRoadSafetyItemDetailRepository
{
    ...
    protected $columns = array('wg_customer_config_road_safety_item_detail.*');
    ...
    public function __construct(CustomerConfigRoadSafetyItemDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_config_road_safety_item_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_road_safety_item_detail", function ($join) {
            $join->on('wg_customer_config_road_safety_item_detail.road_safety_item_detail_id', '=', 'wg_customer_road_safety_item_detail.id');

        })->join("wg_road_safety_item", function ($join) {
            $join->on('wg_customer_road_safety_item_detail.wg_road_safety_id', '=', 'wg_road_safety_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_road_safety_item_detail.customer_id") {
