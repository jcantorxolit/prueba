...
class PollRepository {
    ...
    protected $columns = array('wg_poll.*');
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_poll.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_poll.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_poll.type', '=', 'diags.value');
            });*/
