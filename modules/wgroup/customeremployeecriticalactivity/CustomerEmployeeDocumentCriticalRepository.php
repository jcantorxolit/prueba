...
/**
 * Description of CustomerEmployeeCriticalActivityRepository
 *
 * @author TeamCloud
 */
class CustomerEmployeeCriticalActivityRepository {
    ...
    protected $columns = array('wg_customer_employee_critical_activity.*');
    ...
    public function __construct(CustomerEmployeeCriticalActivity $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_employee_critical_activity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_employee_critical_activity.customer_employee_id") {
