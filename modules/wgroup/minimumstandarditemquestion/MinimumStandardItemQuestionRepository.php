 ...
 * @author TeamCloud
 */
class MinimumStandardItemQuestionRepository
{
    ...
    protected $columns = array('wg_minimum_standard_item_detail.*');
    ...
    public function __construct(MinimumStandardItemQuestion $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_minimum_standard_item_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_minimum_standard_item", function ($join) {
            $join->on('wg_minimum_standard_item_detail.wg_minimum_standard_item_id', '=', 'wg_minimum_standard_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_minimum_standard_item_detail.wg_minimum_standard_item_id") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_minimum_standard_item_detail.type") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_minimum_standard_item_detail.wg_minimum_standard_item_id"
                        && $field != "wg_minimum_standard_item_detail.type"
                    ) {
