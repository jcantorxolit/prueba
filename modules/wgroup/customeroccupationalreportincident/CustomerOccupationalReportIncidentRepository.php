 ...
 * @author TeamCloud
 */
class CustomerOccupationalReportIncidentRepository {
    ...
    protected $columns = array('wg_customer_occupational_report_incident.*');
    ...
    public function __construct(CustomerOccupationalReportIncident $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_occupational_report_incident.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_occupational_report_incident.customer_employee_id', '=', 'wg_customer_employee.id');
        });
        $query->join("wg_employee", function($join)
        {
            $join->on('wg_customer_employee.employee_id', '=', 'wg_employee.id');
        });
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'absenteeism_disability_type') dtype "), function($join)
        {
            $join->on('wg_customer_occupational_report_incident.type', '=', 'dtype.value');
        })->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'employee_contract_type') ctype "), function($join)
        {
            $join->on(DB::raw('wg_customer_employee.contractType COLLATE utf8_general_ci'), '=', 'ctype.value');
        });
