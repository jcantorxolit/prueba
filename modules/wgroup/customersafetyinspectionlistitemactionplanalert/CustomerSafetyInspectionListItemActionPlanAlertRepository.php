 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionListItemActionPlanAlertRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_list_item_action_plan_alert.*');
    ...
    public function __construct(CustomerSafetyInspectionListItemActionPlanAlert $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_list_item_action_plan_alert.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_list_item_action_plan_alert.status") {
