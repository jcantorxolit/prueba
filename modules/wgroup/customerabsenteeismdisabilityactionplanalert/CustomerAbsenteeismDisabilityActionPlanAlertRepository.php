 ...
 * @author TeamCloud
 */
class CustomerAbsenteeismDisabilityActionPlanAlertRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_disability_action_plan_alert.*');
    ...
    public function __construct(CustomerAbsenteeismDisabilityActionPlanAlert $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_disability_action_plan_alert.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_absenteeism_disability_action_plan_alert.status") {
