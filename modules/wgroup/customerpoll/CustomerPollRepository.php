...
class CustomerPollRepository {
    ...
    protected $columns = array('wg_customer_poll.*');
    ...
    public function __construct(CustomerPoll $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_poll", "wg_poll.id", "=", "wg_customer_poll.poll_id");
        ...
        {
            $join->on('wg_customer_poll.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_poll.customer_id") {
    ...
    public function getFilteredOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_poll.*']);
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_poll.poll_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
