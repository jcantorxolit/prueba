...
class QuoteDetailRepository {
    ...
    protected $columns = array('wg_report.*');
    ...
    public function __construct(QuoteDetail $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_report.*']);
        $query->join("wg_agent", "wg_agent.id", "=", "wg_report.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_report.type', '=', 'diags.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_report.customer_id") {
