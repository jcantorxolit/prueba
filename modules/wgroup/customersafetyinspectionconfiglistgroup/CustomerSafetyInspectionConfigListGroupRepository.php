 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionConfigListGroupRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_config_list_group.*');
    ...
    public function __construct(CustomerSafetyInspectionConfigListGroup $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_config_list_group.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_config_list_group.customer_safety_inspection_config_list_id") {
