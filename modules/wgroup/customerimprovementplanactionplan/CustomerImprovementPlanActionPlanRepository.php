 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanActionPlanRepository
{
    ...
    protected $columns = array('wg_customer_improvement_plan_action_plan.*');
    ...
    public function __construct(CustomerImprovementPlanActionPlan $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_action_plan.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_improvement_plan", function ($join) {
            ...
            $join->on('wg_customer_improvement_plan_action_plan.customer_improvement_plan_cause_root_cause_id', '=', 'wg_customer_improvement_plan_cause_root_cause.id');

        })->leftjoin('wg_customer_improvement_plan_cause', function ($join) {
            $join->on('wg_customer_improvement_plan_cause_root_cause.customer_improvement_plan_cause_id', '=', 'wg_customer_improvement_plan_cause.id');

        })->leftjoin('wg_improvement_plan_cause_category', function ($join) {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_action_plan.customer_improvement_plan_id") {
    ...
    public function getFilteredOptionalCause($filter = array(), $isCount = false, $join = "")
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_action_plan.*']);
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_action_plan.customer_improvement_plan_cause_root_cause_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
