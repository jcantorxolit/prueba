 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostOpportunityRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_first_opportunity.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostOpportunity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_first_opportunity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_ql", function ($join) {
            $join->on('wg_customer_health_damage_ql_first_opportunity.customer_health_damage_qualification_lost_id', '=', 'wg_customer_health_damage_ql.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_first_opportunity.customer_health_damage_qualification_lost_id") {
