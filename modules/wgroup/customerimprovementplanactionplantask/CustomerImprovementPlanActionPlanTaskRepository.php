 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanActionPlanTaskRepository
{
    ...
    protected $columns = array('wg_customer_improvement_plan_action_plan_task.*');
    ...
    public function __construct(CustomerImprovementPlanActionPlanTask $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_action_plan_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join('wg_customer_improvement_plan_action_plan', function ($join) {
            $join->on('wg_customer_improvement_plan_action_plan_task.customer_improvement_plan_action_plan_id', '=', 'wg_customer_improvement_plan_action_plan.id');
            ...
            $join->on('wg_customer_improvement_plan_action_plan_task.responsible', '=', 'responsible.id');
            $join->on('wg_customer_improvement_plan_action_plan_task.responsibleType', '=', 'responsible.type');
            $join->on('wg_customer_improvement_plan.customer_id', '=', 'responsible.customer_id');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('improvement_plan_action_plan_task_type')), function ($join) {
            $join->on('wg_customer_improvement_plan_action_plan_task.type', '=', 'improvement_plan_action_plan_task_type.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('improvement_plan_action_plan_task_status')), function ($join) {
            $join->on('wg_customer_improvement_plan_action_plan_task.status', '=', 'improvement_plan_action_plan_task_status.value');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_action_plan_task.customer_improvement_plan_action_plan_id") {
