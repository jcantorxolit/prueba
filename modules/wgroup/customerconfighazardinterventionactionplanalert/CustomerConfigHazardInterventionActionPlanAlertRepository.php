 ...
 * @author TeamCloud
 */
class CustomerConfigHazardInterventionActionPlanAlertRepository {
    ...
    public function __construct(CustomerConfigHazardInterventionActionPlanAlert $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_hazard_intervention_action_plan.status") {
