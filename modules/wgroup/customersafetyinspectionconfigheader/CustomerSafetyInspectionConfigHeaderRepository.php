 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionConfigHeaderRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_config_header.*');
    ...
    public function __construct(CustomerSafetyInspectionConfigHeader $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_config_header.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_safety_inspection_config_header.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_config_header.customer_id") {
    ...
    public function getFilterContract($filter = array(), $isCount = false, $join = "", $filterContractor = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_config_header.*']);
        ...
        if ($filterContractor) {
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_customer_safety_inspection_config_header.contractor_id") {
                        $query->where($field, "=", $valueField, 'and');
                    } else if ($field == "wg_customer_safety_inspection_config_header.customer_id") {
                        $query->where($field, "=", $valueField, 'and');
                    }
