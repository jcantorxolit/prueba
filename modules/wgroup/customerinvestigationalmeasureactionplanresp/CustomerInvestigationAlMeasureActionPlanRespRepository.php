 ...
 * @author TeamCloud
 */
class CustomerInvestigationAlMeasureActionPlanRespRepository {
    ...
    protected $columns = array('wg_customer_investigation_al_measure_action_plan_resp.*');
    ...
    public function __construct(CustomerInvestigationAlMeasureActionPlanResp $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_measure_action_plan_resp.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_measure_action_plan_resp.status") {
