 ...
 * @author TeamCloud
 */
class CustomerConfigActivityProcessRepository {
    ...
    protected $columns = array('wg_customer_config_activity_process.*');
    ...
    public function __construct(CustomerConfigActivityProcess $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_activity_process.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin("wg_customer_config_workplace", "wg_customer_config_workplace.id", "=", "wg_customer_config_activity_process.workplace_id")
            ->leftjoin("wg_customer_config_macro_process", "wg_customer_config_macro_process.id", "=", "wg_customer_config_activity_process.macro_process_id")
            ->leftjoin("wg_customer_config_process", "wg_customer_config_process.id", "=", "wg_customer_config_activity_process.process_id");

        foreach ($filter as $key => $item) {
            try {
                list($field, $valueField) = $item;

                if ($field == "wg_customer_config_activity_process.activity_id") {
