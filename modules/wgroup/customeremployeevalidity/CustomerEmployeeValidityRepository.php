...
/**
 * Description of CustomerEmployeeValidityRepository
 *
 * @author TeamCloud
 */
class CustomerEmployeeValidityRepository {
    ...
    protected $columns = array('wg_customer_employee_validity.*');
    ...
    public function __construct(CustomerEmployeeValidity $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_employee_validity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_employee_validity.*']);
        $query->join("wg_customer_employee", "wg_customer_employee.id", "=", "wg_customer_employee_validity.customer_employee_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_employee_validity.customer_employee_id") {
