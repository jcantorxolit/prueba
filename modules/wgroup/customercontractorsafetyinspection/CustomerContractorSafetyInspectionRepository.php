 ...
 * @author TeamCloud
 */
class CustomerContractorSafetyInspectionRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection.*');
    ...
    public function __construct(CustomerContractorSafetyInspection $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_safety_inspection.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection.customer_id") {
        ...
        if ($filterContractor) {
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_customer_safety_inspection.contractor_id") {
                        $query->where($field, "=", $valueField, 'and');
                    } else if ($field == "wg_customer_safety_inspection.customer_id") {
                        $query->where($field, "=", $valueField, 'and');
                    }
