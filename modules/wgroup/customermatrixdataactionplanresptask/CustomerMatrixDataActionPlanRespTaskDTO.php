 ...
 * @author jdblandon
 */
class CustomerMatrixDataActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo CustomerMatrixDataActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerMatrixDataActionPlanRespTask::find($object->id))) {
                // No existe
        ...
        return CustomerMatrixDataActionPlanRespTask::find($model->id);
    }
    ...
    public static function  fillAndUpdateModel($object)
    {
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new CustomerMatrixDataActionPlanRespTaskTracking();
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerMatrixDataActionPlanRespTask) {
                    $parsed[] = (new CustomerMatrixDataActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerMatrixDataActionPlanRespTask) {
            return (new CustomerMatrixDataActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerMatrixDataActionPlanRespTaskDTO();
            }
        }
    }
