...
/**
 * Description of CustomerInternalCertificateProgramRequirementRepository
 *
 * @author TeamCloud
 */
class CustomerInternalCertificateProgramRequirementRepository {
    ...
    protected $columns = array('wg_customer_internal_certificate_program_requirement.*');
    ...
    public function __construct(CustomerInternalCertificateProgramRequirement $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program_requirement.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program_requirement.*']);
        $query->join("wg_certificate_program", "wg_certificate_program.id", "=", "wg_customer_internal_certificate_program_requirement.certificate_program_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_certificate_program_requirement.certificate_program_id") {
