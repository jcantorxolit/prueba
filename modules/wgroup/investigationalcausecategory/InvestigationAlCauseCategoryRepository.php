 ...
 * @author TeamCloud
 */
class InvestigationAlCauseCategoryRepository {
    ...
    public function __construct(InvestigationAlCauseCategory $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_progam_prevention_category.status") {
