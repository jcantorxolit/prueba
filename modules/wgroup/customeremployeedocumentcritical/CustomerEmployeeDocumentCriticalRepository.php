...
/**
 * Description of CustomerEmployeeDocumentCriticalRepository
 *
 * @author TeamCloud
 */
class CustomerEmployeeDocumentCriticalRepository {
    ...
    protected $columns = array('wg_customer_employee_document_critical.*');
    ...
    public function __construct(CustomerEmployeeDocumentCritical $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_employee_document_critical.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_employee_document_critical.customer_employee_id") {
