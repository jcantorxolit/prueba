 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceDocumentRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_document.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceDocument $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_document.customer_health_damage_qualification_source_id") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_customer_health_damage_qs_document.entityCode") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_customer_health_damage_qs_document.entityId") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_health_damage_qs_document.customer_health_damage_qualification_source_id"
                        && $field != "wg_customer_health_damage_qs_document.entityCode"
                        && $field != "wg_customer_health_damage_qs_document.entityId"
                    ) {
