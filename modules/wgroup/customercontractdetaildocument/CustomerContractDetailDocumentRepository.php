...
/**
 * Description of CustomerContractDetailDocumentRepository
 *
 * @author TeamCloud
 */
class CustomerContractDetailDocumentRepository {
    ...
    protected $columns = array('wg_customer_contract_detail_document.*');
    ...
    public function __construct(CustomerContractDetailDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_contract_detail_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_contract_detail_document.customer_contract_detail_id") {
