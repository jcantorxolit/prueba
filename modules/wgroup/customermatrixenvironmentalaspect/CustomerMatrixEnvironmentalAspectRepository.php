 ...
 * @author TeamCloud
 */
class CustomerMatrixEnvironmentalAspectRepository
{
    ...
    protected $columns = array('wg_customer_matrix_environmental_aspect.*');
    ...
    public function __construct(CustomerMatrixEnvironmentalAspect $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_matrix_environmental_aspect.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_matrix", "wg_customer_matrix.id", "=", "wg_customer_matrix_environmental_aspect.customer_matrix_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix_environmental_aspect.customer_matrix_id") {
