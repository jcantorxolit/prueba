 ...
 * @author TeamCloud
 */
class CustomerInvestigationAlFactorRepository
{
    ...
    protected $columns = array('wg_customer_investigation_al_factor.*');
    ...
    public function __construct(CustomerInvestigationAlFactor $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_factor.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_investigation_al", function ($join) {
            $join->on('wg_customer_investigation_al_factor.customer_investigation_id', '=', 'wg_customer_investigation_al.id');
        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_factor')), function($join)
        {
            $join->on('wg_customer_investigation_al_factor.factor', '=', 'investigation_factor.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_factor.customer_investigation_id") {
