...
use Wgroup\CertificateGradeAgent\CertificateGradeAgentDTO;
use Wgroup\CertificateGradeCalendar\CertificateGradeCalendarDTO;
use Mail;
...
/**
 * Description of CertificateGradeDTO
 *
 * @author jdblandon
 */
class CertificateGradeDTO {
    ...
    /**
     * @param $model: Modelo CertificateGrade
     */
    private function getBasicInfo($model) {

        $this->id = $model->id;
        $this->program = CertificateProgramDTO::parse($model->program);
        ...
        $this->agents = CertificateGradeAgentDTO::parse($model->agents);
        $this->calendar = CertificateGradeCalendarDTO::parse($model->calendar);
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CertificateGrade::find($object->id))) {
                // No existe
        ...
        $model->certificate_program_id = $object->program->id;

        if ($object->code == "" || $object->code = 0) {
            $code = CertificateGrade::max(DB::raw("CAST(code AS UNSIGNED)"));
        ...
        if ($object->calendar) {
            ...
            CertificateGradeCalendarDTO::bulkInsert($object);
        }
        ...
        if ($object->agents) {
            ...
            CertificateGradeAgentDTO::bulkInsert($object);
        }


        return CertificateGrade::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CertificateGrade) {
                    $parsed[] = (new CertificateGradeDTO())->parseModel($model, $fmt_response);
                }else {
                    $parsed[] = (new CertificateGradeDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CertificateGrade) {
            return (new CertificateGradeDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CertificateGradeDTO();
            }
        }
    }
