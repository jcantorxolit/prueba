...
class CertificateGradeService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "") {
        ...
        $this->quoteRepository = new CertificateGradeRepository($model);
        ...
        // sorting
        $columns = [
            'wg_certificate_grade.id',
            'wg_certificate_grade.code',
            'wg_certificate_grade.name',
            'wg_certificate_grade.description',
            'wg_certificate_grade.status',
        ];
        ...
        if (empty($sorting)) {
            $this->quoteRepository->sortBy('wg_certificate_grade.id', 'desc');
        }

        $filters = array();

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_certificate_grade.status', $search);
            $filters[] = array('wg_certificate_grade.name', $search);
            $filters[] = array('wg_certificate_grade.code', $search);
            $filters[] = array('wg_certificate_grade.description', $search);
        }

        if ($typeFilter == "1") {
            $filters[] = array('wg_certificate_grade.status', '1');
        } else if ($typeFilter == "0") {
            $filters[] = array('wg_certificate_grade.status', '0');
        }


        $this->quoteRepository->setColumns(['wg_certificate_grade.*']);
    ...
    public function getAllByFilters($search, $perPage = 10, $currentPage = 0, $status = '', $location = '', $agentId = 0, $programId = '', $startDate = '', $endDate = '') {
...
, cg.created_at
FROM `wg_certificate_grade` cg
LEFT JOIN wg_certificate_program cp on cg.certificate_program_id = cp.id
...
LEFT JOIN (SELECT certificate_grade_id, count(*) as aggregate FROM `wg_certificate_grade_participant` group by certificate_grade_id) cgp on cgp.certificate_grade_id = cg.id ) p ";
        ...
        if ($agentId != '' && $agentId != '0') {
            $operator = ($where != '') ? "AND" : 'WHERE';
            $where .= " $operator p.id in (SELECT certificate_grade_id FROM `wg_certificate_grade_agent` WHERE `wg_certificate_grade_agent`.`agent_id` = '$agentId')";
        }
        ...
        if ($startDate != '' && $endDate != '') {
            $operator = ($where != '') ? "AND" : 'WHERE';
            $where .= " $operator p.id in (SELECT certificate_grade_id FROM `wg_certificate_grade_calendar` WHERE `wg_certificate_grade_calendar`.`startDate` between '$startDate' and '$endDate')";
        } else if ($startDate != '') {
            $operator = ($where != '') ? "AND" : 'WHERE';
            $where .= " $operator p.id in (SELECT certificate_grade_id FROM `wg_certificate_grade_calendar` WHERE `wg_certificate_grade_calendar`.`startDate` >= '$startDate')";
        } else if ($endDate != '') {
            $operator = ($where != '') ? "AND" : 'WHERE';
            $where .= " $operator p.id in (SELECT certificate_grade_id FROM `wg_certificate_grade_calendar` WHERE `wg_certificate_grade_calendar`.`startDate` <= '$startDate')";
        }
    ...
    public function getAllByFiltersCount($search, $status = '', $location = '', $agentId = 0, $programId = '', $startDate = '', $endDate = '') {
