 ...
 * @author TeamCloud
 */
class CustomerHealthDamageRestrictionRepository {
    ...
    protected $columns = array('wg_customer_health_damage_restriction.*');
    ...
    public function __construct(CustomerHealthDamageRestriction $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_restriction.*']);
