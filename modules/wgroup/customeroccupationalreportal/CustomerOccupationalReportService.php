...
class CustomerOccupationalReportService
{
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId = 0)
    {
        ...
        $this->customerAbsenteeismDisabilityRepository = new CustomerOccupationalReportRepository($model);
...
left join (select * from system_parameters where `group` = 'employee_contract_type') ct on ce.contractType COLLATE utf8_general_ci = ct.value";
    ...
    public function getOccupationalReportData($id)
    {
		...
		wg_investigation_economic_activity.`name` as economic_activity_customer,
		wg_investigation_economic_activity.code as economic_activity_customer_code,
		...
		wg_investigation_economic_activity_branch.`name` as economic_activity_branch,
		wg_investigation_economic_activity_branch.code as economic_activity_branch_code,
	...
	from wg_customer_occupational_report_al cora
	...
	left join wg_investigation_economic_activity ON cora.customer_economic_activity = wg_investigation_economic_activity.id
	left join wg_investigation_economic_activity wg_investigation_economic_activity_branch  ON cora.customer_branch_economic__activity = wg_investigation_economic_activity_branch.id
...
LEFT JOIN
  ( SELECT coral.lesion_id
   FROM wg_customer_occupational_report_al_lesion coral
   INNER JOIN wg_customer_occupational_report_al cora ON coral.customer_occupational_report_al_id = cora.id
...
LEFT JOIN
  ( SELECT corab.body_part_id
   FROM wg_customer_occupational_report_al_body corab
   INNER JOIN wg_customer_occupational_report_al cora ON corab.customer_occupational_report_al_id = cora.id
...
LEFT JOIN
  ( SELECT coraf.factor_id
   FROM wg_customer_occupational_report_al_factor coraf
   INNER JOIN wg_customer_occupational_report_al cora ON coraf.customer_occupational_report_al_id = cora.id
...
LEFT JOIN
  ( SELECT coram.mechanism_id
   FROM wg_customer_occupational_report_al_mechanism coram
   INNER JOIN wg_customer_occupational_report_al cora ON coram.customer_occupational_report_al_id = cora.id
...
from
wg_customer_occupational_report_al_witness w
INNER JOIN wg_customer_occupational_report_al cora ON w.customer_occupational_report_al_id = cora.id
where cora.id = :id";
...
FROM
	wg_customer_occupational_report_al o
...
from
	wg_customer_occupational_report_al o
inner join wg_customer_occupational_report_al_lesion p on o.id = p.customer_occupational_report_al_id
inner join ( SELECT *
   FROM system_parameters
...
from
	wg_customer_occupational_report_al o
inner join wg_customer_occupational_report_al_body p on o.id = p.customer_occupational_report_al_id
inner join ( SELECT *
   FROM system_parameters
...
from
	wg_customer_occupational_report_al o
inner join wg_customer_occupational_report_al_factor p on o.id = p.customer_occupational_report_al_id
inner join ( SELECT *
   FROM system_parameters
