...
use Wgroup\CustomerOccupationalReportAlWitness\CustomerOccupationalReportWitness;
 ...
 * Idea Model
 */
class CustomerOccupationalReport extends Model
{

    /**
     * @var string The database table used by the model.
     */
    public $table = 'wg_customer_occupational_report_al';
    ...
    public $belongsTo = [
        ...
        'jobModel' => ['Wgroup\CustomerConfigJobData\CustomerConfigJobData', 'key' => 'job', 'otherKey' => 'id'],
    ...
    public function  getCustomerEconomicActivity()
    {
        ...
        return InvestigationAlEconomicActivity::find($this->customer_economic_activity);
    }
    ...
    public function  getAccidentRegularWorkText()
    {
        ...
        return $respository->findOne($this->accident_regular_work_text);
    }
	...
	, IFNULL(coral.customer_occupational_report_al_id, 0) customerOccupationalReportAlId
	, lty.value itemId
...
LEFT JOIN
  ( SELECT coral.lesion_id, coral.id, coral.customer_occupational_report_al_id
   ...
   WHERE cora.id = :id) coral ON lty. VALUE = coral.lesion_id COLLATE utf8_general_ci";
	...
	, IFNULL(corab.customer_occupational_report_al_id, 0) customerOccupationalReportAlId
	, lty.value itemId
	, lty.item description
...
LEFT JOIN
  ( SELECT corab.body_part_id, corab.id, corab.customer_occupational_report_al_id
   ...
   WHERE cora.id = :id) corab ON lty. VALUE = corab.body_part_id COLLATE utf8_general_ci";
	...
	, IFNULL(coraf.customer_occupational_report_al_id, 0) customerOccupationalReportAlId
	, lty.value itemId
	, lty.item description
...
LEFT JOIN
  ( SELECT coraf.factor_id, coraf.id, coraf.customer_occupational_report_al_id
   ...
   WHERE cora.id = :id) coraf ON lty. VALUE = coraf.factor_id COLLATE utf8_general_ci";
	...
	, IFNULL(coram.customer_occupational_report_al_id, 0) customerOccupationalReportAlId
	, lty.value itemId
	, lty.item description
...
LEFT JOIN
  ( SELECT coram.mechanism_id, coram.id, coram.customer_occupational_report_al_id
   ...
   WHERE cora.id = :id) coram ON lty. VALUE = coram.mechanism_id COLLATE utf8_general_ci";
    ...
    {
        return CustomerOccupationalReportWitness::whereCustomerOccupationalReportAlId($this->id)->get();
    }
}
