...
class CustomerHealthDamageQualificationLostService
{
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $customerId = 0, $audit = null)
    {
        ...
        $this->customerAbsenteeismDisabilityRepository = new CustomerHealthDamageQualificationLostRepository($model);
        ...
        // sorting
        $columns = [
            'wg_customer_health_damage_ql.id',
            'wg_customer_health_damage_ql.examinationType',
            'wg_customer_health_damage_ql.medicalConcept',
        ...
        if (empty($sorting)) {
            $this->customerAbsenteeismDisabilityRepository->sortBy('wg_customer_health_damage_ql.id', 'desc');
        }
        ...
        /*if ($typeFilter == "1") {
            $filters[] = array('wg_customer_health_damage_ql.status', '1');
        } else if ($typeFilter == "0") {
            $filters[] = array('wg_customer_health_damage_ql.status', '0');
        }*/

        $this->customerAbsenteeismDisabilityRepository->setColumns(['wg_customer_health_damage_ql.*']);
    ...
    public function getAllByBilling($customerId = 0)
    {
        ...
        $query = "SELECT
	`wg_customer_health_damage_ql`.id,
	`wg_customer_health_damage_ql`.amountPaid,
	IFNULL(`wg_customer_health_damage_ql`.charged,0) charged,
	wg_employee.fullName,
	`start`,
	`end`,
	category,
	`wg_customer_health_damage_ql`.type,
	cause,
	'true' alive,
	numberDays
FROM
	`wg_customer_health_damage_ql`
INNER JOIN `wg_customer_employee` ON `wg_customer_health_damage_ql`.`customer_employee_id` = `wg_customer_employee`.`id`
...
) dtype ON `wg_customer_health_damage_ql`.`type` = `dtype`.`value`
INNER JOIN (
	SELECT
		*
	FROM
...
ORDER BY
	`wg_customer_health_damage_ql`.`start` DESC";
...
SELECT
	...
	`dtype`.item typeText,
	`wg_customer_health_damage_ql`.category,
	`absenteeism_disability_causes`.item causeItem,
	`absenteeism_disability_causes`.value causeValue,
	DATE_FORMAT(`wg_customer_health_damage_ql`.`start`,'%d/%m/%Y') startDateFormat,
	DATE_FORMAT(`wg_customer_health_damage_ql`.`end`,'%d/%m/%Y') endDateFormat,
	`wg_customer_health_damage_ql`.`start`,
	`wg_customer_health_damage_ql`.`end`,
...
INNER JOIN `wg_customer_employee` ON `wg_customer_health_damage_ql`.`customer_employee_id` = `wg_customer_employee`.`id`
INNER JOIN `wg_employee` ON `wg_customer_employee`.`employee_id` = `wg_employee`.`id`
LEFT JOIN (select COUNT(*) qty, customer_disability_id from wg_customer_health_damage_ql_report_al group by customer_disability_id) dr
	ON `dr`.`customer_disability_id` = `wg_customer_health_damage_ql`.`id`
LEFT JOIN (select COUNT(*) qty, id, customer_disability_id from wg_customer_health_damage_ql_action_plan group by customer_disability_id) ap
	ON `ap`.`customer_disability_id` = `wg_customer_health_damage_ql`.`id`
LEFT JOIN (select COUNT(*) qty, customer_disability_id from wg_customer_health_damage_ql_document where type = 'INC' group by customer_disability_id) inc
	ON `inc`.`customer_disability_id` = `wg_customer_health_damage_ql`.`id`
LEFT JOIN (select COUNT(*) qty, customer_disability_id from wg_customer_health_damage_ql_document where type = 'INV' group by customer_disability_id) inv
	ON `inv`.`customer_disability_id` = `wg_customer_health_damage_ql`.`id`
LEFT JOIN (
...
LEFT JOIN (
	...
	) absenteeism_disability_causes ON wg_customer_health_damage_ql.cause COLLATE utf8_general_ci = `absenteeism_disability_causes`.`value`
LEFT JOIN (
		SELECT * FROM system_parameters WHERE system_parameters.namespace = 'wgroup' AND system_parameters.`group` = 'employee_contract_type'
