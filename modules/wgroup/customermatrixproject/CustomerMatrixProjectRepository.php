 ...
 * @author TeamCloud
 */
class CustomerMatrixProjectRepository
{
    ...
    protected $columns = array('wg_customer_matrix_project.*');
    ...
    public function __construct(CustomerMatrixProject $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_matrix_project.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_matrix", "wg_customer_matrix.id", "=", "wg_customer_matrix_project.customer_matrix_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix_project.customer_matrix_id") {
