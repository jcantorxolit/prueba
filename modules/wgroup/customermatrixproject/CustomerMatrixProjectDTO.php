 ...
 * @author jdblandon
 */
class CustomerMatrixProjectDTO
{
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerMatrixProject::find($object->id))) {
                // No existe
        ...
        return CustomerMatrixProject::find($model->id);
    ...
    public static function parse($info, $fmt_response = "1")
    {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerMatrixProject) {
                    $parsed[] = (new CustomerMatrixProjectDTO())->parseModel($model, $fmt_response);
                } else {
                    $parsed[] = (new CustomerMatrixProjectDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerMatrixProject) {
            return (new CustomerMatrixProjectDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerMatrixProjectDTO();
            }
        }
    }
