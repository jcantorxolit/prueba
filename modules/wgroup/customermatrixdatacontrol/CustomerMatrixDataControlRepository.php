 ...
 * @author TeamCloud
 */
class CustomerMatrixDataControlRepository
{
    ...
    protected $columns = array('wg_customer_matrix_data_control.*');
    ...
    public function __construct(CustomerMatrixDataControl $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_matrix_data_control.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_matrix_data", "wg_customer_matrix_data.id", "=", "wg_customer_matrix_data_control.customer_matrix_data_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix_data_control.customer_matrix_data_id") {
