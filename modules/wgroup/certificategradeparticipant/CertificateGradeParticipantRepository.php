...
/**
 * Description of CertificateGradeParticipantRepository
 *
 * @author TeamCloud
 */
class CertificateGradeParticipantRepository {
    ...
    protected $columns = array('wg_certificate_grade_participant.*');
    ...
    public function __construct(CertificateGradeParticipant $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_grade_participant.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_certificate_grade_participant.*']);
        $query->join("wg_certificate_grade", "wg_certificate_grade.id", "=", "wg_certificate_grade_participant.certificate_grade_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_grade_participant.certificate_grade_id") {
