...
use Wgroup\CertificateGrade\CertificateGradeDTO;
use Wgroup\CertificateGrade\CertificateGradeService;
use Wgroup\CertificateGradeCalendar\CertificateGradeCalendar;
 ...
 * @author Andres Mejia
 */
class CertificateGradeController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        //set service
        $this->service = new CertificateGradeService();
    ...
    public function index()
    {
        ...
        try {
            ...
            $data = $this->service->getAllByFilters(@$search['value'], $length, $currentPage, $status, $location, $agentId, $program, $startDate, $endDate);

            // Counts
            $recordsTotal = $this->service->getAllByFiltersCount("", $status, $location, $agentId, $program, $startDate, $endDate);
            $recordsFiltered = $this->service->getAllByFiltersCount(@$search['value'], $status, $location, $agentId, $program, $startDate, $endDate);

            // extract info
            $result = $data; //CertificateGradeDTO::parse($data);
            //$result = $data;
    ...
    public function save()
    {
        ...
        try {
            ...
            $model = CertificateGradeDTO::fillAndSaveModel($info);

            $result = CertificateGradeDTO::parse($model);
    ...
    public function delete()
    {
        ...
        try {
            ...
            if (!($model = CertificateGrade::find($id))) {
    ...
    public function generateCertificate()
    {
        ...
        try {
            ...
            foreach ($model->getParticipants() as $participant) {
                ...
                if ($participant->isApproved == 1 && ($participant->hasCertificate == 0 || $participant->hasCertificate == 1)) {
                    $participant->hasCertificate = 1;
                    $grade = CertificateGradeDTO::parse($model);
                    ...
                    if (count($grade->agents) > 0) {
                        ...
                        $signature = CmsHelper::getUrlSite() . "/uploads/template/images/firma_gerente.png";
                        $signatureText = "";
                    }

                    $maxDate = CertificateGradeCalendar::where('certificate_grade_id', $grade->id)->max("startDate");
                    ...
                    $participant->certificateCreatedAt = $maxDate; //Carbon::now("America/Bogota");
                    $participant->save();

                    $participantName = $participant->name . " " . $participant->lastName;

                    $tokens = array(
                        "curso" => strtoupper($participant->grade->name), "horas" => $grade->program->hourDuration
                    );

                    $data = [
                        "participant" => $participantName,
                        "identification" => number_format((float)$participant->identificationNumber, 0, ',', '.'),
                        "captionHeader" => nl2br($participant->grade->program->captionHeader),
                        "captionFooter" =>  $this->replaceTokens($tokens, $participant->grade->program->captionFooter),
            ...
            $model->updateParticipantCertificateExpiration();
    ...
    public function export()
    {
        ...
        try {
            ...
            $model = CertificateGradeDTO::find($id);
