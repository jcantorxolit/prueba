...
use Wgroup\CustomerConfigActivityProcess\CustomerConfigActivityProcess;
use Wgroup\CustomerConfigActivityProcess\CustomerConfigActivityProcessDTO;
use Wgroup\CustomerConfigActivityProcess\CustomerConfigActivityProcessService;
...
use Carbon\Carbon;
use Wgroup\CustomerConfigActivityProcess\CustomerConfigActivityProcessStaging;
 ...
 * @author Andres Mejia
 */
class CustomerConfigActivityProcessController extends BaseController
{
    ...
    public function index()
    {
        ...
        $activityId = $this->request->get("activityId", "0");
        ...
        try {
            ...
            $data = $this->service->getAllBy(@$search['value'], $length, $currentPage, $orders, "", $activityId);

            // Counts
            $recordsTotal = $this->service->getCount("", $activityId);
            $recordsFiltered = $this->service->getCount(@$search['value'], $activityId);

            // extract info
            $result = CustomerConfigActivityProcessDTO::parse($data);
    ...
    public function listIndex()
    {
        ...
        try {
            ...
            $data = DB::table('wg_customer_config_activity')
                ->join('wg_customer_config_activity_process', 'wg_customer_config_activity_process.activity_id', '=', 'wg_customer_config_activity.id')
                ->where('workplace_id', $workPlaceId)
                ->where('macro_process_id', $macroProcessId)
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerConfigActivityProcessDTO::fillAndSaveModel($info);

            $result = CustomerConfigActivityProcessDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerConfigActivityProcessDTO();
            }
            else {
                if (!($model = CustomerConfigActivityProcess::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $jobId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($jobId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerConfigActivityProcessStaging::truncate();
                    CustomerConfigActivityProcessStaging::insert($data);
