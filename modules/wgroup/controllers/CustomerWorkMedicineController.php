...
use Wgroup\CustomerWorkMedicine\CustomerWorkMedicine;
use Wgroup\CustomerWorkMedicine\CustomerWorkMedicineDTO;
use Wgroup\CustomerWorkMedicine\CustomerWorkMedicineService;
 ...
 * @author Andres Mejia
 */
class CustomerWorkMedicineController extends BaseController {
    ...
    public function index()
    {
        ...
        try {
            ...
            **/
            ...
            $recordsTotal = $this->service->getCount("", $length, $currentPage, $customerId, null);
            $recordsFiltered = $this->service->getCount(@$search['value'], $length, $currentPage, $customerId, $audit);

            // extract info
            $result = CustomerWorkMedicineDTO::parse($data);
    ...
    public function employeeIndex()
    {
        ...
        try {
            ...
            // get all tracking by customer with pagination
            //$data = $this->service->getAll(@$search['value'], $length, $currentPage, $orders, "", $customerId);
            $data = $this->service->getAllByEmployee(@$search['value'], $length, $currentPage, $customerEmployeeId, $audit);

            // Counts
            //$recordsTotal = $this->service->getCount("", $customerId);
            //$recordsFiltered = $this->service->getCount(@$search['value'], $customerId);

            $recordsTotal = $this->service->getCountByEmployee("", $length, $currentPage, $customerEmployeeId, null);
            $recordsFiltered = $this->service->getCountByEmployee(@$search['value'], $length, $currentPage, $customerEmployeeId, $audit);
    ...
    public function save() {
        ...
        try {
            ...
            $model = CustomerWorkMedicineDTO::fillAndSaveModel($info);

            // Parse to send on response
            $result = CustomerWorkMedicineDTO::parse($model);
    ...
    public function delete()
    {
        ...
        try {
            ...
            if (!($model = CustomerWorkMedicine::find($id))) {
    ...
    public function upload() {
        ...
        try {
            ...
            $model = CustomerWorkMedicineDocumentDTO ::fillAndSaveModel($info);

            // Parse to send on response
            $result = CustomerWorkMedicineDocumentDTO::parse($model);
