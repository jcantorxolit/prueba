...
use Wgroup\CustomerMatrixActivity\CustomerMatrixActivityService;
use DB;
use Validator;
use Input;
use Excel;
...
use Carbon\Carbon;
use Wgroup\CustomerMatrixActivity\CustomerMatrixActivityStaging;
 ...
 * @author Andres Mejia
 */
class CustomerMatrixActivityController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        //set service
        $this->service = new CustomerMatrixActivityService();
    ...
    public function index()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerMatrixActivityDTO::parse($data);
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerMatrixActivityDTO::fillAndSaveModel($info);

            $result = CustomerMatrixActivityDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerMatrixActivityDTO();
            }
            else {
                if (!($model = CustomerMatrixActivity::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $customerId, $workPlaceId, $macroProcessId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($customerId, $workPlaceId, $macroProcessId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerMatrixActivityStaging::truncate();
                    CustomerMatrixActivityStaging::insert($data);
