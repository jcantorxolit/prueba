...
use Carbon\Carbon;
use Wgroup\CustomerConfigJobActivity\CustomerConfigJobActivityStaging;
 ...
 * @author Andres Mejia
 */
class CustomerConfigJobActivityController extends BaseController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $data = $this->service->getAllBy(@$search['value'], $length, $currentPage, $orders, "", $jobId);

            // Counts
            $recordsTotal = $this->service->getCount("", $jobId);
            $recordsFiltered = $this->service->getCount(@$search['value'], $jobId);

            // extract info
            $result = CustomerConfigJobActivityDTO::parse($data);
    ...
    public function listIndex()
    {
        ...
        try {
            ...
            $data = DB::table('wg_customer_config_job_activity')
                ->join('wg_customer_config_activity', 'wg_customer_config_activity.id', '=', 'wg_customer_config_job_activity.activity_id')
                ->select('wg_customer_config_job_activity.id', 'wg_customer_config_job_activity.job_id', 'wg_customer_config_job_activity.activity_id', 'wg_customer_config_activity.name')
                ->where('wg_customer_config_job_activity.job_id', $jobId)
                ->where('wg_customer_config_activity.status', 'Activo')
                ->get();
            //$data = CustomerConfigJobActivity::whereJobId($jobId)->whereStatus('Activo')->get();
    ...
    public function listBy()
    {
        ...
        try {
            ...
            // get all tracking by customer with pagination
            $data = $this->service->getAllListBy($customerId);
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerConfigJobActivityDTO::fillAndSaveModel($info);

            $result = CustomerConfigJobActivityDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerConfigJobActivityDTO();
            }
            else {
                if (!($model = CustomerConfigJobActivity::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $customerId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($customerId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerConfigJobActivityStaging::truncate();
                    CustomerConfigJobActivityStaging::insert($data);
