...
use Wgroup\CustomerConfigJobActivityDocument\CustomerConfigJobActivityDocument;
use Wgroup\CustomerConfigJobActivityDocument\CustomerConfigJobActivityDocumentDTO;
use Wgroup\CustomerConfigJobActivityDocument\CustomerConfigJobActivityDocumentService;
...
use Carbon\Carbon;
use Wgroup\CustomerConfigJobActivityDocument\CustomerConfigJobActivityDocumentStaging;
 ...
 * @author Andres Mejia
 */
class CustomerConfigJobActivityDocumentController extends BaseController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerConfigJobActivityDocumentDTO::parse($data);
    ...
    public function listIndex()
    {
        ...
        try {
            ...
            $data = CustomerConfigJobActivityDocument::whereJobId($jobId)->whereStatus('Activo')->get();
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            if (CustomerConfigJobActivityDocumentDTO::isUnique($info)) {
                $model = CustomerConfigJobActivityDocumentDTO::fillAndSaveModel($info);
            } else {
                throw new \Exception("El tipo de documento ya existe");
            }

            $result = CustomerConfigJobActivityDocumentDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerConfigJobActivityDocumentDTO();
            }
            else {
                if (!($model = CustomerConfigJobActivityDocument::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $jobId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($jobId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerConfigJobActivityDocumentStaging::truncate();
                    CustomerConfigJobActivityDocumentStaging::insert($data);
