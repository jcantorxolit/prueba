...
use Wgroup\CustomerMatrix\CustomerMatrix;
use Wgroup\CustomerMatrix\CustomerMatrixDTO;
use Wgroup\CustomerMatrix\CustomerMatrixService;
use DB;
use Validator;
use Input;
use Excel;
...
use Carbon\Carbon;
use Wgroup\CustomerMatrix\CustomerMatrixStaging;
 ...
 * @author Andres Mejia
 */
class CustomerMatrixController extends BaseController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerMatrixDTO::parse($data);
    ...
    public function getChartList()
    {
        ...
        try {
            $data = $this->service->getSummary($customerMatrixId);
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerMatrixDTO::fillAndSaveModel($info);

            $result = CustomerMatrixDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerMatrixDTO();
            }
            else {
                if (!($model = CustomerMatrix::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $customerId, $workPlaceId, $macroProcessId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($customerId, $workPlaceId, $macroProcessId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerMatrixStaging::truncate();
                    CustomerMatrixStaging::insert($data);
