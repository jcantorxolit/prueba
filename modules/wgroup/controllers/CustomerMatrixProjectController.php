...
use Wgroup\CustomerMatrixProject\CustomerMatrixProjectService;
use DB;
use Validator;
use Input;
use Excel;
...
use Carbon\Carbon;
use Wgroup\CustomerMatrixProject\CustomerMatrixProjectStaging;
 ...
 * @author Andres Mejia
 */
class CustomerMatrixProjectController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        //set service
        $this->service = new CustomerMatrixProjectService();
    ...
    public function index()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerMatrixProjectDTO::parse($data);
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerMatrixProjectDTO::fillAndSaveModel($info);

            $result = CustomerMatrixProjectDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerMatrixProjectDTO();
            }
            else {
                if (!($model = CustomerMatrixProject::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $customerId, $workPlaceId, $macroProcessId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($customerId, $workPlaceId, $macroProcessId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerMatrixProjectStaging::truncate();
                    CustomerMatrixProjectStaging::insert($data);
