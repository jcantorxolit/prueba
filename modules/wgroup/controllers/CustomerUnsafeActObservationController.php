...
use Wgroup\CustomerUnsafeActObservation\CustomerUnsafeActObservation;
use Wgroup\CustomerUnsafeActObservation\CustomerUnsafeActObservationDTO;
use Wgroup\CustomerUnsafeActObservation\CustomerUnsafeActObservationService;
use Carbon\Carbon;
 ...
 * @author Andres Mejia
 */
class CustomerUnsafeActObservationController extends BaseController
{
    ...
    public function index()
    {
        ...
        $customerUnsafeActId = $this->request->get("customerUnsafeActId", "0");
        ...
        try {
            ...
            $data = $this->service->getAllBy(@$search['value'], $length, $currentPage, $orders, $customerUnsafeActId);

            // Counts
            $recordsTotal = $this->service->getCount("", $customerUnsafeActId);
            $recordsFiltered = $this->service->getCount(@$search['value'], $customerUnsafeActId);

            // extract info
            $result = CustomerUnsafeActObservationDTO::parse($data);
    ...
    public function save()
    {
        ...
        try {
            ...
            $model = CustomerUnsafeActObservationDTO::fillAndSaveModel($info);

            // Parse to send on response
            $result = CustomerUnsafeActObservationDTO::parse($model);
    ...
    public function upload()
    {
        ...
        try {
            ...
            $model = CustomerUnsafeActObservation::find($id);
            ...
            $this->response->setResult(\AdeN\Api\Helpers\FileSystemHelper::attachInstance($model->image));
            //here code.
        } catch (Exception $exc) {
    ...
    public function get()
    {
        ...
        try {
            ...
            if (!($model = CustomerUnsafeActObservation::find($id))) {
