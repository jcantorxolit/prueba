...
use Wgroup\CustomerConfigActivity\CustomerConfigActivityService;
...
use Carbon\Carbon;
use Wgroup\CustomerConfigActivity\CustomerConfigActivityStaging;
 ...
 * @author Andres Mejia
 */
class CustomerConfigActivityController extends BaseController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerConfigActivityDTO::parse($data);
            //$result = array();
    ...
    public function listIndex()
    {
        ...
        try {
            ...
            $data = CustomerConfigActivity::whereJobId($jobId)->whereStatus('Activo')->get();
    ...
    public function save()
    {
        ...
        try {
            ...
            //Get data
            $model = CustomerConfigActivityDTO::fillAndSaveModel($info);

            $result = CustomerConfigActivityDTO::parse($model);
    ...
    public function get()
    {
        ...
        try {
            ...
            if ($id == "0") {
                $result = new CustomerConfigActivityDTO();
            }
            else {
                if (!($model = CustomerConfigActivity::find($id))) {
    ...
    protected function checkImportPostBack($uploadedFile, $jobId)
    {
        ...
        try {
            ...
            Excel::load($uploadedFile, function ($file) use ($jobId)
            {
                ...
                if (count($data) > 0) {
                    // var_dump($data);
                    CustomerConfigActivityStaging::truncate();
                    CustomerConfigActivityStaging::insert($data);
                    DB::statement('CALL TL_Activity()');
                }
            });
