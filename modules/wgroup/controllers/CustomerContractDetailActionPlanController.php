...
use Wgroup\CustomerContractDetailActionPlan\CustomerContractDetailActionPlanService;
use Wgroup\CustomerContractDetailActionPlan\CustomerContractDetailActionPlanActionPlan;
use Wgroup\CustomerContractDetailActionPlan\CustomerContractDetailActionPlanActionPlanDTO;
 ...
 */
class CustomerContractDetailActionPlanController extends BaseController {
    ...
    public function index($programId = 0, $managementId = 0){
        ...
        try {
            ...
            $data["questions"] = CustomerContractDetailActionPlanDTO::parse($questions);

           // $modelCategories = ProgramPreventionCategoryDTO::parse($categories);
            //$modelQuestions = CustomerContractDetailActionPlanDTO::parse($questions);
...
*/
    ...
    public function getActionPlan()
    {
        ...
        try {
            ...
            */
            ...
            if (!($model = CustomerContractDetailActionPlanActionPlan::find($id))) {
                throw new \Exception("Customer not found", 404);
            }


            //Get data
            $result = CustomerContractDetailActionPlanActionPlanDTO::parse($model);
        ...
        foreach($dtos as $program){
            ...
            foreach($questions as $question){
                if($question->program_id == $program->id){
                    $question->plan = new CustomerContractDetailActionPlanActionPlanDTO();
                    if (($mdlActionPlan = CustomerContractDetailActionPlanActionPlan::find($question->actionPlanId))) {
                        $question->plan = CustomerContractDetailActionPlanActionPlanDTO::parse($mdlActionPlan);
    ...
    public function get() {
        ...
        try {
            ...
            if(!($model = CustomerContractDetailActionPlanDTO::find($id))){
