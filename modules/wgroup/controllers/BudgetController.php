...
use Wgroup\Budget\BudgetService;
...
class BudgetController extends BaseController {
    ...
    public function index()
    {
        ...
        try {
            ...
            $data = $this->service->getAll(@$search['value'], $length, $currentPage, $audit);

            $recordsTotal = $this->service->getAllCountBy(@$search['value'], null);
            $recordsFiltered = $this->service->getAllCountBy(@$search['value'], $audit);
    ...
    public function getBudgets()
    {
        ...
        $classification = $this->request->get("classification", "0");

        try {
            $data = Budget::whereClassification($classification)->get();
    ...
    public function save() {
        ...
        try {
            ...
            $model = BudgetDTO::fillAndSaveModel($info);

            // Parse to send on response
            $result = BudgetDTO::parse($model);
        ...
        try {
            ...
            if (isset($info->disabilities)) {
                foreach ($info->disabilities as $disability) {
                    $model = BudgetDTO::fillAndUpdateModel($disability);
                }
            }
    ...
    public function upload() {
        ...
        try {
            ...
            $model = BudgetDocumentDTO ::fillAndSaveModel($info);

            // Parse to send on response
            $result = BudgetDocumentDTO::parse($model);
