...
use Wgroup\CustomerContractDetailDocument\CustomerContractDetailDocument;
use Wgroup\CustomerContractDetailDocument\CustomerContractDetailDocumentDTO;
use Wgroup\CustomerContractDetailDocument\CustomerContractDetailDocumentService;
 ...
 * @author Andres Mejia
 */
class CustomerContractDetailDocumentController extends BaseController
{
    ...
    public function index()
    {
        ...
        $contractorDetailId = $this->request->get("contractor_detail_id", "0");
        ...
        try {
            ...
            $data = $this->service->getAllByPermission(@$search['value'], $length, $currentPage, $orders, "", $contractorDetailId);

            //$data = array();

            //Log::info("Despues de");
            // Counts
            $recordsTotal = $this->service->getCount("", $contractorDetailId);
            //$recordsFiltered = $this->service->getCount(@$search['value'], $customerId);

            // extract info
            $result = CustomerContractDetailDocumentDTO::parse($data, "2");
    ...
    public function users()
    {
        ...
        try {
            ...
            // extract info
            $result = CustomerContractDetailDocumentDTO::parse($data, "3");
    ...
    public function save()
    {
        ...
        try {
            ...
            $model = CustomerContractDetailDocumentDTO::fillAndSaveModel($info);

            // Parse to send on response
            $result = CustomerContractDetailDocumentDTO::parse($model);
    ...
    public function upload()
    {
        ...
        try {
            ...
            $model = CustomerContractDetailDocument::find($document);
    ...
    public function get()
    {
        ...
        try {
            ...
            if (!($model = CustomerContractDetailDocument::find($id))) {
    ...
    public function delete()
    {
        ...
        try {
            ...
            if (!($model = AgentDocument::find($customer))) {
                throw new Exception("Record not found to delete.");
            }

            $model->status = 2;
            $model->updatedBy = $this->user ? $this->user->id : null;
            $model->save();
