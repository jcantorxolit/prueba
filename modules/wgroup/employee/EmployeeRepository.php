...
class EmployeeRepository {
    ...
    protected $columns = array('wg_employee.*');
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        /*
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'agent_type') agentType "), function($join)
        {
            $join->on('wg_employee.type', '=', 'agentType.value');
        ...
        {
            $join->on('wg_employee.documentType COLLATE utf8_general_ci', '=', 'tipod.value');
        });
        */
