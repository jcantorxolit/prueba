 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceJusticeRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_labor_justice.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceJustice $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_labor_justice.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_qs", function ($join) {
            $join->on('wg_customer_health_damage_qs_labor_justice.customer_health_damage_qualification_source_id', '=', 'wg_customer_health_damage_qs.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_labor_justice.customer_health_damage_qualification_source_id") {
