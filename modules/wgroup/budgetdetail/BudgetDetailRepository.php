...
class BudgetDetailRepository
{
    ...
    protected $columns = array('wg_budget_detail.*');
    ...
    public function __construct(BudgetDetail $model)
    {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_budget", function ($join) {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_budget_detail.budget_id") {
