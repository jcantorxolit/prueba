...
class BudgetDetailService
{
     ...
     * @return mixed
     */
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $budgetId = 0)
    {
        ...
        $this->customerContractorRepository = new BudgetDetailRepository($model);
        ...
        $columns = [
            'wg_budget_detail.id',
            'wg_budget_detail.period',
            'wg_budget_detail.year',
            'wg_budget_detail.month',
            'wg_budget_detail.amount',
        ];
        ...
        if (empty($sorting)) {
            $this->customerContractorRepository->sortBy('wg_budget_detail.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_budget_detail.budget_id', $budgetId);

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_budget_detail.period', $search);
            $filters[] = array('wg_budget_detail.year', $search);
            $filters[] = array('wg_budget_detail.month', $search);
            $filters[] = array('wg_budget_detail.amount', $search);
        }

        $this->customerContractorRepository->setColumns(['wg_budget_detail.*']);

        return $this->customerContractorRepository->getFilteredsOptional($filters, false, "");
    }

    public function getCount($search = "", $budgetId)
    {
