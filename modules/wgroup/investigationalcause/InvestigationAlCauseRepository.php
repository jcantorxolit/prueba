 ...
 * @author TeamCloud
 */
class InvestigationAlCauseRepository
{
    ...
    protected $columns = array('wg_progam_prevention_category.*');
    ...
    public function __construct(InvestigationAlCause $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_progam_prevention_category.*']);
    ...
    {
        $query = $this->query();
        $query->select(['wg_investigation_cause.*']);


        $query->join("wg_investigation_cause_category", function ($join) {
            $join->on('wg_investigation_cause.cause_category_id', '=', 'wg_investigation_cause_category.id');
        });

        $query->leftjoin(DB::raw("(SELECT * FROM wg_investigation_cause) wg_investigation_cause_parent "), function ($join) {
            $join->on('wg_investigation_cause.parent_id', '=', 'wg_investigation_cause_parent.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_investigation_cause_category.abbreviation") {
