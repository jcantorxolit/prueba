 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostOpportunityDiagnosticRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_first_opportunity_detail.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostOpportunityDiagnostic $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_first_opportunity_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_ql_first_opportunity", function ($join) {
            $join->on('wg_customer_health_damage_ql_first_opportunity_detail.customer_health_damage_ql_first_opportunity_id', '=', 'wg_customer_health_damage_ql_first_opportunity.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_first_opportunity_detail.customer_health_damage_ql_first_opportunity_id") {
