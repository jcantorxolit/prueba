 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostDocumentRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_document.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostDocument $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        })->leftjoin(DB::raw(SystemParameter::getRelationTable('work_health_damage_qs_document_type')), function ($join) {
            $join->on('wg_customer_health_damage_ql_document.type', '=', 'work_health_damage_qs_document_type.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_document.customer_health_damage_qualification_lost_id") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_customer_health_damage_ql_document.entityCode") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_customer_health_damage_ql_document.entityId") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_health_damage_ql_document.customer_health_damage_qualification_lost_id"
                        && $field != "wg_customer_health_damage_ql_document.entityCode"
                        && $field != "wg_customer_health_damage_ql_document.entityId"
                    ) {
