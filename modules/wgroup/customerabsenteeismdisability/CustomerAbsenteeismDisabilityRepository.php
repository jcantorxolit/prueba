 ...
 * @author TeamCloud
 */
class CustomerAbsenteeismDisabilityRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_disability.*');
    ...
    public function __construct(CustomerAbsenteeismDisability $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_disability.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'absenteeism_disability_type') dtype "), function($join)
        {
            $join->on('wg_customer_absenteeism_disability.type', '=', 'dtype.value');
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'employee_contract_type') ctype "), function($join)
        {
