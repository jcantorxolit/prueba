 ...
 * @author TeamCloud
 */
class CustomerContractDetailActionPlanAlertRepository {
    ...
    protected $columns = array('wg_customer_contract_detail_action_plan_alert.*');
    ...
    public function __construct(CustomerContractDetailActionPlanAlert $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_contract_detail_action_plan_alert.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_contract_detail_action_plan_alert.status") {
