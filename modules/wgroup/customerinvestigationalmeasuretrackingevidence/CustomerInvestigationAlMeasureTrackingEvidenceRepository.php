...
/**
 * Description of CustomerInvestigationAlMeasureTrackingEvidenceRepository
 *
 * @author TeamCloud
 */
class CustomerInvestigationAlMeasureTrackingEvidenceRepository
{
    ...
    protected $columns = array('wg_customer_investigation_al_measure_tracking_evidence.*');
    ...
    public function __construct(CustomerInvestigationAlMeasureTrackingEvidence $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_measure_tracking_evidence.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_investigation_al_measure_tracking", function ($join) {
            $join->on('wg_customer_investigation_al_measure_tracking_evidence.customer_investigation_measure_tracking_id', '=', 'wg_customer_investigation_al_measure_tracking.id');

        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_measure_tracking_evidence_type')), function ($join) {
            $join->on('wg_customer_investigation_al_measure_tracking_evidence.type', '=', 'investigation_measure_tracking_evidence_type.value');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_measure_tracking_evidence.customer_investigation_measure_tracking_id") {
