 ...
 * @author TeamCloud
 */
class CustomerOccupationalInvestigationAlCauseRepository
{
    ...
    protected $columns = array('wg_customer_occupational_investigation_al_cause.*');
    ...
    public function __construct(CustomerOccupationalInvestigationAlCause $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_occupational_investigation_al_cause.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_occupational_investigation_al", function ($join) {
            $join->on('wg_customer_occupational_investigation_al_cause.customer_occupational_investigation_id', '=', 'wg_customer_occupational_investigation_al.id');
        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_cause_classification_immediate')), function($join)
        {
            $join->on('wg_customer_occupational_investigation_al_cause.factor', '=', 'investigation_cause_classification_immediate.value');
        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_cause_classification_basic')), function($join)
        {
            $join->on('wg_customer_occupational_investigation_al_cause.factor', '=', 'investigation_cause_classification_basic.value');
        });

        $query->leftjoin("wg_investigation_cause", function($join)
        {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_occupational_investigation_al_cause.customer_occupational_investigation_id" ||
                    $field == "wg_customer_occupational_investigation_al_cause.type"
                ) {
