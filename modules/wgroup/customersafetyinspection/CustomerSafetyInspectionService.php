...
class CustomerSafetyInspectionService
{
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId = 0)
    {
        ...
        $this->customerContractorRepository = new CustomerSafetyInspectionRepository($model);
...
FROM
    ...
    public function insertHeaderFields($safetyInspectionId, $userId)
    {
        $query = "INSERT INTO `wg_customer_safety_inspection_header_field`
SELECT NULL id,
		csi.id customer_safety_inspection_id,
		csichf.id customer_safety_inspection_config_header_field_id,
		NULL varcharValue,
		NULL numericValue,
		NULL dateValue,
		1 isActive,
		...
		NOW() created_at,
		NULL updated_at
FROM wg_customer_safety_inspection csi
INNER JOIN wg_customer_safety_inspection_config_header csich ON csi.customer_safety_inspection_header_id = csich.id
INNER JOIN wg_customer_safety_inspection_config_header_field  csichf ON csich.id = csichf.customer_safety_inspection_config_header_id
LEFT JOIN wg_customer_safety_inspection_header_field csih  ON csih.customer_safety_inspection_id = csi.id AND csichf.id = csih.customer_safety_inspection_config_header_field_id
WHERE csi.id = :safety_inspection_id AND csich.isActive = 1 AND csichf.isActive = 1 AND csih.id IS NULL";


        DB::statement($query, array(
            'safety_inspection_id' => $safetyInspectionId
        ));
    }

    public function insertListItem($safetyInspectionId, $userId)
    {
        $query = "INSERT INTO `wg_customer_safety_inspection_list_item`
SELECT
	NULL id,
...
INNER JOIN wg_customer_safety_inspection_config_list_group csiclg on csiclg.customer_safety_inspection_config_list_id = csicl.id
INNER JOIN wg_customer_safety_inspection_config_list_item csicli on csicli.customer_safety_inspection_config_list_group_id = csiclg.id
LEFT JOIN wg_customer_safety_inspection_list_item csili on csili.customer_safety_inspection_list_id = csil.id and csili.customer_safety_inspection_config_list_item_id = csicli.id
WHERE csi.id = :safety_inspection_id and csicl.isActive = 1 and csiclg.isActive and csicli.isActive and csili.id is null ";
