 ...
 * @author TeamCloud
 */
class CustomerMatrixRepository {
    ...
    protected $columns = array('wg_customer_matrix.*');
    ...
    public function __construct(CustomerMatrix $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_matrix.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('config_matrix_status')), function($join)
            ...
            })->leftjoin(DB::raw(SystemParameter::getRelationTable('wg_matrix_type')), function($join)
            {
                $join->on('wg_customer_matrix.type', '=', 'wg_matrix_type.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix.customer_id") {
