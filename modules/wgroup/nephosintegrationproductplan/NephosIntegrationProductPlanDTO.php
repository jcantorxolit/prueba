 ...
 * @author jdblandon
 */
class NephosIntegrationProductPlanDTO
{
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = NephosIntegrationProductPlan::find($object->id))) {
                // No existe
                $model = new NephosIntegrationProductPlan();
                $isEdit = false;
            }
        } else {
            $model = new NephosIntegrationProductPlan();
        ...
        return NephosIntegrationProductPlan::find($model->id);
    ...
    {
        $instance = new NephosIntegrationProductPlanDTO();
    ...
    public static function parse($info, $fmt_response = "1")
    {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof NephosIntegrationProductPlan) {
                    $parsed[] = (new NephosIntegrationProductPlanDTO())->parseModel($model, $fmt_response);
                } else {
                    $parsed[] = (new NephosIntegrationProductPlanDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof NephosIntegrationProductPlan) {
            return (new NephosIntegrationProductPlanDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new NephosIntegrationProductPlanDTO();
            }
        }
    }
