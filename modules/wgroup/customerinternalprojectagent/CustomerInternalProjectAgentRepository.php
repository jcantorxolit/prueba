 ...
 * @author TeamCloud
 */
class CustomerInternalProjectAgentRepository {
    ...
    protected $columns = array('wg_customer_internal_project_user.*');
    ...
    public function __construct(CustomerInternalProjectAgent $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_project_user.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_project_user.status") {
