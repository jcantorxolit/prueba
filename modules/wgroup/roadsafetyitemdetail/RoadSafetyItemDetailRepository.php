 ...
 * @author TeamCloud
 */
class RoadSafetyItemDetailRepository
{
    ...
    public function __construct(RoadSafetyItemDetail $model)
    {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_road_safety_item", function ($join) {
            $join->on('wg_road_safety_item_detail.road_safety_item_id', '=', 'wg_road_safety_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_road_safety_item_detail.road_safety_item_id") {
