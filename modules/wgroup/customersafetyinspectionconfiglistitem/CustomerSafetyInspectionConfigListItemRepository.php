 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionConfigListItemRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_config_list_item.*');
    ...
    public function __construct(CustomerSafetyInspectionConfigListItem $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_config_list_item.*']);
