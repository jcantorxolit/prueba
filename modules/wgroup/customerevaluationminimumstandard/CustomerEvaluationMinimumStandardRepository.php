 ...
 * @author TeamCloud
 */
class CustomerEvaluationMinimumStandardRepository {
    ...
    protected $columns = array('wg_customer_evaluation_minimum_standard.*');
    ...
    public function __construct(CustomerEvaluationMinimumStandard $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_evaluation_minimum_standard.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_evaluation_minimum_standard.customer_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_evaluation_minimum_standard.customer_id") {
