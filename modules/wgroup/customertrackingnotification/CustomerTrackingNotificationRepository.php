 ...
 * @author TeamCloud
 */
class CustomerTrackingNotificationRepository {
    ...
    protected $columns = array('wg_customer_tracking_notification.*');
    ...
    public function __construct(CustomerTrackingNotification $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_tracking_notification.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_tracking_notification.status") {
