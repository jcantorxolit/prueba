 ...
 * @author TeamCloud
 */
class CustomerAbsenteeismIndicatorRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_indicator.*');
    ...
    public function __construct(CustomerAbsenteeismIndicator $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_indicator.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        //$query->join("wg_agent", "wg_agent.id", "=", "wg_customer_absenteeism_indicator.agent_id");


        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'absenteeism_indicator_period') dtype "), function($join)
        {
            $join->on('wg_customer_absenteeism_indicator.period', '=', 'dtype.value');
        })->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'absenteeism_disability_classification') ctype "), function($join)
        {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_absenteeism_indicator.customer_id") {
