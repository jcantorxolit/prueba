 ...
 * @author TeamCloud
 */
class CustomerConfigJobRepository {
    ...
    protected $columns = array('wg_customer_config_job.*');
    ...
    public function __construct(CustomerConfigJob $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_job.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin("wg_customer_config_workplace", "wg_customer_config_workplace.id", "=", "wg_customer_config_job.workplace_id")
            ->leftjoin("wg_customer_config_macro_process", "wg_customer_config_macro_process.id", "=", "wg_customer_config_job.macro_process_id")
            ->leftjoin("wg_customer_config_process", "wg_customer_config_process.id", "=", "wg_customer_config_job.process_id")
            ->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function($join)
            {
                $join->on('wg_customer_config_job.status', '=', 'config_workplace_status.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job.job_id" || $field == "wg_customer_config_job.customer_id") {
