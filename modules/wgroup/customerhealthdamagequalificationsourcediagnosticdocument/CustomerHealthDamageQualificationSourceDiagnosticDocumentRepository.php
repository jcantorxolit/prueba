 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceDiagnosticDocumentRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_diagnostic_document.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceDiagnosticDocument $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_diagnostic_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_qs_diagnostic", function ($join) {
            $join->on('wg_customer_health_damage_qs_diagnostic_document.customer_health_damage_qualification_source_diagnostic_id', '=', 'wg_customer_health_damage_qs_diagnostic.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_diagnostic_document.customer_health_damage_qualification_source_diagnostic_id") {
