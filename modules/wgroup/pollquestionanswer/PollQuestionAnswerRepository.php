...
/**
 * Description of PollQuestionAnswerRepository
 *
 * @author TeamCloud
 */
class PollQuestionAnswerRepository {
    ...
    protected $columns = array('wg_poll_question_answer.*');
    ...
    public function __construct(PollQuestionAnswer $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_poll_question_answer.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_poll_question_answer.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_poll_question_answer.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_poll_question_answer.type', '=', 'diags.value');
            });*/
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_poll_question_answer.poll_question_id") {
