...
/**
 * Description of CertificateProgramSpecialityRepository
 *
 * @author TeamCloud
 */
class CertificateProgramSpecialityRepository {
    ...
    protected $columns = array('wg_certificate_program_speciality.*');
    ...
    public function __construct(CertificateProgramSpeciality $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_program_speciality.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_certificate_program_speciality.*']);
        $query->join("wg_certificate_program", "wg_certificate_program.id", "=", "wg_certificate_program_speciality.certificate_program_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_program_speciality.certificate_program_id") {
