 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanCauseRepository
{
     ...
     * @var array
     */
    protected $currentSort = [array('wg_customer_improvement_plan_cause.id', 'desc')];
    protected $columns = array(
        'wg_customer_improvement_plan_cause.*'
    );
    ...
    public function __construct(CustomerImprovementPlanCause $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_cause.*']);
    ...
    {
        $query = $this->query();
        //$query->select(['wg_customer_improvement_plan_cause.*', 'wg_customer_improvement_plan_cause_root_cause.cause']);
        $query->select([
            'wg_customer_improvement_plan_cause.id',
            DB::raw('wg_improvement_plan_cause_category.name AS cause'),
            DB::raw('wg_customer_improvement_plan_cause_root_cause.id AS rootCauseId'),
            DB::raw('wg_customer_improvement_plan_cause_root_cause.cause AS rootCause'),
            'users.name',
            'wg_customer_improvement_plan_cause_root_cause.factor',
            DB::raw("DATE_FORMAT(wg_customer_improvement_plan_cause.created_at,'%d/%m/%Y') as causeCreatedAt"),
            DB::raw("DATE_FORMAT(wg_customer_improvement_plan_cause_root_cause.created_at,'%d/%m/%Y') as rootCreatedAt"),
        ]);


        $query->join("wg_customer_improvement_plan", function ($join) {
            $join->on('wg_customer_improvement_plan_cause.customer_improvement_plan_id', '=', 'wg_customer_improvement_plan.id');

        })->join("wg_improvement_plan_cause_category", function ($join) {
            $join->on('wg_customer_improvement_plan_cause.cause', '=', 'wg_improvement_plan_cause_category.id');

        })->leftjoin("wg_customer_improvement_plan_cause_root_cause", function ($join) {
            $join->on('wg_customer_improvement_plan_cause.id', '=', 'wg_customer_improvement_plan_cause_root_cause.customer_improvement_plan_cause_id');

        })->leftjoin("users", function ($join) {
            $join->on('wg_customer_improvement_plan_cause.createdBy', '=', 'users.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_cause.customer_improvement_plan_id") {
