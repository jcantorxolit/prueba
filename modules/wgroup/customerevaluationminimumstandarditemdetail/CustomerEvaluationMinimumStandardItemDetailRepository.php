 ...
 * @author TeamCloud
 */
class CustomerEvaluationMinimumStandardItemDetailRepository
{
    ...
    protected $columns = array('wg_customer_evaluation_minimum_standard_item_detail.*');
    ...
    public function __construct(CustomerEvaluationMinimumStandardItemDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_evaluation_minimum_standard_item_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_evaluation_minimum_standard_item_detail.status") {
