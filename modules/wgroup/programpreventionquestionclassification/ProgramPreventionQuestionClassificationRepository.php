 ...
 * @author TeamCloud
 */
class ProgramPreventionQuestionClassificationRepository {
    ...
    protected $columns = array('wg_progam_prevention_question_classification.*');
    ...
    public function __construct(ProgramPreventionQuestionClassification $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_progam_prevention_question_classification.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join(DB::raw(SystemParameter::getRelationTable('wg_customer_size')), function($join)
        {
            $join->on('wg_progam_prevention_question_classification.customer_size', '=', 'wg_customer_size.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_progam_prevention_question_classification.program_prevention_question_id") {
