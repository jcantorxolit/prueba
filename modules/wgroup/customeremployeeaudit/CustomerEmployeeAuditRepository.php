 ...
 * @author TeamCloud
 */
class CustomerEmployeeAuditRepository {
    ...
    protected $columns = array('wg_customer_employee_audit.*');
    ...
    public function __construct(CustomerEmployeeAudit $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_employee_audit.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_employee_audit.customer_employee_id") {
