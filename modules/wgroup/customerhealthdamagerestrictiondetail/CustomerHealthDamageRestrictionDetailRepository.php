 ...
 * @author TeamCloud
 */
class CustomerHealthDamageRestrictionDetailRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_restriction_detail.*');
    ...
    public function __construct(CustomerHealthDamageRestrictionDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_restriction_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_restriction_detail.customer_health_damage_restriction_id") {
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_health_damage_restriction_detail.customer_health_damage_restriction_id") {
                        if ($field == 'wg_customer_health_damage_restriction_detail.isPermanentManagement') {
