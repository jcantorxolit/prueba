 ...
 * @author TeamCloud
 */
class CustomerWorkMedicineSveRepository
{
    ...
    protected $columns = array('wg_customer_work_medicine_sve.*');
    ...
    public function __construct(CustomerWorkMedicineSve $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_work_medicine_sve.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_work_medicine", function ($join) {
            $join->on('wg_customer_work_medicine_sve.customer_work_medicine_id', '=', 'wg_customer_work_medicine.id');
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_medicine_sve_type') work_medicine_sve_type "), function($join)
        {
            $join->on('wg_customer_work_medicine_sve.type', '=', 'work_medicine_sve_type.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_work_medicine_sve.customer_work_medicine_id") {
