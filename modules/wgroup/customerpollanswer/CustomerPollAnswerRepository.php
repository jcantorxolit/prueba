...
/**
 * Description of CustomerPollAnswerRepository
 *
 * @author TeamCloud
 */
class CustomerPollAnswerRepository {
    ...
    protected $columns = array('wg_customer_poll_answer.*');
    ...
    public function __construct(CustomerPollAnswer $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_poll_answer.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_poll_answer.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_customer_poll_answer.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_customer_poll_answer.type', '=', 'diags.value');
            });*/
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_poll_answer.customer_poll_id") {
