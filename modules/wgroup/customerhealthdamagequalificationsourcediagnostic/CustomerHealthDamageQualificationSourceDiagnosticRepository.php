 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceDiagnosticRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_diagnostic.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceDiagnostic $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_diagnostic.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_qs", function ($join) {
            $join->on('wg_customer_health_damage_qs_diagnostic.customer_health_damage_qualification_source_id', '=', 'wg_customer_health_damage_qs.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_diagnostic.customer_health_damage_qualification_source_id") {
