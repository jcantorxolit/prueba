 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionConfigHeaderFieldRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_header_field.*');
    ...
    public function __construct(CustomerSafetyInspectionConfigHeaderField $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_header_field.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_header_field.customer_safety_inspection_config_header_id") {
