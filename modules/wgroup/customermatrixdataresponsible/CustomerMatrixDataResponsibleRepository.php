 ...
 * @author TeamCloud
 */
class CustomerMatrixDataResponsibleRepository
{
    ...
    protected $columns = array('wg_customer_matrix_data_responsible.*');
    ...
    public function __construct(CustomerMatrixDataResponsible $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_matrix_data_responsible.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_matrix_data", "wg_customer_matrix_data.id", "=", "wg_customer_matrix_data_responsible.customer_matrix_data_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_matrix_data_responsible.customer_matrix_data_id") {
