 ...
 * @author TeamCloud
 */
class CustomerDiagnosticPreventionDocumentQuestionRepository {
    ...
    protected $columns = array('wg_customer_diagnostic_prevention_document_question.*');
    ...
    public function __construct(CustomerDiagnosticPreventionDocumentQuestion $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_prevention_document_question.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id") {
