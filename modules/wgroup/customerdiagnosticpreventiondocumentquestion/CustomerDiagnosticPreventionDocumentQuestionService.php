...
class CustomerDiagnosticPreventionDocumentQuestionService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId = 0) {
        ...
        $this->customerContractorRepository = new CustomerDiagnosticPreventionDocumentQuestionRepository($model);
        ...
        // sorting
        $columns = [
            'wg_customer_diagnostic_prevention_document_question.id',
            'wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id',
            'wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id',
        ];
        ...
        if (empty($sorting)) {
            $this->customerContractorRepository->sortBy('wg_customer_diagnostic_prevention_document_question.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id', $customerId);

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_customer_diagnostic_prevention_document_question.id', $search);
            $filters[] = array('wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id', $search);
            $filters[] = array('wg_customer_diagnostic_prevention_document_question.customer_diagnostic_prevention_document_id', $search);
        }

        $this->customerContractorRepository->setColumns(['wg_customer_diagnostic_prevention_document_question.*']);
    ...
    left join (
        select * from wg_customer_diagnostic_prevention_document_question where customer_diagnostic_prevention_document_id = :customer_diagnostic_prevention_document_id
