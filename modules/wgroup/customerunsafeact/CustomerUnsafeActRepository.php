...
/**
 * Description of CustomerUnsafeActRepository
 *
 * @author TeamCloud
 */
class CustomerUnsafeActRepository
{
    ...
    protected $columns = array('wg_customer_unsafe_act.*');
    ...
    public function __construct(CustomerUnsafeAct $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_unsafe_act.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_config_workplace", function ($join) {
            $join->on('wg_customer_unsafe_act.work_place', '=', 'wg_customer_config_workplace.id');
        })->leftjoin("wg_config_job_activity_hazard_classification", function ($join) {
            $join->on('wg_customer_unsafe_act.risk_type', '=', 'wg_config_job_activity_hazard_classification.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_unsafe_act.customer_d") {
