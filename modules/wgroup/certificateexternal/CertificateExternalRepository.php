...
/**
 * Description of CertificateExternalRepository
 *
 * @author TeamCloud
 */
class CertificateExternalRepository {
    ...
    protected $columns = array('wg_certificate_external.*');
    ...
    public function __construct(CertificateExternal $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_external.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_external.customer_id") {
