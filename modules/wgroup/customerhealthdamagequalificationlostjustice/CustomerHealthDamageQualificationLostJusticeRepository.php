 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostJusticeRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_ordinary_labor_law.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostJustice $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_ordinary_labor_law.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_ql", function ($join) {
            $join->on('wg_customer_health_damage_ql_ordinary_labor_law.customer_health_damage_qualification_lost_id', '=', 'wg_customer_health_damage_ql.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_ordinary_labor_law.customer_health_damage_qualification_lost_id") {
