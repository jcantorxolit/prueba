 ...
 * @author jdblandon
 */
class CustomerOccupationalReportBodyDTO {
    ...
    public static function  bulkInsert($object, $entityId)
    {
        ...
        try {
            foreach ($object as $record) {
                ...
                if ($record) {
                    if ($record->id) {
                        if (!($model = CustomerOccupationalReportBody::find($record->id))) {
                            $isEdit = false;
                            $model = new CustomerOccupationalReportBody();
                        } else {
                            if (!$record->isActive) {
                                $model->delete();
                            }
        ...
        return CustomerOccupationalReportBody::find($model->id);
    }
