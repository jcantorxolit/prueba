...
/**
 * Description of CustomerInvestigationAlDocumentRepository
 *
 * @author TeamCloud
 */
class CustomerInvestigationAlDocumentRepository {
    ...
    protected $columns = array('wg_customer_investigation_al_document.*');
    ...
    public function __construct(CustomerInvestigationAlDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_customer_investigation_al", function ($join) {
            $join->on('wg_customer_investigation_al_document.customer_investigation_id', '=', 'wg_customer_investigation_al.id');
        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_document_type')), function($join)
        {
            $join->on('wg_customer_investigation_al_document.type', '=', 'investigation_document_type.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_document.customer_investigation_id" || $field == "investigation_document_type.value") {
