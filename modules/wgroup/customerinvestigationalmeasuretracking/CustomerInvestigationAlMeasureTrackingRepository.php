...
/**
 * Description of CustomerInvestigationAlMeasureTrackingRepository
 *
 * @author TeamCloud
 */
class CustomerInvestigationAlMeasureTrackingRepository
{
    ...
    protected $columns = array('wg_customer_investigation_al_measure_tracking.*');
    ...
    public function __construct(CustomerInvestigationAlMeasureTracking $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_measure_tracking.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_investigation_al_measure", function ($join) {
            $join->on('wg_customer_investigation_al_measure_tracking.customer_investigation_measure_id', '=', 'wg_customer_investigation_al_measure.id');

        })->join("wg_customer_investigation_al", function ($join) {
            $join->on('wg_customer_investigation_al_measure.customer_investigation_id', '=', 'wg_customer_investigation_al.id');

        });

        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_control_type')), function ($join) {
            $join->on('wg_customer_investigation_al_measure.controlType', '=', 'investigation_control_type.value');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('investigation_measure_tracking_status')), function ($join) {
            $join->on('wg_customer_investigation_al_measure_tracking.status', '=', 'investigation_measure_tracking_status.value');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_measure_tracking.customer_investigation_measure_id") {
    ...
    public function getFilteredOptionalInvestigation($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_measure.customer_investigation_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
