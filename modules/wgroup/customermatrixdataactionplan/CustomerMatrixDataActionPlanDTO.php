...
use Wgroup\CustomerMatrixDataActionPlanAlert\CustomerMatrixDataActionPlanAlertDTO;
use Wgroup\CustomerMatrixDataActionPlanResp\CustomerMatrixDataActionPlanResp;
use Wgroup\CustomerMatrixDataActionPlanResp\CustomerMatrixDataActionPlanRespDTO;
 ...
 * @author jdblandon
 */
class CustomerMatrixDataActionPlanDTO {
    ...
    /**
     * @param $model: Modelo CustomerMatrixDataActionPlan
     */
    private function getBasicInfo($model) {

        //Codigo
        $this->id = $model->id;
        $this->customerMatrixDataId = $model->customer_matrix_data_id;
        ...
        $this->alerts = CustomerMatrixDataActionPlanAlertDTO::parse($model->getAlerts());
        $this->responsibles = CustomerMatrixDataActionPlanRespDTO::parse($model->getResponsible());
    ...
    public static function  UpdateModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerMatrixDataActionPlan::find($object->id))) {
                // No existe
                $model = new CustomerMatrixDataActionPlan();
                $isEdit = false;
            }
        } else {
            $model = new CustomerMatrixDataActionPlan();
            ...
            foreach ($object->alerts as $actionPlanAlert) {
                if ($actionPlanAlert && isset($actionPlanAlert->type) && isset($actionPlanAlert->preference) && isset($actionPlanAlert->timeType)) {
                    ...
                    if ($actionPlanAlert->id) {
                        // Existe
                        if (!($customerManagementActionPlanAlert = CustomerMatrixDataActionPlanAlert::find($actionPlanAlert->id))) {
                            // No existe
                            $customerManagementActionPlanAlert = new CustomerMatrixDataActionPlanAlert();
                            $isAlertEdit = false;
                        }
                    } else {
                        $customerManagementActionPlanAlert = new CustomerMatrixDataActionPlanAlert();
                    ...
                    $customerManagementActionPlanAlert->customer_matrix_data_action_plan_id    = $model->id;
                ...
                if ($actionPlanResponsible && $actionPlanResponsible->contactId > 0) {
                    if ($actionPlanResponsible->isActive)
                    {
                        if ($actionPlanResponsible->id) {
                            // Existe
                            if (!($customerManagementActionPlanResp = CustomerMatrixDataActionPlanResp::find($actionPlanResponsible->id))) {
                                // No existe
                                $customerManagementActionPlanResp = new CustomerMatrixDataActionPlanResp();
                                $isAlertEdit = false;
                            }
                        } else {
                            $customerManagementActionPlanResp = new CustomerMatrixDataActionPlanResp();
                        ...
                        $customerManagementActionPlanResp->customer_matrix_data_action_plan_id    = $model->id;
...
*/
        ...
        return CustomerMatrixDataActionPlan::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerMatrixDataActionPlan) {
                    $parsed[] = (new CustomerMatrixDataActionPlanDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerMatrixDataActionPlan) {
            return (new CustomerMatrixDataActionPlanDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerMatrixDataActionPlanDTO();
            }
        }
    }
