 ...
 * @author TeamCloud
 */
class CustomerInvestigationAlMeasureActionPlanRespTaskRepository {
    ...
    protected $columns = array('wg_customer_investigation_al_measure_action_plan_resp_task.*');
    ...
    public function __construct(CustomerInvestigationAlMeasureActionPlanRespTask $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_measure_action_plan_resp_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_investigation_al_measure_action_plan_resp_task.agent_id");
        ...
        {
            $join->on('wg_customer_investigation_al_measure_action_plan_resp_task.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_measure_action_plan_resp_task.customer_id") {
