 ...
 * @author jdblandon
 */
class CustomerInvestigationAlMeasureActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo CustomerInvestigationAlMeasureActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerInvestigationAlMeasureActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new CustomerInvestigationAlMeasureActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new CustomerInvestigationAlMeasureActionPlanRespTask();
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new CustomerInvestigationAlMeasureActionPlanRespTaskTracking();
        ...
        return CustomerInvestigationAlMeasureActionPlanRespTask::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerInvestigationAlMeasureActionPlanRespTask) {
                    $parsed[] = (new CustomerInvestigationAlMeasureActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerInvestigationAlMeasureActionPlanRespTask) {
            return (new CustomerInvestigationAlMeasureActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerInvestigationAlMeasureActionPlanRespTaskDTO();
            }
        }
    }
