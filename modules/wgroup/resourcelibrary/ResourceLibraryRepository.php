...
/**
 * Description of ResourceLibraryRepository
 *
 * @author TeamCloud
 */
class ResourceLibraryRepository
{
    ...
    protected $currentSort = [
        array("wg_resource_library.name", "asc")
    ];
    //protected $currentSort = [array('id', 'desc')];
    protected $columns = array('wg_resource_library.*');
    ...
    public function __construct(ResourceLibrary $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_resource_library.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "", $audit = null)
    {
        $query = $this->query();
        $query->select(['wg_resource_library.*']);
        ...
        if ($audit == null) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_resource_library.isActive") {
            ...
            $query->where(function ($query) use ($audit) {
                foreach ($audit->filters as $filter) {
                    try {
                        if ($filter != null) {
                            switch ($filter->criteria->value) {
                                case "=":
                                    $query->where($filter->field->name, "=", $filter->value, 'or');
                                    break;

                                case "LIKE":
                                    $query->where($filter->field->name, "like", '%' . $filter->value . '%', 'or');
                                    break;
