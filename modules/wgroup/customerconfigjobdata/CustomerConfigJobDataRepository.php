 ...
 * @author TeamCloud
 */
class CustomerConfigJobDataRepository {
    ...
    protected $columns = array('wg_customer_config_job_data.*');
    ...
    public function __construct(CustomerConfigJobData $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_job_data.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function($join)
            {
                $join->on('wg_customer_config_job_data.status', '=', 'config_workplace_status.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job_data.customer_id") {
