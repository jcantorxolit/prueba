 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceOpportunityRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_qs_first_opportunity.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSourceOpportunity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs_first_opportunity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_qs", function ($join) {
            $join->on('wg_customer_health_damage_qs_first_opportunity.customer_health_damage_qualification_source_id', '=', 'wg_customer_health_damage_qs.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_qs_first_opportunity.customer_health_damage_qualification_source_id") {
