 ...
 * @author TeamCloud
 */
class CustomerContractDetailRepository {
    ...
    protected $columns = array('wg_customer_contract_detail.*');
    ...
    public function __construct(CustomerContractDetail $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_contract_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_contract_detail.status") {
