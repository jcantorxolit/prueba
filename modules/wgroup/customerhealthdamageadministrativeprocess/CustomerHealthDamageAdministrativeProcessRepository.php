 ...
 * @author TeamCloud
 */
class CustomerHealthDamageAdministrativeProcessRepository {
    ...
    protected $columns = array('wg_customer_health_damage_administrative_process.*');
    ...
    public function __construct(CustomerHealthDamageAdministrativeProcess $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_administrative_process.*']);
