 ...
 * @author TeamCloud
 */
class CustomerConfigJobActivityHazardTrackingRepository {
    ...
    protected $columns = array('wg_customer_config_job_activity_hazard_tracking.*');
    ...
    public function __construct(CustomerConfigJobActivityHazardTracking $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_job_activity_hazard_tracking.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job_activity_hazard_tracking.job_activity_hazard_id") {
