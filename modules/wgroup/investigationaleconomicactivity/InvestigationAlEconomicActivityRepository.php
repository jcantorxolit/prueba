 ...
 * @author TeamCloud
 */
class InvestigationAlEconomicActivityRepository
{
    ...
    protected $columns = array('wg_investigation_economic_activity.*');
    ...
    public function __construct(InvestigationAlEconomicActivity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_investigation_economic_activity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_investigation_economic_activity.abbreviation") {
