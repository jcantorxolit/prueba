 ...
 * @author TeamCloud
 */
class CustomerHealthDamageDiagnosticSourceDetailRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_diagnostic_source_detail.*');
    ...
    public function __construct(CustomerHealthDamageDiagnosticSourceDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_diagnostic_source_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_health_damage_presumed_diagnostic_source') work_health_damage_presumed_diagnostic_source "), function($join)
        {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_diagnostic_source_detail.customer_health_damage_diagnostic_source_id") {
    ...
    public function getFilterContract($filter = array(), $isCount = false, $join = "")
    {
        ...
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_health_damage_code_cie_10') work_health_damage_code_cie_10 "), function($join)
        {
            $join->on('wg_customer_health_damage_diagnostic_source_detail.codeCIE10', '=', 'work_health_damage_code_cie_10.value');
