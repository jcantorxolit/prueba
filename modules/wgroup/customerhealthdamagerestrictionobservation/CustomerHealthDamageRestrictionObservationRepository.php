 ...
 * @author TeamCloud
 */
class CustomerHealthDamageRestrictionObservationRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_restriction_observation.*');
    ...
    public function __construct(CustomerHealthDamageRestrictionObservation $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_restriction_observation.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_health_damage_restriction_observation_type') work_health_damage_restriction_observation_type "), function($join)
        {
            $join->on('wg_customer_health_damage_restriction_observation.type', '=', 'work_health_damage_restriction_observation_type.value');

        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_health_damage_restriction_observation_access_level') work_health_damage_restriction_observation_access_level "), function($join)
        {
            $join->on('wg_customer_health_damage_restriction_observation.accessLevel', '=', 'work_health_damage_restriction_observation_access_level.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_restriction_observation.customer_health_damage_restriction_id") {
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_health_damage_restriction_observation.customer_health_damage_restriction_id") {
                        if ($field == 'wg_customer_health_damage_restriction_observation.isPermanentManagement') {
                            if (strtoupper($valueField) == 'SI') {
                                $query->where($field, "like", '%' . 1 . '%', 'or');
                            } else  if (strtoupper($valueField) == 'NO') {
                                $query->where($field, "like", '%' . 0 . '%', 'or');
                            }
                        } else {
    ...
    public function getFilteredCustomer($filter = array(), $isCount = false, $join = "")
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_restriction_observation.*']);
