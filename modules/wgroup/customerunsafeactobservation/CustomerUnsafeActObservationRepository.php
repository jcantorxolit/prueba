...
/**
 * Description of CustomerUnsafeActObservationRepository
 *
 * @author TeamCloud
 */
class CustomerUnsafeActObservationRepository
{
    ...
    protected $columns = array('wg_customer_unsafe_act_observation.*');
    ...
    public function __construct(CustomerUnsafeActObservation $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_unsafe_act_observation.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('customer_unsafe_act_status')), function ($join) {
            $join->on('wg_customer_unsafe_act_observation.status', '=', 'customer_unsafe_act_status.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_unsafe_act_observation.customer_unsafe_act_id") {
