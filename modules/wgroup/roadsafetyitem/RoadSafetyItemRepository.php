 ...
 * @author TeamCloud
 */
class RoadSafetyItemRepository
{
    ...
    protected $currentSort = [
        ...
        array('wg_road_safety_item.numeral', 'asc'),
    ];

    protected $columns = array('wg_road_safety_item.*');
    ...
    public function __construct(RoadSafetyItem $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_road_safety_item.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_road_safety", function ($join) {
            $join->on('wg_road_safety_item.road_safety_id', '=', 'wg_road_safety.id');
