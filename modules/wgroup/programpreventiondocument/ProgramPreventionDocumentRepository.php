...
/**
 * Description of ProgramPreventionDocumentRepository
 *
 * @author TeamCloud
 */
class ProgramPreventionDocumentRepository {
    ...
    protected $columns = array('wg_progam_prevention_document.*');
    ...
    public function __construct(ProgramPreventionDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_progam_prevention_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_progam_prevention_document.customer_employee_id") {
