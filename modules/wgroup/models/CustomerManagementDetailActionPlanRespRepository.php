 ...
 * @author TeamCloud
 */
class CustomerManagementDetailActionPlanRespRepository {
    ...
    protected $columns = array('wg_customer_management_detail_action_plan_resp.*');
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_management_detail_action_plan_resp.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_management_detail_action_plan_resp.status") {
