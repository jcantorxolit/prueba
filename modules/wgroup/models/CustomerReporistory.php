...
class CustomerReporistory
{
     ...
     * @var array
     */
    protected $currentSort = [array('businessname', 'asc')];
    protected $columns = array('wg_customers.*');
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "", $agentJoin = false)
    {
        $query = $this->query();
        $query->select(['wg_customers.*']);


        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'tipocliente') tipoc "), function ($join) {
            $join->on('wg_customers.type', '=', 'tipoc.value');
        })->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'tipodoc') tipod "), function ($join) {
            $join->on('wg_customers.documentType', '=', 'tipod.value');
        });

        if ($agentJoin) {
            $query->join(DB::raw("(SELECT DISTINCT customer_id, agent_id FROM wg_customer_agent) agent "), function ($join) {
                $join->on('wg_customers.id', '=', 'agent.customer_id');
            });
        }
        ...
        if (!$agentJoin) {
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_customers.isDeleted") {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "agent.agent_id") {
                        $query->where($field, "=", $valueField, 'and');
                    } else if ($field == "wg_customers.isDeleted") {
                        $query->where($field, "=", $valueField, 'and');
                    }
            ...
            $query->where(function ($query) use ($filter) {
                ...
                foreach ($filter as $key => $item) {
                    try {
                        ...
                        if ($field != "agent.agent_id" && $field != "wg_customers.isDeleted") {
                            $query->where($field, "like", '%' . $valueField . '%', 'or');
                        }
    ...
    public function getFilteredsOptionalAgent($filter = array(), $isCount = false, $join = "", $agentJoin = false)
    {
        $query = $this->query();
        $query->select(['wg_customers.*']);
            ...
            $query->where(function ($query) use ($filter) {
                ...
                foreach ($filter as $key => $item) {
                    try {
                        ...
                        if ($field != "agent.agent_id" || "wg_customers.isDeleted") {
                            $query->where($field, "like", '%' . $valueField . '%', 'or');
                        }
