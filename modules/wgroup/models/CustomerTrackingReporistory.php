 ...
 * @author TeamCloud
 */
class CustomerTrackingReporistory {
    ...
    protected $columns = array('wg_customer_tracking.*');
    ...
    public function __construct(CustomerTracking $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_tracking.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_tracking.agent_id");
        ...
        {
            $join->on('wg_customer_tracking.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_tracking.customer_id") {
