...
class CustomerProjectRepository {
    ...
    protected $columns = array('wg_customer_project.*');
    ...
    public function __construct(CustomerProject $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_project.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_project.*']);
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_project.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_customer_project.type', '=', 'diags.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_project.customer_id") {
