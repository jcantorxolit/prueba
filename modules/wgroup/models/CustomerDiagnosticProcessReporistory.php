 ...
 * @author TeamCloud
 */
class CustomerDiagnosticProcessReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_process.*');
    ...
    public function __construct(CustomerDiagnosticProcess $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_process.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_process.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_process.status") {
