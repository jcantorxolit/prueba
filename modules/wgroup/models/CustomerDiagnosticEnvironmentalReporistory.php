 ...
 * @author TeamCloud
 */
class CustomerDiagnosticEnvironmentalReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_environmental_measures.*');
    ...
    public function __construct(CustomerDiagnosticEnvironmental $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_environmental_measures.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_environmental_measures.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_environmental_measures.status") {
