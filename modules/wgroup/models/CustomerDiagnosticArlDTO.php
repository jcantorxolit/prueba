 ...
 * @author jdblandon
 */
class CustomerDiagnosticArlDTO {
    ...
    private function getBasicInfo($model) {
        ...
        $this->arlProfessional = $model->getArlProfessional();
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerDiagnosticArl::find($object->id))) {
                // No existe
        ...
        $model->arlProfessional = $object->arlProfessional->value == "-S-" ? null : $object->arlProfessional->value;
        ...
        return CustomerDiagnosticArl::find($model->id);
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerDiagnosticArl) {
                    $parsed[] = (new CustomerDiagnosticArlDTO())->parseModel($model, $fmt_response);
                } else {
                    $parsed[] = (new CustomerDiagnosticArlDTO())->parseArray($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerDiagnosticArl) {
            return (new CustomerDiagnosticArlDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerDiagnosticArlDTO();
            }
        }
    }
