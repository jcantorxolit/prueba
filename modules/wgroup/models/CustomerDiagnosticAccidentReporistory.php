 ...
 * @author TeamCloud
 */
class CustomerDiagnosticAccidentReporistory {
    ...
    protected $columns = array('wg_customer_diagnostic_accident.*');
    ...
    public function __construct(CustomerDiagnosticAccident $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_accident.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_accident.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customer_diagnostic_accident.status") {
