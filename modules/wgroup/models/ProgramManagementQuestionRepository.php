...
/**
 * Description of ProgramManagementQuestionRepository
 *
 * @author TeamCloud
 */
class ProgramManagementQuestionRepository {
    ...
    protected $columns = array('wg_program_management_question.*');
    ...
    public function __construct(ProgramManagementQuestion $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_program_management_question.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_program_management_category", function ($join) {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_program_management_question.status") {
