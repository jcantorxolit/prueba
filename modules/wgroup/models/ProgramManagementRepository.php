...
/**
 * Description of ProgramManagementRepository
 *
 * @author TeamCloud
 */
class ProgramManagementRepository {
    ...
    protected $columns = array('wg_program_management.*');
    ...
    public function __construct(ProgramManagement $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_program_management.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_program_management.status") {
