 ...
 * @author TeamCloud
 */
class CustomerDiagnosticRiskFactorReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_risk_factor.*');
    ...
    public function __construct(CustomerDiagnosticRiskFactor $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_risk_factor.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'diagnostic_risk_factor') frisk "), function($join)
            {
                $join->on('wg_customers_diagnostic_risk_factor.risk', '=', 'frisk.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_risk_factor.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_risk_factor.status") {
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_risk_factor.diagnostic_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
