...
/**
 * Description of CustomerDiagnosticReporistory
 *
 * @author TeamCloud
 */
class CustomerDiagnosticWorkPlaceReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_workplace.*');
    ...
    public function __construct(CustomerDiagnosticWorkPlace $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_workplace.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("rainlab_user_countries", "rainlab_user_countries.id", "=", "wg_customers_diagnostic_workplace.country_id")
            ->join("rainlab_user_states", "rainlab_user_states.id", "=", "wg_customers_diagnostic_workplace.state_id")
            ->join("wg_towns", "wg_towns.id", "=", "wg_customers_diagnostic_workplace.city_id");
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_workplace.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_workplace.status") {
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_workplace.diagnostic_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
