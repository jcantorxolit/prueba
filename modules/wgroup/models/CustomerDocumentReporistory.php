 ...
 * @author TeamCloud
 */
class CustomerDocumentReporistory {
    ...
    protected $columns = array('wg_customer_document.*');
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_document.customer_id") {
