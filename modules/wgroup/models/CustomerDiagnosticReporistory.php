 ...
 * @author TeamCloud
 */
class CustomerDiagnosticReporistory {
    ...
    protected $columns = array('wg_customer_diagnostic.*');
    ...
    public function __construct(CustomerDiagnostic $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic.*']);
        $query->join("users", "users.id", "=", "wg_customer_diagnostic.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'diagnostic_status') diags "), function($join)
            {
                $join->on('wg_customer_diagnostic.status', '=', 'diags.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic.customer_id") {
