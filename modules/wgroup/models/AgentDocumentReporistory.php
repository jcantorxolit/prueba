 ...
 * @author TeamCloud
 */
class AgentDocumentReporistory {
    ...
    protected $columns = array('wg_agent_document.*');
    ...
    public function __construct(AgentDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_agent_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "", $hideCanceled) {
        $query = $this->query();
        $query->select(['wg_agent_document.*']);
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_agent_document.agent_id") {
                    $query->where($field, "=", $valueField, 'and');

                    if ($hideCanceled) {
                        $query->where("wg_agent_document.status", "<>", 2, 'and');
                    }
                }
