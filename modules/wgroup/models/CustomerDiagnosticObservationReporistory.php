 ...
 * @author TeamCloud
 */
class CustomerDiagnosticObservationReporistory {
    ...
    protected $columns = array('wg_customer_diagnostic_observation.*');
    ...
    public function __construct(CustomerDiagnosticObservation $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_observation.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_observation.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customer_diagnostic_observation.status") {
