...
class AgentRepository {
    ...
    protected $columns = array('wg_agent.*');
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin(DB::raw(SystemParameter::getRelationTable('agent_type')), function($join)
        {
            $join->on('wg_agent.type', '=', 'agent_type.value');
        });
    ...
    public function getFilteredOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_customer_agent", function($join)
        {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_agent.customer_id") {
