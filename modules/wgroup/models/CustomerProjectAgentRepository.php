 ...
 * @author TeamCloud
 */
class CustomerProjectAgentRepository {
    ...
    protected $columns = array('wg_customer_project_agent.*');
    ...
    public function __construct(CustomerProjectAgent $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_project_agent.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_project_agent.status") {
