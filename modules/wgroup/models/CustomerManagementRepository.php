...
class CustomerManagementRepository {
    ...
    protected $columns = array('wg_customer_management.*');
    ...
    public function __construct(CustomerManagement $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_management.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_management.*']);
        //$query->join("wg_agent", "wg_agent.id", "=", "wg_customer_management.createdBy")
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'management_status') diags "), function($join)
            {
                $join->on('wg_customer_management.status', '=', 'diags.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_management.customer_id") {
