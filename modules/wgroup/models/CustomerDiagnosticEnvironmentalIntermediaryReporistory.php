 ...
 * @author TeamCloud
 */
class CustomerDiagnosticEnvironmentalIntermediaryReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_environmental_measures_inter.*');
    ...
    public function __construct(CustomerDiagnosticEnvironmentalIntermediary $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_environmental_measures_inter.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_environmental_measures_inter.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_environmental_measures_inter.status") {
