...
/**
 * Description of ProgramManagementCategoryRepository
 *
 * @author TeamCloud
 */
class ProgramManagementCategoryRepository {
    ...
    protected $columns = array('wg_program_management_category.*');
    ...
    public function __construct(ProgramManagementCategory $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_program_management_category.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_program_management_category.status") {
