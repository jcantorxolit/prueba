 ...
 * @author TeamCloud
 */
class CustomerDiagnosticDiseaseReporistory {
    ...
    public function __construct(CustomerDiagnosticDisease $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_disease.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_disease.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customer_diagnostic_disease.status") {
