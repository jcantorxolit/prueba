 ...
 * @author TeamCloud
 */
class CustomerDiagnosticPreventionReporistory {
    ...
    protected $columns = array('wg_customer_diagnostic_prevention.*');
    ...
    public function __construct(CustomerDiagnosticPrevention $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_prevention.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_prevention.status") {
