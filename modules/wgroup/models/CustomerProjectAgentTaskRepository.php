 ...
 * @author TeamCloud
 */
class CustomerProjectAgentTaskRepository {
    ...
    protected $columns = array('wg_customer_project_agent_task.*');
    ...
    public function __construct(CustomerProjectAgentTask $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_project_agent_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_project_agent_task.agent_id");
        ...
        {
            $join->on('wg_customer_project_agent_task.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_project_agent_task.customer_id") {
