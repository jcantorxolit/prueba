 ...
 * @author TeamCloud
 */
class CustomerDiagnosticArlIntermediaryReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_arl_intermediary.*');
    ...
    public function __construct(CustomerDiagnosticArlIntermediary $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_arl_intermediary.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_arl_intermediary.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_arl_intermediary.status") {
