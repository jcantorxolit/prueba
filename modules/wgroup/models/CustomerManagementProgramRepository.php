...
/**
 * Description of CustomerManagementProgramRepository
 *
 * @author TeamCloud
 */
class CustomerManagementProgramRepository {
    ...
    protected $columns = array('wg_customer_management_program.*');
    ...
    public function __construct(CustomerManagementProgram $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_management_program.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_management_program.*']);
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_management_program.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'management_status') diags "), function($join)
            {
                $join->on('wg_customer_management_program.status', '=', 'diags.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_management_program.customer_id") {
