 ...
 * @author TeamCloud
 */
class CustomerDiagnosticRiskTaskReporistory {
    ...
    protected $columns = array('wg_customers_diagnostic_risk_task.*');
    ...
    public function __construct(CustomerDiagnosticRiskTask $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customers_diagnostic_risk_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'diagnostic_risk_task') ftask "), function($join)
        {
            $join->on('wg_customers_diagnostic_risk_task.task', '=', 'ftask.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_risk_task.id") {
                    $query->where($field, "=", $valueField , 'and');
                }
                else if ($field == "wg_customers_diagnostic_risk_task.status") {
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customers_diagnostic_risk_task.diagnostic_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
