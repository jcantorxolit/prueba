 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionListObservationRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_list_observation.*');
    ...
    public function __construct(CustomerSafetyInspectionListObservation $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_list_observation.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_customer_safety_inspection_config_list", function($join)
        {
            $join->on('wg_customer_safety_inspection_list_observation.customer_safety_inspection_list_id', '=', 'wg_customer_safety_inspection_config_list.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection.id") {
