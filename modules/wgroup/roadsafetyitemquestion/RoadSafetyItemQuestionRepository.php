 ...
 * @author TeamCloud
 */
class RoadSafetyItemQuestionRepository
{
    ...
    protected $columns = array('wg_road_safety_item_detail.*');
    ...
    public function __construct(RoadSafetyItemQuestion $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_road_safety_item_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_road_safety_item", function ($join) {
            $join->on('wg_road_safety_item_detail.wg_road_safety_item_id', '=', 'wg_road_safety_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_road_safety_item_detail.wg_road_safety_item_id") {
                    $query->where($field, "=", $valueField, 'and');
                }

                if ($field == "wg_road_safety_item_detail.type") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function ($query) use ($filter) {
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_road_safety_item_detail.wg_road_safety_item_id"
                        && $field != "wg_road_safety_item_detail.type"
                    ) {
