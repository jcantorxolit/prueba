 ...
 * @author TeamCloud
 */
class CustomerWorkMedicineTrackingRepository
{
    ...
    protected $columns = array('wg_customer_work_medicine_tracking.*');
    ...
    public function __construct(CustomerWorkMedicineTracking $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_work_medicine_tracking.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_work_medicine", function ($join) {
            $join->on('wg_customer_work_medicine_tracking.customer_work_medicine_id', '=', 'wg_customer_work_medicine.id');
        })->leftjoin(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'work_medicine_tracking_type') work_medicine_tracking_type "), function($join)
        {
            $join->on('wg_customer_work_medicine_tracking.type', '=', 'work_medicine_tracking_type.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_work_medicine_tracking.customer_work_medicine_id") {
