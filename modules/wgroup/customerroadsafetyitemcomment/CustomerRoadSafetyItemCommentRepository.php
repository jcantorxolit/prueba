 ...
 * @author TeamCloud
 */
class CustomerRoadSafetyItemCommentRepository
{
    ...
    protected $columns = array('wg_customer_road_safety_item_comment.*');
    ...
    public function __construct(CustomerRoadSafetyItemComment $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_road_safety_item_comment.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        {
            $join->on('wg_customer_road_safety_item_comment.createdBy', '=', 'users.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_road_safety_item_comment.customer_road_safety_item_id") {
