 ...
 * @author jdblandon
 */
class customerContractDetailActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo customerContractDetailActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = customerContractDetailActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new customerContractDetailActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new customerContractDetailActionPlanRespTask();
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new customerContractDetailActionPlanRespTaskTracking();
        ...
        return customerContractDetailActionPlanRespTask::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof customerContractDetailActionPlanRespTask) {
                    $parsed[] = (new customerContractDetailActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof customerContractDetailActionPlanRespTask) {
            return (new customerContractDetailActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new customerContractDetailActionPlanRespTaskDTO();
            }
        }
    }
