 ...
 * @author TeamCloud
 */
class CustomerContractorRepository {
    ...
    protected $columns = array('wg_customer_contractor.*');
    ...
    public function __construct(CustomerContractor $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_contractor.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        {
            $join->on('wg_customer_contractor.contractor_id', '=', 'wg_customers.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_contractor.customer_id") {
    ...
    public function getFilterContract($filter = array(), $isCount = false, $join = "", $filterContractor = false) {
        ...
        $query->join("wg_customers", function($join)
        {
            $join->on('wg_customer_contractor.customer_id', '=', 'wg_customers.id');
        });
        ...
        if ($filterContractor) {
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field == "wg_customer_contractor.contractor_id") {
                        $query->where($field, "=", $valueField, 'and');
                    } else if ($field == "wg_customer_contractor.customer_id") {
                        $query->where($field, "=", $valueField, 'and');
                    }
