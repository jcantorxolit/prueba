...
/**
 * Description of CustomerAbsenteeismDisabilityDocumentRepository
 *
 * @author TeamCloud
 */
class CustomerAbsenteeismDisabilityDocumentRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_disability_document.*');
    ...
    public function __construct(CustomerAbsenteeismDisabilityDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_disability_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_absenteeism_disability_document.customer_disability_id") {
