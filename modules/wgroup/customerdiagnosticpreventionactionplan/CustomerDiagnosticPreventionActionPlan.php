 ...
 * Idea Model
 */
class CustomerDiagnosticPreventionActionPlan extends Model
{

    /**
     * @var string The database table used by the model.
     */
    public $table = 'wg_customer_diagnostic_prevention_action_plan';
    ...
    public $hasMany = [
        'alerts' => ['Wgroup\CustomerDiagnosticPreventionActionPlan\CustomerDiagnosticPreventionActionPlanAlert', 'key' => 'action_plan_id', 'otherKey' => 'id'],
    ];

    public function getAlerts(){
        return CustomerDiagnosticPreventionActionPlanAlert::whereActionPlanId($this->id)->get();
    }
...
FROM
	wg_customers cs
INNER JOIN
	wg_customer_diagnostic c ON cs.id = c.customer_id
INNER JOIN
	wg_customer_diagnostic_prevention d ON c.id = d.diagnostic_id
INNER JOIN
	wg_customer_diagnostic_prevention_action_plan p ON d.id = p.diagnostic_detail_id
INNER JOIN (
		SELECT ct.* FROM wg_contact ct INNER JOIN wg_customers c ON c.id = ct.customer_id
	) ct ON ct.customer_id = cs.id
LEFT JOIN
	wg_customer_diagnostic_prevention_action_plan_resp r on p.id = r.action_plan_id and r.contact_id = ct.id
LEFT JOIN (
