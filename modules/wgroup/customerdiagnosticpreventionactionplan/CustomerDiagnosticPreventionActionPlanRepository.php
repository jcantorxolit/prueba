 ...
 * @author TeamCloud
 */
class CustomerDiagnosticPreventionActionPlanRepository {
    ...
    protected $columns = array('wg_customer_diagnostic_prevention_action_plan.*');
    ...
    public function __construct(CustomerDiagnosticPreventionActionPlan $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_prevention_action_plan.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_diagnostic_prevention_action_plan.agent_id");
        ...
        {
            $join->on('wg_customer_diagnostic_prevention_action_plan.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_prevention_action_plan.diagnostic_detail_id") {
