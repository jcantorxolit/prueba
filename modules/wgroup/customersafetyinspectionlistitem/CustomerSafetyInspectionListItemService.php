...
class CustomerSafetyInspectionListItemService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerSafetyInspectionConfigListId = 0) {
        ...
        $model = new CustomerSafetyInspectionListItem();
        ...
        $this->customerContractorRepository = new CustomerSafetyInspectionListItemRepository($model);
    ...
    public function getLists($safetyInspectionId)
    {
...
FROM
    ...
    public function getListGroups($safetyInspectionId)
    {
...
ORDER BY list.sort";
    ...
    public function getListItems($safetyInspectionId, $action = null)
    {
...
WHERE csi.id = :customer_safety_inspection_id and csicl.isActive = 1 and csiclg.isActive and csicli.isActive $andWhere";

        $results = DB::select( $query, array(
            'customer_safety_inspection_id' => $safetyInspectionId,
        ));

        return $results;
    }

    public function getHeaderFields($safetyInspectionId)
    {
        $query = "SELECT
	csihf.id,
	csihf.varcharValue,
...
WHERE csi.id = :customer_safety_inspection_id AND csihf.isActive = 1 AND csichf.isActive AND csich.isActive = 1
ORDER BY csichf.sort";
    ...
    public function getValidationList($safetyInspectionId)
    {
        $query = "SELECT
	csiclv_d.id,
