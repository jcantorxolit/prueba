 ...
 * @author TeamCloud
 */
class CustomerContractorSafetyInspectionListItemRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_list_item.*');
    ...
    public function __construct(CustomerContractorSafetyInspectionListItem $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_list_item.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_customer_safety_inspection_config_list_group", function($join)
        {
            $join->on('wg_customer_safety_inspection_list_item.customer_safety_inspection_config_list_group_id', '=', 'wg_customer_safety_inspection_config_list_group.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_config_list.id") {
