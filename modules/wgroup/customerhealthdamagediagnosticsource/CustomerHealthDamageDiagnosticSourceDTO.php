 ...
 * @author jdblandon
 */
class CustomerHealthDamageDiagnosticSourceDTO {
    ...
    /**
     * @param $model: Modelo CustomerHealthDamageDiagnosticSource
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerHealthDamageDiagnosticSource::find($object->id))) {
                // No existe
        ...
        return CustomerHealthDamageDiagnosticSource::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerHealthDamageDiagnosticSource) {
                    $parsed[] = (new CustomerHealthDamageDiagnosticSourceDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerHealthDamageDiagnosticSource) {
            return (new CustomerHealthDamageDiagnosticSourceDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerHealthDamageDiagnosticSourceDTO();
            }
        }
    }
