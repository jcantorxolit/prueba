 ...
 * @author TeamCloud
 */
class CustomerHealthDamageDiagnosticSourceRepository {
    ...
    protected $columns = array('wg_customer_health_damage_diagnostic_source.*');
    ...
    public function __construct(CustomerHealthDamageDiagnosticSource $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_diagnostic_source.*']);
