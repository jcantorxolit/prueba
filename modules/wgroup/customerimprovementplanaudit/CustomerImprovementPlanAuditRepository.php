 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanAuditRepository
{
    ...
    protected $columns = array('wg_customer_improvement_plan_audit.*');
    ...
    public function __construct(CustomerImprovementPlanAudit $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_audit.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_improvement_plan", function ($join) {
            $join->on('wg_customer_improvement_plan_audit.customer_improvement_plan_id', '=', 'wg_customer_improvement_plan.id');

        })->leftjoin("users", function ($join) {
            $join->on('wg_customer_improvement_plan_audit.createdBy', '=', 'users.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_audit.customer_improvement_plan_id") {
