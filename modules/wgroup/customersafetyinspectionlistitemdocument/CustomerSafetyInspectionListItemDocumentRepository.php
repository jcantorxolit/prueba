...
/**
 * Description of CustomerSafetyInspectionListItemDocumentRepository
 *
 * @author TeamCloud
 */
class CustomerSafetyInspectionListItemDocumentRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_list_item_document.*');
    ...
    public function __construct(CustomerSafetyInspectionListItemDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_list_item_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_list_item_document.customer_safety_inspection_list_item_id") {
