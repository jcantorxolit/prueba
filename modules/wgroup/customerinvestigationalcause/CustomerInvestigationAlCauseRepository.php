 ...
 * @author TeamCloud
 */
class CustomerInvestigationAlCauseRepository
{
    ...
    protected $columns = array('wg_customer_investigation_al_cause.*');
    ...
    public function __construct(CustomerInvestigationAlCause $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_investigation_al_cause.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_investigation_al", function ($join) {
            $join->on('wg_customer_investigation_al_cause.customer_investigation_id', '=', 'wg_customer_investigation_al.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_investigation_al_cause.customer_investigation_id") {
