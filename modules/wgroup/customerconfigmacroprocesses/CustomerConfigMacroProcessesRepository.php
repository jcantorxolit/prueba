 ...
 * @author TeamCloud
 */
class CustomerConfigMacroProcessesRepository
{
    ...
    protected $columns = array('wg_customer_config_macro_process.*');
    ...
    public function __construct(CustomerConfigMacroProcesses $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_config_macro_process.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_config_workplace", "wg_customer_config_workplace.id", "=", "wg_customer_config_macro_process.workplace_id")
            ->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function ($join) {
                $join->on('wg_customer_config_macro_process.status', '=', 'config_workplace_status.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_macro_process.customer_id") {
