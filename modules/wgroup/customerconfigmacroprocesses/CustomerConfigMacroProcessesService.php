...
class CustomerConfigMacroProcessesService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId) {
        ...
        $this->customerConfigWorkPlaceRepository = new CustomerConfigMacroProcessesRepository($model);
        ...
        // sorting
        $columns = [
            'wg_customer_config_macro_process.name',
            'wg_customer_config_macro_process.status'
        ];
        ...
        if (empty($sorting)) {
            $this->customerConfigWorkPlaceRepository->sortBy('wg_customer_config_macro_process.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_customer_config_macro_process.customer_id', $customerId);

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_customer_config_macro_process.name', $search);
            $filters[] = array('wg_customer_config_macro_process.status', $search);
            $filters[] = array('wg_customer_config_workplace.name', $search);
        }

        $this->customerConfigWorkPlaceRepository->setColumns(['wg_customer_config_macro_process.*']);
