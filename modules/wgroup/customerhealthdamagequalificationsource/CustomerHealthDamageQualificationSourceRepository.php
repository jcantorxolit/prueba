 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationSourceRepository {
    ...
    protected $columns = array('wg_customer_health_damage_qs.*');
    ...
    public function __construct(CustomerHealthDamageQualificationSource $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_qs.*']);
