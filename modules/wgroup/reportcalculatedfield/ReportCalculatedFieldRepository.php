...
/**
 * Description of ReportCalculatedFieldRepository
 *
 * @author TeamCloud
 */
class ReportCalculatedFieldRepository {
    ...
    protected $columns = array('wg_report_calculated_field.*');
    ...
    public function __construct(ReportCalculatedField $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_report_calculated_field.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_report_calculated_field.*']);
        /*$query->join("wg_agent", "wg_agent.id", "=", "wg_report_calculated_field.createdBy")
           ->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'project_type') diags "), function($join)
            {
                $join->on('wg_report_calculated_field.type', '=', 'diags.value');
            });*/
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_report_calculated_field.report_id") {
