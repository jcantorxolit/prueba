 ...
 * @author TeamCloud
 */
class CustomerImprovementPlanTrackingRepository
{
    ...
    protected $columns = array('wg_customer_improvement_plan_tracking.*');
    ...
    public function __construct(CustomerImprovementPlanTracking $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_improvement_plan_tracking.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join('wg_customer_improvement_plan', function ($join) {
            $join->on('wg_customer_improvement_plan_tracking.customer_improvement_plan_id', '=', 'wg_customer_improvement_plan.id');

        })->leftjoin(DB::raw(Customer::getRelatedAgentAndUser()), function ($join) {
            $join->on('wg_customer_improvement_plan_tracking.responsible', '=', 'responsible.id');
            $join->on('wg_customer_improvement_plan_tracking.responsibleType', '=', 'responsible.type');
            $join->on('wg_customer_improvement_plan.customer_id', '=', 'responsible.customer_id');

        })->leftjoin(DB::raw(SystemParameter::getRelationTable('improvement_plan_tracking_status')), function ($join) {
            $join->on('wg_customer_improvement_plan_tracking.status', '=', 'improvement_plan_tracking_status.value');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_improvement_plan_tracking.customer_improvement_plan_id") {
