 ...
 * @author TeamCloud
 */
class CustomerConfigMinimumStandardItemDetailRepository
{
    ...
    protected $columns = array('wg_customer_config_minimum_standard_item_detail.*');
    ...
    public function __construct(CustomerConfigMinimumStandardItemDetail $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_config_minimum_standard_item_detail.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_minimum_standard_item_detail", function ($join) {
            $join->on('wg_customer_config_minimum_standard_item_detail.minimum_standard_item_detail_id', '=', 'wg_customer_minimum_standard_item_detail.id');

        })->join("wg_minimum_standard_item", function ($join) {
            $join->on('wg_customer_minimum_standard_item_detail.wg_minimum_standard_id', '=', 'wg_minimum_standard_item.id');
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_minimum_standard_item_detail.customer_id") {
