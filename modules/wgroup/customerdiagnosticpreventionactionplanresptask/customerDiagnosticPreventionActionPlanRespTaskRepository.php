...
namespace Wgroup\customerDiagnosticPreventionActionPlanRespTask;
 ...
 * @author TeamCloud
 */
class customerDiagnosticPreventionActionPlanRespTaskRepository {
    ...
    protected $columns = array('wg_customer_internal_project_user_task.*');
    ...
    public function __construct(customerDiagnosticPreventionActionPlanRespTask $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_project_user_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_internal_project_user_task.agent_id");
        ...
        {
            $join->on('wg_customer_internal_project_user_task.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_project_user_task.customer_id") {
