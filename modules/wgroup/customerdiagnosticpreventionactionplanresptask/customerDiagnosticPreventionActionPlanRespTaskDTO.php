 ...
 * @author jdblandon
 */
class customerDiagnosticPreventionActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo customerDiagnosticPreventionActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = customerDiagnosticPreventionActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new customerDiagnosticPreventionActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new customerDiagnosticPreventionActionPlanRespTask();
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new customerDiagnosticPreventionActionPlanRespTaskTracking();
        ...
        return customerDiagnosticPreventionActionPlanRespTask::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof customerDiagnosticPreventionActionPlanRespTask) {
                    $parsed[] = (new customerDiagnosticPreventionActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof customerDiagnosticPreventionActionPlanRespTask) {
            return (new customerDiagnosticPreventionActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new customerDiagnosticPreventionActionPlanRespTaskDTO();
            }
        }
    }
