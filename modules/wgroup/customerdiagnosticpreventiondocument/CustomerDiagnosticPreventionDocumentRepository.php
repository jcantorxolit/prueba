...
/**
 * Description of CustomerDiagnosticPreventionDocumentRepository
 *
 * @author TeamCloud
 */
class CustomerDiagnosticPreventionDocumentRepository {
    ...
    protected $columns = array('wg_customer_diagnostic_prevention_document.*');
    ...
    public function __construct(CustomerDiagnosticPreventionDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_diagnostic_prevention_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_diagnostic_prevention_document.customer_employee_id") {
