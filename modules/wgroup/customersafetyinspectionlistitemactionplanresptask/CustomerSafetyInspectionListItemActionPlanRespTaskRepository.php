 ...
 * @author TeamCloud
 */
class CustomerSafetyInspectionListItemActionPlanRespTaskRepository {
    ...
    protected $columns = array('wg_customer_safety_inspection_list_item_action_plan_resp_task.*');
    ...
    public function __construct(CustomerSafetyInspectionListItemActionPlanRespTask $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_safety_inspection_list_item_action_plan_resp_task.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->join("wg_agent", "wg_agent.id", "=", "wg_customer_safety_inspection_list_item_action_plan_resp_task.agent_id");

        /*
        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'tracking_tiposeg') tipos "), function($join)
        {
            $join->on('wg_customer_safety_inspection_list_item_action_plan_resp_task.type', '=', 'tipos.value');
        ...
        */
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_safety_inspection_list_item_action_plan_resp_task.customer_id") {
