...
namespace Wgroup\CustomerInvestigationAlMeasureActionPlan;
...
use Wgroup\CustomerInvestigationAlMeasureActionPlanAlert\CustomerInvestigationAlMeasureActionPlanAlert;
use Wgroup\CustomerInvestigationAlMeasureActionPlanAlert\CustomerInvestigationAlMeasureActionPlanAlertDTO;
use Wgroup\CustomerInvestigationAlMeasureActionPlanResp\CustomerInvestigationAlMeasureActionPlanResp;
use Wgroup\CustomerInvestigationAlMeasureActionPlanResp\CustomerInvestigationAlMeasureActionPlanRespDTO;
 ...
 * @author jdblandon
 */
class CustomerInvestigationAlMeasureActionPlanDTO {
    ...
    /**
     * @param $model: Modelo CustomerInvestigationAlMeasureActionPlan
     */
    private function getBasicInfo($model) {
        ...
        $this->alerts = CustomerInvestigationAlMeasureActionPlanAlertDTO::parse($model->getAlerts());
        $this->responsibles = CustomerInvestigationAlMeasureActionPlanRespDTO::parse($model->getResponsible());
    ...
    public static function  UpdateModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerInvestigationAlMeasureActionPlan::find($object->id))) {
                // No existe
                $model = new CustomerInvestigationAlMeasureActionPlan();
                $isEdit = false;
            }
        } else {
            $model = new CustomerInvestigationAlMeasureActionPlan();
    ...
    public static function  fillAndSaveModel($object)
    {
            ...
            foreach ($object->alerts as $actionPlanAlert) {
                if ($actionPlanAlert->type != null) {
                    ...
                    if ($actionPlanAlert->id) {
                        // Existe
                        if (!($customerManagementActionPlanAlert = CustomerInvestigationAlMeasureActionPlanAlert::find($actionPlanAlert->id))) {
                            // No existe
                            $customerManagementActionPlanAlert = new CustomerInvestigationAlMeasureActionPlanAlert();
                            $isAlertEdit = false;
                        }
                    } else {
                        $customerManagementActionPlanAlert = new CustomerInvestigationAlMeasureActionPlanAlert();
                ...
                if ($actionPlanResponsible && $actionPlanResponsible->contactId > 0) {
                    if ($actionPlanResponsible->isActive)
                    {
                        if ($actionPlanResponsible->id) {
                            // Existe
                            if (!($customerManagementActionPlanResp = CustomerInvestigationAlMeasureActionPlanResp::find($actionPlanResponsible->id))) {
                                // No existe
                                $customerManagementActionPlanResp = new CustomerInvestigationAlMeasureActionPlanResp();
                                $isAlertEdit = false;
                            }
                        } else {
                            $customerManagementActionPlanResp = new CustomerInvestigationAlMeasureActionPlanResp();
                        ...
                        */
                        ...
                        try {
                            if ($actionPlanResponsible->email != "") {
                                if ($customerAbsenteeismDisability = CustomerAbsenteeismDisability::find($object->customerInvestigationMeasureId)) {
        ...
        return CustomerInvestigationAlMeasureActionPlan::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerInvestigationAlMeasureActionPlan) {
                    $parsed[] = (new CustomerInvestigationAlMeasureActionPlanDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerInvestigationAlMeasureActionPlan) {
            return (new CustomerInvestigationAlMeasureActionPlanDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerInvestigationAlMeasureActionPlanDTO();
            }
        }
    }
