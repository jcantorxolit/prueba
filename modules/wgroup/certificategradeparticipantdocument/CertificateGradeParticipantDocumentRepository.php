...
/**
 * Description of CertificateGradeParticipantDocumentRepository
 *
 * @author TeamCloud
 */
class CertificateGradeParticipantDocumentRepository {
    ...
    protected $columns = array('wg_certificate_grade_participant_document.*');
    ...
    public function __construct(CertificateGradeParticipantDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_grade_participant_document.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_grade_participant_document.certificate_grade_participant_id") {
