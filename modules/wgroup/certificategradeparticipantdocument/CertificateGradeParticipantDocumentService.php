...
class CertificateGradeParticipantDocumentService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $certificateGradeParticipantId = 0) {
        ...
        $this->customerDocumentRepository = new CertificateGradeParticipantDocumentRepository($model);
        ...
        // sorting
        $columns = [
            'wg_certificate_grade_participant_document.id',
            'wg_certificate_grade_participant_document.requirement',
            'wg_certificate_grade_participant_document.description',
            'wg_certificate_grade_participant_document.version',
            'wg_certificate_grade_participant_document.status'
        ];
        ...
        if (empty($sorting)) {
            $this->customerDocumentRepository->sortBy('wg_certificate_grade_participant_document.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_certificate_grade_participant_document.certificate_grade_participant_id', $certificateGradeParticipantId);

        if (strlen(trim($search)) > 0) {
            $filters[] = array('wg_certificate_grade_participant_document.requirement', $search);
            $filters[] = array('wg_certificate_grade_participant_document.description', $search);
            $filters[] = array('wg_certificate_grade_participant_document.version', $search);
            $filters[] = array('wg_certificate_grade_participant_document.status', $search);
        }

        if ($typeFilter == "1") {
            $filters[] = array('wg_certificate_grade_participant_document.status', '1');
        } else if ($typeFilter == "0") {
            $filters[] = array('wg_certificate_grade_participant_document.status', '0');
        }


        $this->customerDocumentRepository->setColumns(['wg_certificate_grade_participant_document.*']);
    ...
    public function getAllBySearch($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $certificateGradeParticipant = 0) {
        ...
        $query = "select DISTINCT d.id, d.certificate_grade_participant_id, p.item requeriment, d.description, d.version, '' agent
            , d.created_at
from wg_certificate_grade_participant_document d
