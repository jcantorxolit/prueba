...
class CustomerParameterRepository {
    ...
    protected $columns = array('wg_customer_parameter.*');
    ...
    public function __construct(CustomerParameter $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_parameter.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_parameter.customer_id") {
                    $query->where($field, "=", $valueField, 'and');
                } else if ($field == "wg_customer_parameter.group") {
                    $query->where($field, "=", $valueField, 'and');
                }
        ...
        $query->where(function($query) use ($filter){
            ...
            foreach ($filter as $key => $item) {
                try {
                    ...
                    if ($field != "wg_customer_parameter.customer_id" && $field != "wg_customer_parameter.group") {
