 ...
 * @author TeamCloud
 */
class CustomerEconomicGroupRepository {
    ...
    protected $columns = array('wg_customer_economic_group.*');
    ...
    public function __construct(CustomerEconomicGroup $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_economic_group.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_economic_group.parent_id") {
    ...
    public function getFilterContract($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_economic_group.contractor_id") {
                    $query->where($field, "=", $valueField, 'and');
                }
