...
class EmployeeChildrenRepository {
    ...
    protected $columns = array('wg_employee_children.*');
    ...
    public function __construct(EmployeeChildren $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_employee_children.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_employee_children.employee_id") {
