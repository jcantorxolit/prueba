 ...
 * @author TeamCloud
 */
class CustomerRoadSafetyItemRepository
{
    ...
    protected $columns = array('wg_customer_road_safety_item.*');
    ...
    public function __construct(CustomerRoadSafetyItem $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_road_safety_item.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_road_safety_item.status") {
