...
/**
 * Description of CustomerInternalCertificateProgramRepository
 *
 * @author TeamCloud
 */
class CustomerInternalCertificateProgramRepository {
    ...
    protected $columns = array('wg_customer_internal_certificate_program.*');
    ...
    public function __construct(CustomerInternalCertificateProgram $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        $query = $this->query();
        $query->select(['wg_customer_internal_certificate_program.*']);
        //$query->join("wg_customers", "wg_customers.id", "=", "wg_customer_internal_certificate_program.customer_id");
        //diagnostic_status
        //if ($join == "user") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_internal_certificate_program.customer_id") {
