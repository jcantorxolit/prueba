 ...
 * @author jdblandon
 */
class CustomerOccupationalInvestigationAlLesionDTO {
    ...
    public static function  bulkInsert($object, $entityId)
    {
        ...
        try {
            foreach ($object as $record) {
                ...
                if ($record) {
                    if ($record->id) {
                        if (!($model = CustomerOccupationalInvestigationAlLesion::find($record->id))) {
                            $isEdit = false;
                            $model = new CustomerOccupationalInvestigationAlLesion();
                        } else {
                            if (!$record->isActive) {
                                $model->delete();
                            }
        ...
        return CustomerOccupationalInvestigationAlLesion::find($model->id);
    }
