 ...
 * @author TeamCloud
 */
class CustomerAbsenteeismIndicatorTargetRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_indicator_target.*');
    ...
    public function __construct(CustomerAbsenteeismIndicatorTarget $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_indicator_target.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        //$query->join("wg_agent", "wg_agent.id", "=", "wg_customer_absenteeism_indicator_target.agent_id");
        ...
        {
            $join->on('wg_customer_absenteeism_indicator_target.period', '=', 'dtype.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_absenteeism_indicator_target.customer_id") {
