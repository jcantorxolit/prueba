 ...
 * @author TeamCloud
 */
class CustomerConfigJobActivityHazardRepository {
    ...
    protected $columns = array('wg_customer_config_job_activity_hazard.*');
    ...
    public function __construct(CustomerConfigJobActivityHazard $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_job_activity_hazard.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin("wg_config_job_activity_hazard_classification", "wg_config_job_activity_hazard_classification.id", "=", "wg_customer_config_job_activity_hazard.classification")
            ->leftjoin("wg_config_job_activity_hazard_description", "wg_config_job_activity_hazard_description.id", "=", "wg_customer_config_job_activity_hazard.description")
            ->leftjoin("wg_config_job_activity_hazard_effect", "wg_config_job_activity_hazard_effect.id", "=", "wg_customer_config_job_activity_hazard.health_effect")
            ->leftjoin("wg_config_job_activity_hazard_type", "wg_config_job_activity_hazard_type.id", "=", "wg_customer_config_job_activity_hazard.type")
            ->leftjoin(DB::raw(ConfigGeneral::getRelationTable('ND')), function($join)
            {
                $join->on('wg_customer_config_job_activity_hazard.measure_nd', '=', 'ND.id');
            })
            ->leftjoin(DB::raw(ConfigGeneral::getRelationTable('NE')), function($join)
            {
                $join->on('wg_customer_config_job_activity_hazard.measure_ne', '=', 'NE.id');
            })
            ->leftjoin(DB::raw(ConfigGeneral::getRelationTable('NC')), function($join)
            {
                $join->on('wg_customer_config_job_activity_hazard.measure_nc', '=', 'NC.id');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job_activity_hazard.job_activity_id") {
