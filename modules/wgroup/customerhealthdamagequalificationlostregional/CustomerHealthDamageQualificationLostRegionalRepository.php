 ...
 * @author TeamCloud
 */
class CustomerHealthDamageQualificationLostRegionalRepository
{
    ...
    protected $columns = array('wg_customer_health_damage_ql_first_instance.*');
    ...
    public function __construct(CustomerHealthDamageQualificationLostRegional $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_health_damage_ql_first_instance.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->join("wg_customer_health_damage_ql", function ($join) {
            $join->on('wg_customer_health_damage_ql_first_instance.customer_health_damage_qualification_lost_id', '=', 'wg_customer_health_damage_ql.id');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_health_damage_ql_first_instance.customer_health_damage_qualification_lost_id") {
