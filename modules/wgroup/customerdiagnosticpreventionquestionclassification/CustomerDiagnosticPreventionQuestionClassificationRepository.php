 ...
 * @author TeamCloud
 */
class CustomerDiagnosticPreventionQuestionClassificationRepository {
    ...
    public function __construct(CustomerDiagnosticPreventionQuestionClassification $model) {
        $this->model = $model;
    }
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_progam_prevention_question_classification.customer_diagnostic_prevention_document_id") {
