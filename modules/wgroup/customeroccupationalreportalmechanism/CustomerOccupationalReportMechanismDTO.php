...
namespace Wgroup\CustomerOccupationalReportAlMechanism;
 ...
 * @author jdblandon
 */
class CustomerOccupationalReportMechanismDTO {
    ...
    public static function  bulkInsert($object, $entityId)
    {
        ...
        try {
            foreach ($object as $record) {
                ...
                if ($record) {
                    if ($record->id) {
                        if (!($model = CustomerOccupationalReportMechanism::find($record->id))) {
                            $isEdit = false;
                            $model = new CustomerOccupationalReportMechanism();
                        } else {
                            if (!$record->isActive) {
                                $model->delete();
                            }
        ...
        return CustomerOccupationalReportMechanism::find($model->id);
    }
