 ...
 * @author jdblandon
 */
class CustomerOccupationalReportLesionDTO {
    ...
    public static function  bulkInsert($object, $entityId)
    {
        ...
        try {
            foreach ($object as $record) {
                ...
                if ($record) {
                    if ($record->id) {
                        if (!($model = CustomerOccupationalReportLesion::find($record->id))) {
                            $isEdit = false;
                            $model = new CustomerOccupationalReportLesion();
                        } else {
                            if (!$record->isActive) {
                                $model->delete();
                            }
        ...
        return CustomerOccupationalReportLesion::find($model->id);
    }
