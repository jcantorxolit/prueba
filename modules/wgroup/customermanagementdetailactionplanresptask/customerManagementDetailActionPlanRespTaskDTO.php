 ...
 * @author jdblandon
 */
class customerManagementDetailActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo customerManagementDetailActionPlanRespTask
     */
    private function getBasicInfo($model) {
        ...
        */
        ...
        $this->source = "Programas Empresariales";
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = customerManagementDetailActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new customerManagementDetailActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new customerManagementDetailActionPlanRespTask();
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new customerManagementDetailActionPlanRespTaskTracking();
        ...
        return customerManagementDetailActionPlanRespTask::find($model->id);
    }
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof customerManagementDetailActionPlanRespTask) {
                    $parsed[] = (new customerManagementDetailActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof customerManagementDetailActionPlanRespTask) {
            return (new customerManagementDetailActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new customerManagementDetailActionPlanRespTaskDTO();
            }
        }
    }
