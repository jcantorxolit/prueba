...
/**
 * Description of CertificateLogBookDocumentRepository
 *
 * @author TeamCloud
 */
class CertificateLogBookDocumentRepository {
    ...
    protected $columns = array('wg_certificate_grade_participant_logbook.*');
    ...
    public function __construct(CertificateLogBookDocument $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_certificate_grade_participant_logbook.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_certificate_grade_participant_logbook.certificate_grade_participant_id") {
