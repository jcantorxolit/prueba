 ...
 * @author TeamCloud
 */
class CustomerAbsenteeismIndirectCostRepository {
    ...
    protected $columns = array('wg_customer_absenteeism_disability_indirect_cost.*');
    ...
    public function __construct(CustomerAbsenteeismIndirectCost $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_absenteeism_disability_indirect_cost.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        //$query->join("wg_agent", "wg_agent.id", "=", "wg_customer_absenteeism_disability_indirect_cost.agent_id");


        $query->join(DB::raw("(SELECT * FROM system_parameters where system_parameters.namespace = 'wgroup' and system_parameters.group = 'absenteeism_disability_concept') absenteeism_disability_concept "), function($join)
        {
            $join->on('wg_customer_absenteeism_disability_indirect_cost.concept', '=', 'absenteeism_disability_concept.value');
        });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_absenteeism_disability_indirect_cost.customer_disability_id") {
