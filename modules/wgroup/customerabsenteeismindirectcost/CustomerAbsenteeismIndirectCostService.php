...
class CustomerAbsenteeismIndirectCostService {
    ...
    public function getAllBy($search, $perPage = 10, $currentPage = 0, $sorting = array(), $typeFilter = "", $customerId = 0, $year = 0, $workCenter = "", $classification = "") {
        ...
        $this->customerAbsenteeismDisabilityRepository = new CustomerAbsenteeismIndirectCostRepository($model);
        ...
        // sorting
        $columns = [
            'wg_customer_absenteeism_disability_indirect_cost.id',
            'wg_customer_absenteeism_disability_indirect_cost.customer_disability_id',
            'wg_customer_absenteeism_disability_indirect_cost.concept',
            'wg_customer_absenteeism_disability_indirect_cost.amount',
        ];
        ...
        if (empty($sorting)) {
            $this->customerAbsenteeismDisabilityRepository->sortBy('wg_customer_absenteeism_disability_indirect_cost.id', 'desc');
        }

        $filters = array();

        $filters[] = array('wg_customer_absenteeism_disability_indirect_cost.customer_disability_id', $customerId);

        if ($year != 0) {
            //$filters[] = array('wg_customer_absenteeism_disability_indirect_cost.customer_id', $year);
        }

        if (strlen(trim($search)) > 0) {
            $filters[] = array('absenteeism_disability_concept.item', $search);
            $filters[] = array('wg_customer_absenteeism_disability_indirect_cost.amount', $search);
        }

        if ($typeFilter == "1") {
            $filters[] = array('wg_customer_absenteeism_disability_indirect_cost.status', '1');
        } else if ($typeFilter == "0") {
            $filters[] = array('wg_customer_absenteeism_disability_indirect_cost.status', '0');
        }

        $this->customerAbsenteeismDisabilityRepository->setColumns(['wg_customer_absenteeism_disability_indirect_cost.*']);
...
from wg_customer_absenteeism_disability_indirect_cost cd
...
left join (select * from system_parameters where `group` = 'absenteeism_disability_contract_type') ct on cd.contractType COLLATE utf8_general_ci = ct.value";
    ...
    public function getSummaryDisabilityReport($customerId, $year, $cause = "")
    {
        $query = "Select
            ...
            from wg_customer_absenteeism_disability_indirect_cost wgc ";
    ...
    public function getEventNumberReport($customerId, $year, $workCenter = "", $classification = "")
    {
        ...
        $query = "select item, CONVERT(p.`value`,UNSIGNED INTEGER) nIndex
                    ...
                    left join (select 	classification,
                                        ...
                                        from wg_customer_absenteeism_disability_indirect_cost indicator
                                        $where
