 ...
 * @author TeamCloud
 */
class CustomerConfigJobActivityInterventionRepository {
    ...
    protected $columns = array('wg_customer_config_job_activity_hazard_intervention.*');
    ...
    public function __construct(CustomerConfigJobActivityIntervention $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_customer_config_job_activity_hazard_intervention.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        $query->leftjoin("wg_customer_config_job", "wg_customer_config_job.id", "=", "wg_customer_config_job_activity_hazard_intervention.job_id")
            ->leftjoin(DB::raw(SystemParameter::getRelationTable('config_workplace_status')), function($join)
            {
                $join->on('wg_customer_config_job_activity_hazard_intervention.status', '=', 'config_workplace_status.value');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job_activity_hazard_intervention.job_id") {
