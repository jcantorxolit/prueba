 ...
 * @author TeamCloud
 */
class CustomerConfigJobActivityRepository
{
    ...
    protected $columns = array('wg_customer_config_job_activity.*');
    ...
    public function __construct(CustomerConfigJobActivity $model)
    {
        $this->model = $model;
    }
    ...
    {
        $query = $this->query();
        $query->select(['wg_customer_config_job_activity.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "")
    {
        ...
        $query->leftjoin("wg_customer_config_job", "wg_customer_config_job.id", "=", "wg_customer_config_job_activity.job_id")
            ->leftjoin('wg_customer_config_activity', function ($join) {
                $join->on('wg_customer_config_activity.id', '=', 'wg_customer_config_job_activity.activity_id');
            });
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_customer_config_job_activity.job_id") {
