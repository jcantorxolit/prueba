...
/**
 * Description of ProgramPreventionQuestionRepository
 *
 * @author TeamCloud
 */
class ProgramPreventionQuestionRepository {
    ...
    protected $columns = array('wg_progam_prevention_question.*');
    ...
    public function __construct(ProgramPreventionQuestion $model) {
        $this->model = $model;
    }
    ...
    public function getFiltereds($filter = array(), $join = "", $count = false) {
        $query = $this->query();
        $query->select(['wg_progam_prevention_question.*']);
    ...
    public function getFilteredsOptional($filter = array(), $isCount = false, $join = "") {
        ...
        foreach ($filter as $key => $item) {
            try {
                ...
                if ($field == "wg_progam_prevention_question.status") {
