 ...
 * @author jdblandon
 */
class CustomerConfigHazardInterventionActionPlanRespTaskDTO {
    ...
    /**
     * @param $model: Modelo CustomerConfigHazardInterventionActionPlanRespTask
     */
    ...
    public static function  fillAndSaveModel($object)
    {
        ...
        if ($object->id) {
            // Existe
            if (!($model = CustomerConfigHazardInterventionActionPlanRespTask::find($object->id))) {
                // No existe
                $model = new CustomerConfigHazardInterventionActionPlanRespTask();
                $isEdit = false;
            }
        } else {
            $model = new CustomerConfigHazardInterventionActionPlanRespTask();
        ...
        $model->job_activity_hazard_action_plan_resp_id = $object->actionPlanRespId;
        ...
        return CustomerConfigHazardInterventionActionPlanRespTask::find($model->id);
    }
    ...
    public static function  fillAndUpdateModel($object)
    {
        ...
        if (! empty($object->tracking->action)) {
            $modelTracking = new CustomerConfigHazardInterventionActionPlanRespTaskTracking();
    ...
    public static function parse($info, $fmt_response = "1") {
        ...
        if (is_array($data) || $data instanceof Collection) {
            ...
            foreach ($data as $model) {
                if ($model instanceof CustomerConfigHazardInterventionActionPlanRespTask) {
                    $parsed[] = (new CustomerConfigHazardInterventionActionPlanRespTaskDTO())->parseModel($model, $fmt_response);
                }
            }
            return $parsed;
        } else if ($info instanceof CustomerConfigHazardInterventionActionPlanRespTask) {
            return (new CustomerConfigHazardInterventionActionPlanRespTaskDTO())->parseModel($data, $fmt_response);
        } else {
            // return empty instance
            if ($fmt_response == "1") {
                return "";
            } else {
                return new CustomerConfigHazardInterventionActionPlanRespTaskDTO();
            }
        }
    }
